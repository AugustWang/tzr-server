...
try_do_send_goods_batch(Req,QueryString)->
    ...
    ?DEBUG("try_do_send_goods_batch,RoleListArg=~w",[RoleListArg]),
    ...
    ?DEBUG("letter RoleIDList2=~w",[RoleIDList]),
    Type = get_param_int("type", QueryString),
    TypeID = get_param_int("typeid", QueryString),
...
try_do_send_goods_batch_by_condition(Req,QueryString)->
    ...
    do_send_goods_batch_by_condition(Req,{Type, TypeID, Number, BindParam, Color, Quality, Title, Content, StartTime, EndTime},{Sex,Faction,StartLevel,EndLevel,FamilyName,SelCmpParam,LastStamp}).
...
get_role_id_list_by_condition(Condition)->
    {Sex,Faction,StartLevel,EndLevel,FamilyName,SelCmpParam,LastStamp}=Condition,
    ?DEBUG("condition:~w~n",[Condition]),    
    ...
    ?DEBUG("COUNT1:~w~n",[if is_list(RoleIDList1) ->length(RoleIDList1);true->RoleIDList1 end]),
    ...
    ?DEBUG("COUNT2:~w~n",[if is_list(RoleIDList2) ->length(RoleIDList2);true->RoleIDList2 end]),
    ...
    RoleIDList3 = 
        case Guard3 of
            []->all;
            _->db:dirty_select(db_role_ext_p, [{MatchHead3, Guard3, ['$1']}])
        end,
    ?DEBUG("COUNT3:~w~n",[if is_list(RoleIDList3) ->length(RoleIDList3);true->RoleIDList3 end]),
    
    List = lists:foldl(fun(RoleIDList,Acc)-> if RoleIDList=:=all->Acc;true->[RoleIDList|Acc] end end, [], [RoleIDList1,RoleIDList2,RoleIDList3]),
    RoleIDList5=lists_filter(List),
    ?DEBUG("COUNT5:~w~n",[if is_list(RoleIDList5) ->length(RoleIDList5);true->RoleIDList5 end]), 
    RoleIDList5.
%%     RoleIDList4 = filter(RoleIDList3,RoleIDList2),
%%     ?ERROR_MSG("COUNT4:~w~n",[if is_list(RoleIDList4) ->length(RoleIDList4);true->RoleIDList4 end]),
%%     
%%     RoleIDList5 = filter(RoleIDList1,RoleIDList4),
%%     ?ERROR_MSG("COUNT5:~w~n",[if is_list(RoleIDList5) ->length(RoleIDList5);true->RoleIDList5 end]),
...
do_send_goods_batch_2(RoleIDList, Type, TypeID, Number, BindParam, Color, Quality, Title, Content, StartTime, EndTime) ->
    ...
    CreateInfo = #r_goods_create_info{bind=Bind, type=Type, bag_id=0, type_id=TypeID, start_time=StartTime,
                                      end_time=EndTime, num=Number, color=Color, quality=Quality, punch_num=0, rate=0, result=0,
...
%%@doc 单独发送道具
do_send_goods(Req,{RoleID,RoleName, Type, TypeID, Number, BindParam, Color, Quality, StartTime,EndTime,ItemName}) ->
    ?DEBUG("do_send_goods,RoleID=~w,TypeID=~w",[RoleID,TypeID]),
...
do_send_goods_2({RoleID,RoleName, Type, TypeID, Number,StartTime,EndTime}=LogSendInfo, BindParam, Color, Quality,ItemName) ->
    %% 判断是否为绑定
    Bind = case BindParam of
               "1"-> true;
               _ -> false
    ...
    Info = #r_goods_create_info{bind=Bind, type=Type, bag_id=0, type_id=TypeID, start_time=StartTime,
                                end_time=EndTime, num=Number, color=Color1, quality=Quality, punch_num=0, rate=0, result=0,
                                interface_type=present },
