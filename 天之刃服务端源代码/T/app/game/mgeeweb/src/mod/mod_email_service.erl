...
do_send_email_batch(Req) ->
    ...
    Condition={Sex,Faction,StartLevel,EndLevel,FamilyName,SelCmpParam,LastStamp,Status},
    RoleIDList = get_role_id_list_by_condition(Condition),
    ?DEBUG("RoleIDList~w, Text=~w, QueryString=~w",[RoleIDList,Content,QueryString]),
    
    common_letter:send_letter_package({send, Title, RoleIDList, Content, GoodsList}).

    
%%     case gen_server:call({global, mgeew_admin_server}, {admin_role, {send_email_batch,  Title,RoleIDList,Content,GoodsList}}) of
%%         ok ->
%%             mgeeweb_tool:return_json_ok(Req);
%%         _ ->
%%             mgeeweb_tool:return_json_error(Req)
