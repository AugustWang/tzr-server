...
-record(r_sq_fb_map_info,{fb_type,monster_level = 0,monster_total_num=0,create_list=[],monster_born_num = 0,monster_dead_num=0,
                          start_time,end_time,status=0,in_roles_info=[],roles_offline_info=[],monster_change_times=0,roles_dead_info=[]}).
...
-define(create_monster,1).
-define(create_notice,2).
...
%% born_pos:出生地点
%% type_id:怪物类型 或者广播id
-record(r_sq_create_info,{create_time,type=1,born_pos,type_id,notice_id}).

%% fb_type 副本类型
%% min_num - max_num 人数多少到多少
%% exp_rate 经验比例
-record(r_sq_fb_exp,{fb_type, min_num, max_num, exp_rate}).
