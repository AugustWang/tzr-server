...
-define(MISSION_CODE_AUTHFAIL_FAMILY_LIMIT, 23). %%任务验证失败-门派限制
...
-define(MISSION_AUTO_CODE_FAIL_NOT_START, 92). %%委托任务尚未开始
-define(MISSION_AUTO_CODE_FAIL_HAS_FINISHED, 93). %%委托任务已经完成
-define(MISSION_AUTO_CODE_FAIL_GOLD_NOT_ENOUGH, 94).  %%委托任务，扣除的元宝不足
-define(MISSION_AUTO_CODE_FAIL_MAX_LIMIT, 95). %%已经达到委托的任务上限
-define(MISSION_AUTO_CODE_FAIL_PARAM_LOOP_TIMES, 96). %%要委托的次数不对
...
-define(MISSION_LISTENER_TYPE_SPECIAL_EVENT, 5).   %%特殊事件侦听器
-define(MISSION_LISTENER_TYPE_REINFORCE, 6).%%强化装备
-define(MISSION_LISTENER_TYPE_CHANGE_QUALITY, 7).%%品质提升
-define(MISSION_LISTENER_TYPE_COLLECTION, 8).%%收集道具
