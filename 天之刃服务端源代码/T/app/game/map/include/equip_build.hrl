...
%% 装备套装属性配置表 
%% 套装id,套装名称，套装装备列表，套装子属性
-record(r_equip_whole_attr,{id,name,equip_list,sub_whole}).
%%　装备套装子属性记录结构
%%  装备子类型，装备描述（区分子类型），套装属性（使用buff加成处理）
-record(r_equip_sub_whole_attr,{id,desc,buff_list}).
