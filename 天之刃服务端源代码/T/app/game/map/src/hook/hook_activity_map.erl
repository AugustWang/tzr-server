...
-define(DO_HOOK_ACTIVITY_FUN(ConfigKey,ActKeyList,Fun),
    case common_config_dyn:find(mccq_activity,ConfigKey) of
        []-> 
            [];
        [ActKeyList] ->
    ...
    case GoodsList of
        ...
        _ ->
            lists:foreach(fun(E)-> 
                              ...
                              Tip = common_tool:get_format_lang_resources(?_LANG_ACTIVITY_AWARD_ITEM,[GoodsName]),
...
do_add_prop(RoleID,AwardPropList) when is_list(AwardPropList)->  
    case db:transaction(
        ...
        of 
        {aborted,{bag_error,not_enough_pos}=R} ->
            common_broadcast:bc_send_msg_role(RoleID,?BC_MSG_TYPE_SYSTEM,?_LANG_ACTIVITY_AWARD_ITEM_BAG_NOT_POS),
