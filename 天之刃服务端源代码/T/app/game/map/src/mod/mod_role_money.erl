...
%%%
%%% @end
%%% Created : 30 Dec 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
...
do_reduce_money({RoleID, From, ReduceList, SuccReturn, FailedReturn,ShouldNotify}) ->
    case common_transaction:transaction(fun() -> t_do_reduce_money(RoleID, ReduceList) end) of
        {atomic, RoleAttr} ->
...
t_do_reduce_money(RoleID, ReduceList) ->
    case mod_map_role:get_role_attr(RoleID) of        
        {ok, RoleAttr} ->
            RoleAttr2 = lists:foldl(
                          fun(Reduce, Attr) ->
                                  case Reduce of
                                      {gold_any, ReduceGold, ConsumeType, ConsumeDetail} ->
                                          #p_role_attr{gold=Gold, gold_bind=GoldBind} = Attr,

                                          case GoldBind >= ReduceGold of
                                              true ->
                                                  common_consume_logger:use_gold({RoleID, ReduceGold, 0, ConsumeType, ConsumeDetail}),
                                                  Attr#p_role_attr{gold_bind=GoldBind-ReduceGold};
                                              _ ->
                                                  case Gold-(ReduceGold-GoldBind) >= 0 of
                                                      true ->
                                                          common_consume_logger:use_gold({RoleID, GoldBind, (ReduceGold-GoldBind), ConsumeType, ConsumeDetail}),
                                                          Attr#p_role_attr{gold=Gold-(ReduceGold-GoldBind), gold_bind=0};
                                                      false ->
                                                          common_transaction:abort(?_LANG_ROLE_MONEY_NOT_ENOUGH_GOLD_ANY)
                                                  end
                                          end;  

                                      {gold_bind, ReduceGold, ConsumeType, ConsumeDetail} ->
                                          ...
                                          case  GoldBind >= ReduceGold of
                                              true ->
                                                  ...
                                                  Attr#p_role_attr{gold_bind=GoldBind - ReduceGold};
                                              false ->
                                                  common_transaction:abort(?_LANG_ROLE_MONEY_NOT_ENOUGH_GOLD_BIND)
                                          end;
                                      {gold, ReduceGold, ConsumeType, ConsumeDetail} ->
                                      ...
                                      {silver_any, ReduceSilver, ConsumeType, ConsumeDetail} ->
                                          #p_role_attr{silver=Silver, silver_bind=SilverBind} = Attr,
                                          case SilverBind >= ReduceSilver of
                                              true ->
                                                  common_consume_logger:use_silver({RoleID, ReduceSilver, 0, ConsumeType, ConsumeDetail}),
                                                  Attr#p_role_attr{silver_bind=SilverBind - ReduceSilver};
                                              false ->
                                                  case (Silver+SilverBind)-ReduceSilver >= 0 of
                                                      true ->
                                                          common_consume_logger:use_silver({RoleID, SilverBind,(ReduceSilver-SilverBind), ConsumeType, ConsumeDetail}),
                                                          Attr#p_role_attr{silver=(Silver+SilverBind)-ReduceSilver, silver_bind=0};
                                                      false ->
                                                          common_transaction:abort(?_LANG_ROLE_MONEY_NOT_ENOUGH_SILVER_ANY)
                                                  end
                                          end;
                                      {silver_bind, ReduceSilver, ConsumeType, ConsumeDetail} ->
                                          ...
                                          case SilverBind >= ReduceSilver of
                                              true ->
                                                  ...
                                                  Attr#p_role_attr{silver_bind=SilverBind - ReduceSilver};
                                              false ->
                                                  common_transaction:abort(?_LANG_ROLE_MONEY_NOT_ENOUGH_SILVER_BIND)
                                          end;
                                      {silver, ReduceSilver, ConsumeType, ConsumeDetail} ->
...
do_add_money({RoleID, From, ReduceList, SuccReturn, FailedReturn,ShouldNotify}) ->
    case common_transaction:transaction(fun() -> t_do_add_money(RoleID, ReduceList) end) of
        {atomic, RoleAttr} ->
            case ShouldNotify of
                true->
                    do_send_money_change(RoleID,ReduceList,RoleAttr);
