...
-export([
         get_sw_fb_map_dict/2,
         erase_sw_fb_map_dict/2,
...
%% 场景大战副本入口地图进程字典信息
init_sw_fb_enter_dict(MapId) ->
    erlang:put({?SW_FB_ENTER_DICT_PREFIX,MapId},[]).
put_sw_fb_enter_dict(MapId,DataRecordList) ->
    erlang:put({?SW_FB_ENTER_DICT_PREFIX,MapId},DataRecordList).
get_sw_fb_enter_dict(MapId) ->
    erlang:get({?SW_FB_ENTER_DICT_PREFIX,MapId}).

%% 场景大战副本入口地图进程字典信息
init_sw_fb_dict(MapId,MaxSeconds) ->
    ...
    erlang:put({?SW_FB_DICT_PREFIX,MapId},Record).
put_sw_fb_dict(MapId,Record) ->
    erlang:put({?SW_FB_DICT_PREFIX,MapId},Record).
get_sw_fb_dict(MapId) ->
    erlang:get({?SW_FB_DICT_PREFIX,MapId}).

%% 玩家跳转进入场景大战地图进程字典信息
put_sw_fb_map_dict(MapId,RoleId,FbMapProcessName) ->
    erlang:put({?SW_FB_MAP_DICT_PREFIX,MapId,RoleId},FbMapProcessName).
get_sw_fb_map_dict(MapId,RoleId) ->
    erlang:get({?SW_FB_MAP_DICT_PREFIX,MapId,RoleId}).
erase_sw_fb_map_dict(MapId,RoleId) ->
    erlang:erase({?SW_FB_MAP_DICT_PREFIX,MapId,RoleId}).
...
init(MapId, _MapName) ->
    ...
    case lists:keyfind(MapId,#r_sw_fb_npc.map_id,SwFbNpcList) of
        false ->
            ignore;
    ...
    case lists:keyfind(MapId,#r_sw_fb_mcm.fb_map_id,SeFbMcmList) of
        false ->
            ignore;
...
loop(MapId) ->
	case get_sw_fb_dict(MapId) of
		undefined ->
			ignore;
		SwFbDictRecord ->
			%%?DEBUG("SwFbDictRecord=~w",[SwFbDictRecord]),
...
loop2(MapId,SwFbDictRecord,NowSeconds) ->
    %% TODO 检查副本的结束时间
    #r_sw_fb_dict{
                  fb_status = FbStatus,fb_close_flag = FbCloseFlag,
    ...
    case SwFbDictRecord#r_sw_fb_dict.monster_number =< SwFbDictRecord#r_sw_fb_dict.cur_monster_number
        andalso RoleIdList =/= [] of
        true -> %% 副本完成 
            ?TRY_CATCH(common_mod_goal:finish_scene_war_fb(RoleIdList,SwFbDictRecord#r_sw_fb_dict.fb_type),HookGoalError);
        _ ->
            ignore
    end,
    ok.
...
%% catch global:send(FbMapProcessName, {mod_scene_war_fb,{init_sw_fbcollect,CollectInfo}})
do_handle_info({init_sw_fbcollect,CollectInfo}) ->
    do_init_sw_fbcollect(CollectInfo);
...
do_scene_war_fb_enter2(Unique, Module, Method, DataRecord, RoleId, PId) ->
    #m_scene_war_fb_enter_tos{npc_id = NpcId, 
                              ...
                              fb_level = FbLevel,
                              fb_id = FbId,
                              fb_seconds = FbSeconds} = DataRecord,
        ...
        case lists:foldl(
               fun(SwFbNpcRecordT,{AccFbNpcFlag,AccFbNpcRecord}) ->
                       if AccFbNpcFlag =:= false
                          andalso NpcId =:= SwFbNpcRecordT#r_sw_fb_npc.npc_id
    ...
    if RoleMapInfo#p_map_role.level >= SwFbMcmRecord#r_sw_fb_mcm.min_level 
       andalso SwFbMcmRecord#r_sw_fb_mcm.max_level >= RoleMapInfo#p_map_role.level ->
            next;
       true ->
            erlang:throw({error,common_tool:get_format_lang_resources(?_LANG_SCENE_WAR_FB_ENTER_LEVEL,SwFbMcmRecord#r_sw_fb_mcm.min_level),0})
    end,
    ...
    if RoleMapInfo#p_map_role.team_id =/= 0
       andalso FbId =/= 0 andalso FbSeconds =/= 0 ->
            ...
            case ( CurMapId =/= SwFbNpcRecord#r_sw_fb_npc.map_id) of
                true ->
...
get_async_create_map_info(Key) ->
    erlang:get({mod_scene_war_fb, Key}).
...
do_scene_war_fb_enter4({Unique, Module, Method, DataRecord, RoleId, PId},
    ...
    %% 采集物在副本中出生,计算采集物的生成 TODO
    case do_sw_fb_born_collect(SwFbMcmRecord) of
        {error, CollectReason} ->
            ?DEBUG("~ts,Error=~w",["场景大战副本采集物",CollectReason]),
            undefined;
        {ok,CollectInfo} ->
            (catch global:send(FbMapProcessName, {mod_scene_war_fb,{init_sw_fbcollect,CollectInfo}}))
    end,
    ...
    lists:foreach(
      fun({MemberRoleId, _, MemberAttr, _, MemberSwFbFeeRecord, _MemberTimes}) ->
              ...
              case FbType =:= ?SCENE_WAR_FB_TYPE_DXWL of
                  true ->
                      %% 特殊任务事件
                      ?TRY_CATCH(hook_mission_event:hook_special_event(MemberRoleId,?MISSON_EVENT_FB_TYPE_5),MissionEventErr);
                  _ ->
                      ignore
              end,
              %% 切换地图
              put_sw_fb_map_dict(mgeem_map:get_mapid(),MemberRoleId,FbMapProcessName),
...
do_scene_war_fb_enter5({Unique, Module, Method, DataRecord, RoleId, PId},
    ...
    SendSelf = #m_scene_war_fb_enter_toc{succ = true,return_self = true,
                                         ...
                                         fb_seconds = SceneFbRecord#r_scene_war_fb.fb_seconds,
                                         fb_times = FbTimes},
    ?DEBUG("~ts,SendSelf=~w",["进入场景大战副本返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
    ...
    case SceneFbRecord#r_scene_war_fb.fb_type =:= ?SCENE_WAR_FB_TYPE_DXWL of
        true ->
            %% 特殊任务事件
            ?TRY_CATCH(hook_mission_event:hook_special_event(RoleId,?MISSON_EVENT_FB_TYPE_5),MissionEventErr);
        _ ->
            ignore
    end,
    put_sw_fb_map_dict(mgeem_map:get_mapid(),RoleId,FbMapProcessName),
    {DestTx,DestTy} = get_sw_fb_map_born_point(FbMapId),
...
hook_create_monster(FbMapProcessName,SwFbMcmRecord,MapInfoList)->
	case SwFbMcmRecord#r_sw_fb_mcm.born_monster of
		...
		?SCENE_WAR_FB_BORN_MONSTER_CALL->
%% 			case common_config_dyn:find(scene_war_fb,{sw_fb_call_monster,SwFbMcmRecord#r_sw_fb_mcm.fb_map_id}) of
%% 				[]->
%% 					ignore;
%% 				[FbCallMonsterList]->
%% 					case lists:keyfind(?CALL_MONSTER_HEAD, #r_sw_fb_call_monster.auto_call, FbCallMonsterList) of
%% 						false->
%% 							ignore;
%% 						FbCallMonster->
%% 							(catch global:send(FbMapProcessName, {mod_scene_war_fb, {init_sw_monster, 
%% 																					 FbCallMonster#r_sw_fb_call_monster.pass_id}}))
%% 					end
%% 			end
            ignore
	end.
...
%% 场景采集物计算
%% 返回 {ok,r_sw_fb_collect} or {error,Reason}
do_sw_fb_born_collect(SwFbMcmRecord) ->
    #r_sw_fb_mcm{fb_type = FbType,fb_level = FbLevel} = SwFbMcmRecord,
    [SwFbCollectList] = common_config_dyn:find(?SCENE_WAR_FB_CONFIG,sw_fb_collect),
    SwFbCollectList2 = 
        lists:foldl(
          fun(SwFbCollectT,AccSwFbCollectList) ->
                  if FbType =:= SwFbCollectT#r_sw_fb_collect.fb_type
                     andalso FbLevel =:= SwFbCollectT#r_sw_fb_collect.fb_level ->
                          [SwFbCollectT|AccSwFbCollectList];
                     true ->
                          AccSwFbCollectList
                  end
          end,[],SwFbCollectList),
    SwFbCollectList3 = 
        if SwFbCollectList2 =:= [] ->
                lists:foldl(
                  fun(SwFbCollectTT,AccSwFbCollectListT) ->
                          if FbType =:= SwFbCollectTT#r_sw_fb_collect.fb_type
                             andalso SwFbCollectTT#r_sw_fb_collect.fb_level =:= 0 ->
                                  [SwFbCollectTT|AccSwFbCollectListT];
                             true ->
                                  AccSwFbCollectListT
                          end
                  end,[],SwFbCollectList);
    ...
    do_sw_fb_born_collect2(SwFbMcmRecord,SwFbCollectList3).
do_sw_fb_born_collect2(_SwFbMcmRecord,SwFbCollectList) ->
    if SwFbCollectList =:= [] ->
            {error,not_found};
       true ->
            WeightList = [Weight || #r_sw_fb_collect{weight = Weight} <- SwFbCollectList],
            case mod_refining:get_random_number(WeightList,0,-1) of
                -1 ->
                    {error,miss};
                HitIndex ->
                    {ok,lists:nth(HitIndex,SwFbCollectList)}
            end
    end.
%% 场景大战副本费用变化通知
...
do_t_scene_war_fb_enter_fee(RoleId, RoleAttr, Fee, Type) ->
...
do_scene_war_fb_quit3({Unique, Module, Method, DataRecord, RoleId, PId},
    ...
    SendSelf = #m_scene_war_fb_quit_toc{
      ...
      fb_type =FbType,
      fb_level = FbLevel},
    ?DEBUG("~ts,SendSelf=~w",["退出场景大战副本返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
...
do_scene_war_fb_query2(RoleId, DataRecord) ->
...
do_role_dead2(RoleId,_MapRoleInfo,MapId,SwFbDictRecord) ->
    ...
    ok.
%% 初始化副本
%% CollectInfo 结构为 p_collect_point_base_info
do_init_sw_fbcollect(CollectInfo) ->
    %% 调用采集物接口生成采集物 保存采集物的相关数据
    MapId = mgeem_map:get_mapid(),
    ?DEBUG("~ts,MapId=~w,CollectInfo=~w",["场景大战副本采集物信息",MapId,CollectInfo]),
    CollectList = 
        lists:foldl(
          fun(CollectPointBaseInfo,AccCollectList) ->
                  case catch mod_map_collect:new_point(CollectPointBaseInfo#p_collect_point_base_info{mapid = MapId}) of
                      {_, []} ->
                          AccCollectList;
                      {_Point,CollectsT} when erlang:is_list(CollectsT) ->
                          lists:append([CollectsT,AccCollectList]);
                      Error ->
                          ?ERROR_MSG("~ts,Error=~w",["场景大战副本采集物出生出错",Error]),
                          AccCollectList
                  end
          end,[],CollectInfo#r_sw_fb_collect.collects),
    SwFbDictRecord = get_sw_fb_dict(MapId),
    put_sw_fb_dict(MapId,SwFbDictRecord#r_sw_fb_dict{collect_info = CollectList}).
%% 初始化场景副本怪物
       ...
       true ->
            case get_sw_fb_dict(mgeem_map:get_mapid()) of
                ...
                #r_sw_fb_dict{fb_type=FbType} ->
                    [QuitNpcName] = common_config_dyn:find(?SCENE_WAR_FB_CONFIG, {quit_npc_name, FbType}),
