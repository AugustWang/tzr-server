...
do_stall_list_refresh() ->
    ...
    StallList2 =
        lists:foldl(
          fun(#r_stall{role_id=RoleId, remain_time=RemainTime}, AccStallList) ->
...
get_stall_list_item(StallGoods, Mod) ->
    ...
    if GoodsDetail#p_goods.type =:= ?TYPE_EQUIP ->
            ...
            Pro = get_equip_job_type(EquipBaseInfo#p_equip_base_info.kind);
       GoodsDetail#p_goods.type =:= ?TYPE_ITEM ->
            {ok, ItemBassInfo} = mod_item:get_item_baseinfo(GoodsDetail#p_goods.typeid),
            Pro = get_item_job_type(ItemBassInfo#p_item_base_info.kind);
       true ->
            Pro = undefined
    end,
    if Mod =:= new ->
...
get_equip_job_type(Kind) when is_integer(Kind)->
    if Kind =:=101 orelse Kind =:=102 orelse Kind =:= 103 orelse Kind =:= 104->
            Kind rem 100;
        true->
            undefined
    end;
get_equip_job_type(_)->
    undefined.

get_item_job_type(Kind) when is_integer(Kind)->
    if Kind =:=3001 orelse Kind =:=3002 orelse Kind =:= 3003 orelse Kind =:= 3004->
            Kind rem 1000;
        true->
            undefined
    end;
...
do_clear_stall_list() ->
    try
        mod_mysql:do_sql(lists:flatten(io_lib:format("truncate table ~w", [?STALL_LIST_TAB])))
    catch
        _:Reason ->
            ?ERROR_MSG("do clear stall list error, reason: ~w stack: ~w", [Reason, erlang:get_stacktrace()])
    end.
    ...
    case erlang:is_list(StallList) andalso erlang:length(StallList)>0 of
        true->
            try
                ...
                _:Reason->
                    ?ERROR_MSG("do dump stall list error, reason: ~w stack: ~w", [Reason, erlang:get_stacktrace()])
            end;
        false->
            ignore
    end.
...
do_delete_stall_list(IdList) ->
    try
        ...
        _:Reason ->
            ?ERROR_MSG("do delete stall list error, reason: ~w, stack: ~w", [Reason, erlang:get_stacktrace()])
    end.
...
do_update_stall_list(Id, StallListItem) ->
    try
        ...
        _:Reason ->
            ?ERROR_MSG("do update stall list error, reason: ~w, stack: ~w", [Reason, erlang:get_stacktrace()])
    end.
...
do_select_stall_list(SortType) ->
    ...
    case mod_mysql:select(SelectSql) of
        ...
        StallListError ->
            ?ERROR_MSG("get stall list error: ~w", [{StallListError}]),
            ok
    end.
