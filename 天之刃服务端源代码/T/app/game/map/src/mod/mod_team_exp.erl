...
add_exp4(ExpRecordList) ->
    ?DEBUG("~ts,ExpRecordList=~w",["怪物经验原始记录处理",ExpRecordList]),
    ...
    ?DEBUG("~ts,ExpRecordList=~w",["怪物经验是否组队分享处理",ExpRecordList2]),
...
add_exp8(ExpRecord, TeamSubList) ->
    ...
    lists:foreach(fun(RoleExp) ->
                          #r_monster_team_sub_exp{role_id=RoleId, team_exp=Exp, status=Status, kill_flag=KillFlag,
                                                  level=_Level, energy_index=EnergyIndex} = RoleExp,
                          ...
                          ?DEBUG("[Mod Team Exp] ~ts RoleId=~p,Exp=~p,NewExp=~p,Status=~w,KillFlag=~w",
                                 ["角色获取的怪物经验是：", RoleId,Exp,Exp3,Status,KillFlag]),
                          ...
                          mod_map_role:do_monster_dead_add_exp(RoleId, Exp5, MonsterType, Status, KillFlag, EnergyIndex)
                  end,TeamSubList).
%% id,唯一标记，killer_id 杀死怪物的RoleId, map_id 地图id monster_id  怪物id, ,monster_type 怪物类型 
%% monster_tx,monster_ty 怪物死亡坐标，role_exp_list 获取得经验的玩家记录类型r_monster_role_exp
%% team_exp_list  队伍经验记录r_monster_team_exp
...
add_exp6_1(ExpRecord,TeamExpRecord) ->
    ...
    case global:whereis_name(common_misc:get_team_proccess_name(TeamId))of
        ...
        _ ->
            ...
            ?DEBUG("~ts,TeamMemberList=~w",["获取队伍的成员",TeamMemberList]),
...
get_fb_exp_rate(MemberCount,MapID)->
    case mod_shuaqi_fb:is_shuaqi_fb_map_id(MapID) of
        true->
            mod_shuaqi_fb:get_sq_fb_exp_rate(MemberCount,MapID);
        false->
            get_fb_exp_rate1(MemberCount,MapID)
    end.
get_fb_exp_rate1(MemberCount,MapID)->
    case mod_scene_war_fb:is_scene_war_fb_map_id(MapID) of
        ...
        false->
            get_fb_exp_rate2(MemberCount,MapID)
    end.
get_fb_exp_rate2(MemberCount,MapID)->
    case mod_exercise_fb:is_exercise_fb_map_id(MapID) of
        true->
            mod_exercise_fb:get_exe_fb_exp_rate(MemberCount,MapID);
        false->
            100
    end.
...
check_team_member(MapID, MonsterTX, MonsterTY, RoleID, MaxPixels) ->
    case mod_map_actor:get_actor_txty_by_id(RoleID, role) of
        undefined ->
            ?DEBUG("[Mod Team Exp] ~ts",["队员不在同一地图，不能获取经验"]),
            error;
        {TX, TY} ->
...
get_multi_exp(RoleId, Exp, BuffType) ->
    ?DEBUG("~ts,RoleId=~w, Exp=~w, BuffType=~w",["获取多倍经验",RoleId, Exp, BuffType]),
    case mod_map_role:get_role_base(RoleId) of
        {ok,RoleBase} ->
...
get_multi_exp4(_RoleId,Exp,_RoleBase,_Buff,Buf) ->
    if Buf#p_buf.absolute_or_rate =:= 0 ->
       ...
       true ->
            ...
            ?DEBUG("~ts,Value=~w",["获取角色使用的经验倍数",Buf#p_buf.value]),
            (Exp * Value)/10000
    end.
...
get_vip_exp(RoleID, Exp) ->
    ...
    [SafeMapIDList] = common_config_dyn:find(etc, safe_map),
    IsSafeMap = lists:member(MapID, SafeMapIDList),
    case IsSafeMap andalso mod_vip:is_role_vip(RoleID) of
        true ->
            [VipMultiple] = common_config_dyn:find(etc, safe_map_vip_multiple),
            Exp * VipMultiple;
        _ ->
            Exp
    end.
