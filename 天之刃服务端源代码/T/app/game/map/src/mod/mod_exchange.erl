...
%%%
%%% @end
%%% Created : 27 May 2010 by  QingliangCn <qing.liang.cn@gmail.com>
%%%-------------------------------------------------------------------
-module(mod_exchange).
    ...
    case ets:lookup(?ETS_EXCHANGE_FLOW, RoleID) of
        [{RoleID, OtherRoleID, Type, OldStatus, 0, 0, []}] ->
            case ets:lookup(?ETS_EXCHANGE_FLOW, OtherRoleID) of
                %%两条记录的status应该是一样的
                [{OtherRoleID, RoleID, OtherType, OldStatus, OtherSilver, OtherGold, OtherGoods}] ->
                    NewStatus = get_new_status_after_lock(Type, OldStatus),
                    ets:insert(?ETS_EXCHANGE_FLOW, 
                               {RoleID, OtherRoleID, Type, NewStatus, Silver, Gold, Goods}),
                    ets:insert(?ETS_EXCHANGE_FLOW, 
                               {OtherRoleID, RoleID, OtherType, NewStatus, OtherSilver, OtherGold, OtherGoods}),
                    %%通知双方操作的结果
...
do_confirm(Unique, Module, Method, DataIn, RoleID, PID, Line) ->
    ...
    case check_exchange_can_confirm(RoleID) of
        true ->
        ...
        [{OtherRoleID, RoleID, _OtherType, State, OtherSilver, OtherGold, OtherGoods}] ->
            case do_exchange(RoleID, OtherRoleID,{Silver, Gold, Goods}, 
                ...
                ok ->
                    ...
                    common_hook_achievement:hook({mod_exchange,{exchange_confirm,RoleID,OtherRoleID}}),
                    ok;
...
t_exchange_goods(RoleID, OtherRoleID, Goods, OtherGoods, RoleName, OtherRoleName) ->
    %% 检测是否有绑定物品
    Goods2 = t_has_not_bind_goods(RoleID, Goods),
    OtherGoods2 = t_has_not_bind_goods(OtherRoleID, OtherGoods),
...
%% 判断是否有绑定的物品
t_has_not_bind_goods(RoleID, Goods) ->
    lists:foldl(
      fun(GoodsID, Acc) ->
              case mod_bag:check_inbag(RoleID, GoodsID) of
                  {ok, GoodsInfo} ->
...
check_exchange_can_confirm(RoleID) ->
    case ets:lookup(?ETS_EXCHANGE_FLOW, RoleID) of
        [{RoleID, _OtherRoleID, Type, State, _, _, _}] ->
