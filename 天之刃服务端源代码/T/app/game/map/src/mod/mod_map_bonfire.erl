...
calc_bonfire_add(#p_map_bonfire{id=ID}) ->
    ...
    {RateList, _} = 
        lists:foldl(
          fun(?BONFIRE_TYPE_FAMILY, {Acc,RoleIDs}) -> %%只有门派地图才有这种，篝火类型
        ...
        {X,Y} ->
            ...
            lists:foldl(
              fun(#p_map_bonfire{id=ID,state=?BONFIRE_BURN_STATE, pos=#p_pos{tx=TX,ty=TY}}=Bonfire,{Falg,Rate1}) ->
                      ...
                      XV = erlang:abs(TX-X),
                      XY = erlang:abs(TY-Y),
                      case Range >= XV andalso Range >= XY of
                          true ->
