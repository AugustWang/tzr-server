...
-export([init_role_data/1,handle/1,get_give_score/1]).
...
handle({Unique,?FLOWERS,?FLOWERS_GET_RECEVER_INFO,DataIn,_RoleID,PID,_Line,_State}) ->    
    ...
    Data = case common_misc:get_roleid(RoleName) of
               ...
               RRoleID ->
                   ...
                   {ok,RoleAttr} =  common_misc:get_dirty_role_attr(RRoleID),
                   #m_flowers_get_recever_info_toc{succ=true,rolebase=RoleBase,roleattr=RoleAttr}
           end,
...
handle({{give_flowers,FlowersInfo,GiveBase,ReceBase,IsAnonymous},_State}) ->
    ...
    of
        ...
        {atomic, RoleFlowerInfo} ->   
            ...
            common_hook_achievement:hook({mod_flowers,{rece_flowers,GiveBase#p_role_base.role_id,ReceBase#p_role_base.role_id,NewReceAttr#p_role_attr.charm}}),
            %%?DEBUG("RoleAttr:~w, NewRoleAttr:~w~n",[ReceAttr,NewReceAttr]),
            case ReceBase#p_role_base.sex of
                ?FEMALE->
                    RoleReceFlowersRank = common_ranking:get_rece_flowers_rank_record(ReceBase,NewReceAttr),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_rece_flowers,{update,RoleReceFlowersRank}}),
                    RoleReceFlowersTodayRank = common_ranking:get_rece_flowers_today_rank_record(ReceBase,NewReceAttr,FlowersInfo#r_flower.charm),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_rece_flowers_today,{update,RoleReceFlowersTodayRank}});
                _->
                    ignore
            end,
            ...
            RegName = common_misc:get_role_line_process_name(binary_to_list(ReceBase#p_role_base.account_name)),
...
recever_online_handle(RoleID,Goods,FlowersInfo,SelfRoleBase,ReceRoleBase,IsAnonymous) ->
    ...
    of
        ...
        {atomic, {ok,UpdateGoodsList,SelfRoleAttr,NewScore,AddScore}} ->
            ...
            %% 成就 add by caochuncheng 2011-03-08
            common_hook_achievement:hook({mod_flowers,{give_flowers,RoleID,ReceRoleBase#p_role_base.role_id,AddScore}}),
            catch global:send(mod_friend_server,{give_flower,RoleID,ReceRoleBase#p_role_base.role_id,Goods#p_goods.typeid}),
            case SelfRoleBase#p_role_base.sex of
                ?MALE->
                    RoleGiveFlowersRank = common_ranking:get_give_flowers_rank_record(SelfRoleBase,SelfRoleAttr,NewScore),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_give_flowers,{update,RoleGiveFlowersRank}}),
                    RoleGiveFlowersTodayRank = common_ranking:get_give_flowers_today_rank_record(SelfRoleBase,SelfRoleAttr,AddScore),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_give_flowers_today,{update,RoleGiveFlowersTodayRank}});
                _->
                    ignore
            end,
            common_misc:update_goods_notify({role,RoleID},UpdateGoodsList),
            common_misc:send_to_rolemap( ReceRoleBase#p_role_base.role_id, 
                                         {mod_flowers,
                                          {give_flowers,FlowersInfo,
                                           SelfRoleBase,ReceRoleBase,IsAnonymous}}),
...
recever_offline_handle(RoleID,Goods,FlowersInfo,SelfRoleBase,ReceRoleBase,IsAnonymous) ->
    ...
    of
        ...
        {atomic,{ok,UpdateGoodsList,SelfRoleAttr,ReceRoleAttr,NewScore,AddScore,AddCharm}} ->
            ...
            %% 成就 add by caochuncheng 2011-03-08
            common_hook_achievement:hook({mod_flowers,{give_flowers,RoleID,ReceRoleBase#p_role_base.role_id,AddScore}}),
            common_hook_achievement:hook({mod_flowers,{rece_flowers,RoleID,ReceRoleBase#p_role_base.role_id,ReceRoleAttr#p_role_attr.charm}}),
            catch global:send(mod_friend_server,{give_flower,RoleID,ReceRoleBase#p_role_base.role_id,Goods#p_goods.typeid}),
            case SelfRoleBase#p_role_base.sex of
                ?MALE->
                    RoleGiveFlowersRank = common_ranking:get_give_flowers_rank_record(SelfRoleBase,SelfRoleAttr,NewScore),
            ...
            case ReceRoleBase#p_role_base.sex of
                ?FEMALE->
                    RoleReceFlowersRank = common_ranking:get_rece_flowers_rank_record(ReceRoleBase, ReceRoleAttr),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_rece_flowers,{update,RoleReceFlowersRank}}),
                    RoleReceFlowersTodayRank = common_ranking:get_rece_flowers_today_rank_record(ReceRoleBase,ReceRoleAttr,AddCharm),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_rece_flowers_today,{update,RoleReceFlowersTodayRank}});
                _->
                    ignore
            end,
...
broadcast(faction,BroadcastInfo,GiveBase,ReceBase) ->
    ...
    of
        true ->
            ignore;
        false ->
            RegName = common_misc:get_role_line_process_name(binary_to_list(GiveBase#p_role_base.account_name)),
            Pid = global:whereis_name(RegName),
