...
finish(succ, PInfo, RequestRecord) ->
    ...
    Func = {func,fun()->  
                     ...
                     case ShouldGiveRewardProp of
                         true->
                             RewardProp = mod_mission_misc:get_choose_prop_reward(RequestRecord,FinalPropList),
                             #p_mission_prop{prop_id=PropID,prop_type=PropType} = RewardProp,
                             PropName = mod_mission_misc:get_prop_name(PropID,PropType),
                             RoleNameStr = get_role_name_str(RoleID),
                             Notify = io_lib:format(?_LANG_MISSION_SB_GET_GOOD_THING, [RoleNameStr, PropName]),
                     ...
                     catch catch common_hook_achievement:hook({mod_mission,{shoubian,RoleID}}) 
            end},
...
%%@doc 侦听器触发
listener_trigger(_ListenerData, _PInfo) -> ok.
