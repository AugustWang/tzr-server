%% Author: chixiaosheng
%% Created: 2011-4-9
%% Description: 道具搜集模型 - 2次对话
%%      model_status: 必须是2个Status
%%      listener：必须是道具的侦听器
-module(mission_model_4, [RoleID, MissionID, MissionBaseInfo]).
        ...
        CurrentModelStatus =:= ?MISSION_MODEL_STATUS_FIRST ->
            ...
            ListenerList = 
                lists:map(fun(ListenerDataConfig) ->
                                  ...
                                  %%直接将玩家侦听器数据设置为已经满足要求
                                  #p_mission_listener{type=?MISSION_LISTENER_TYPE_PROP,
