...
handle({Unique, Module, ?SYSTEM_CONFIG_CHANGE, DataIn, RoleID, Line}) ->
    do_config_change(Unique, Module, ?SYSTEM_CONFIG_CHANGE, DataIn, RoleID, Line);
handle({Unique, Module, ?SYSTEM_PK_NOT_AGREE, _DataIn, RoleID, Line}) ->
...
sys_config_init(RoleID, Line) ->
    ...
    common_misc:unicast(Line, RoleID, ?DEFAULT_UNIQUE, ?SYSTEM, ?SYSTEM_CONFIG, R).
