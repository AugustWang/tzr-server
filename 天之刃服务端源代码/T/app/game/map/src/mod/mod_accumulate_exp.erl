...
do_refresh_use_gold(Unique, Module, Method, AccumulateExp, RoleID, PID) -> 
    ...
    case common_transaction:t(
           fun() ->
                   {ok, #p_role_attr{gold=Gold, gold_bind=GoldBind, level=Level} = RoleAttr} = mod_map_role:get_role_attr(RoleID),
                   NeedGold = get_need_gold_by_level(Level),
                   case GoldBind >= NeedGold of
                       true ->
                           common_consume_logger:use_gold({RoleID, NeedGold, 0, 
                                                           ?CONSUME_TYPE_GOLD_REFRESH_ACCUMULATE_EXP, 
                                                           ""}),
                           NewRoleAttr = RoleAttr#p_role_attr{gold_bind=GoldBind-NeedGold};
                       false ->
                           case Gold >= NeedGold of
                               true ->
                                   common_consume_logger:use_gold({RoleID, 0, NeedGold, 
                                                                   ?CONSUME_TYPE_GOLD_REFRESH_ACCUMULATE_EXP, 
                                                                   ""}),
                                   NewRoleAttr = RoleAttr#p_role_attr{gold=Gold-NeedGold};
                               false ->
                                   NewRoleAttr = RoleAttr,
                                   Error = common_tool:to_binary(lists:flatten(io_lib:format(?_LANG_ACCUMULATE_EXP_NOT_ENOUGH_GOLD, [NeedGold]))),
...
%%                             RefreshVip = mod_vip:check_can_refresh_exp_free(RoleID, OldRate),
%%                             if
%%                                 RefreshVip =:= ok ->
%%                                     do_refresh_free(Unique, Module, Method, R, RoleID, PID);
%%                                 true ->
...
do_add_exp(Unique, Module, Method, RoleID, PID,Exp,Rec) ->
    case common_transaction:t(
           fun() ->                 
                   {Exp, mod_map_role:t_add_exp(RoleID, Exp, ?EXP_ADD_TYPE_NORMAL)}
           end)
    of 
