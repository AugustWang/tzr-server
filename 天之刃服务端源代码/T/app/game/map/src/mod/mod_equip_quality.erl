...
do_up_equip_quality2(RoleId,DataRecord) ->
    ...
    %% 改造费用
    RefiningFeeRecord =#r_refining_fee{
      type = equip_quality_fee,
      equip_level = EquipGoods#p_goods.level,
      material_level = QualityMaterialLevel,
      material_number = 1,
...
do_up_equip_quality5({Unique, Module, Method, DataRecord, RoleId, PId, Line},
    ...
    case IsSucc =:= true of
        true ->
            Reason = common_tool:get_format_lang_resources(?_LANG_UPQUALITY_SUCC,[EquipGoods#p_goods.quality_rate]),
            ReasonCode = 0;
        _ ->
