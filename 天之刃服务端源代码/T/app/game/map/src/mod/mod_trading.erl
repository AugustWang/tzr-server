...
%% 刷新价格，这时最佳商线会变化，需要控制不要重复刷新
reload_trading_shop_goods_price() ->
    ?DEBUG("~ts",["当前时间点需要重新变化商线，复杂的操作开始了"]),
    TradingShopList= get_trading_shop_info(),
...
do_trading_shop2({_Unique, _Module, _Method, DataRecord, RoleId, _PId, _Line}) ->
    ...
    if MapId =:= CurMapId ->
            next;
       true ->
            ?DEBUG("~ts,DataRecord=~w,CurMapId=~w",["参数错误map_id",DataRecord,CurMapId]),
    ...
    case lists:foldl(
        ...
        true ->
            next;
        false ->
            ?DEBUG("~ts,DataRecord=~w",["根据map_id,npc_id,type_id相关信息在本地图查找不到相应的商贸商店信息",DataRecord]),
            erlang:throw({error,?_LANG_TRADING_SHOP_PARAM_ERROR})
    end,
    ShopInfo = 
        case mod_trading_common:get_shop_goods(MapId,NpcId) of
            undefined ->
                ?DEBUG("~ts,MapId=~w,NpcId=~w",["此地图此NPC没有相关的商贸商店信息",MapId,NpcId]),
                erlang:throw({error,?_LANG_TRADING_SHOP_PARAM_ERROR});
            ShopInfoT ->
                ShopInfoT
        end,
...
do_trading_shop3({Unique, Module, Method, DataRecord, RoleId, PId, _Line},ShopInfo) ->
    ...
    ?DEBUG("~ts,SendSelf=~w",["获取商贸商店信息返回结果",SendSelf]),
...
do_trading_buy2({_Unique, _Module, _Method, DataRecord, RoleId, _PId, _Line}) ->
    ...
    if Number > 0 
       andalso Number =< ShopMapGroupGoodsNumber ->
            next;
       true ->
            ?DEBUG("~ts,Number=~w",["物品购买数量不合法",Number]),
            erlang:throw({error,?_LANG_TRADING_BUY_PARAM_ERROR})
    end,      
    %% 检查玩家是否在NPC附近
    case check_valid_distance(RoleId,NpcId) of
    ...
    if erlang:length(RoleGoodsList) =:= 0
       ...
       andalso MapId =:= RoleTrading#r_role_trading.map_id ->
            next;
       true ->
            ?DEBUG("~ts,RoleGoodsList=~w",["玩家还有其它商贸商店购买的物品未出售，无法购买新的物品",RoleGoodsList]),
    ...
    if RoleBill >= RoleMaxBill ->
            ?DEBUG("~ts,,RoleBill=~w,RoleMaxBill=~w",["玩家商贸商票价值大于商票最大价格，无法购买新的物品",RoleBill,RoleMaxBill]),
    ...
    if RoleBill >= SunGoodsPrice ->
            next;
       true ->
            ?DEBUG("~ts,SunGoodsPrice=~w,RoleBill=~w",["商贸商票银子不够，无法购买这么多的物品",SunGoodsPrice,RoleBill]),
...
do_trading_buy5({Unique, Module, Method, DataRecord, RoleId, PId, _Line},
    ...
    ?DEBUG("~ts,SendSelf=~w",["商贸商店购买物品返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
    FactionId = RoleMapInfo#p_map_role.faction_id,
...
do_trading_buy_error({Unique, Module, Method, DataRecord, _RoleId, PId, _Line},Reason) ->
    ...
    ?DEBUG("~ts,SendSelf=~w",["商贸商店购买物品返回结果",SendSelf2]),
...
do_trading_sale5({Unique, Module, Method, DataRecord, RoleId, PId, _Line},
    ...
    ?DEBUG("~ts,SendSelf=~w",["出售商贸货舱物品返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
    FactionId = RoleMapInfo#p_map_role.faction_id,
    %% 将玩家的商贸信息同步到需要此数据的其它商贸的地图进程
    do_sync_role_trading_info(FactionId,MapId,RoleId,RoleTrading).
...
do_trading_get({Unique, Module, Method, DataRecord, RoleId, PId, Line}) ->
    case catch do_trading_get2({Unique, Module, Method, DataRecord, RoleId, PId, Line}) of
        ...
        {ok,RoleMapInfo,RoleAttr,RoleBillInfo} ->
            do_trading_get3({Unique, Module, Method, DataRecord, RoleId, PId, Line},
                           RoleMapInfo,RoleAttr,RoleBillInfo)
    end.
do_trading_get2({_Unique, _Module, _Method, DataRecord, RoleId, _PId, _Line}) ->
    ...
    RoleBillInfo = 
        case get_role_trading_bill(RoleLevel) of
            {error,_Reason} ->
                ?DEBUG("~ts,RoleLevel=~w",["玩家级别无法查找到获取的商票配置信息",RoleLevel]),
                erlang:throw({error,?_LANG_TRADING_GET_PARAM_ERROR});
            {ok,RoleBillInfoT} ->
                RoleBillInfoT
        end,
    %% 玩家是否加入门派
    if RoleMapInfo#p_map_role.family_id =/= 0 ->
            next;
       true ->
            ?DEBUG("~ts",["玩家没加入门派，不可以领取商贸商票"]),
            erlang:throw({error,?_LANG_TRADING_GET_NOT_FAMILY})
    end,
    FactionId = RoleMapInfo#p_map_role.faction_id,
    case check_trading_bill_npc(FactionId,MapId,NpcId) of
        false ->
            ?DEBUG("~ts,DataRecord=~w",["根据传查找不到相关的领取商贸商票NPC的配置",DataRecord]),
            erlang:throw({error,?_LANG_TRADING_GET_PARAM_ERROR});
        true ->
            next
    end,
...
do_trading_get3({Unique, Module, Method, DataRecord, RoleId, PId, Line},
    ...
    case db:transaction(
           fun() -> 
                   do_t_trading_get(RoleId,DataRecord,RoleMapInfo,RoleAttr,RoleBillInfo)
           end) of
        ...
        {aborted, Reason} ->
            Reason2 = 
                case Reason of
                    {throw,{bag_error,BR}} ->
                        ?DEBUG("~ts,bag_error=~w",["领取商贸商票背包操作错误，兑换失败",BR]),
                        case BR of
                            not_enough_pos ->
...
do_trading_get4({Unique, Module, Method, DataRecord, RoleId, PId, Line},
    ...
    SendSelf = #m_trading_get_toc{
      ...
      award_type = RoleTrading#r_role_trading.award_type},
    ?DEBUG("~ts,SendSelf=~w",["领取商贸商票返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
    FactionId = RoleMapInfo#p_map_role.faction_id,
    MapId = DataRecord#m_trading_get_tos.map_id,
    do_sync_role_trading_info(FactionId,MapId,RoleId,RoleTrading),
...
do_t_trading_get(RoleId,DataRecord,RoleMapInfo,RoleAttr,RoleBillInfo) ->
    ...
    if StallSelfState =:= true
       orelse TradingState =:= 1 ->
            ?DEBUG("~ts,RoleState=~w",["玩家当前状态不能进行商贸活动",RoleState]),
    ...
    do_t_trading_get2(RoleId,DataRecord,RoleMapInfo,RoleAttr,RoleBillInfo,RoleTrading2).
%% RoleMapInfo 结构为 p_map_role
%% RoleBillInfo 结构为 r_role_trading_bill
%% RoleTrading 结构为 r_role_trading
do_t_trading_get2(RoleId,_DataRecord,_RoleMapInfo,RoleAttr,RoleBillInfo,RoleTrading) ->
    %% 根据配置处理商贸商票信息，创建商票物品
    MinActivePoints = get_role_min_active_points(),
    AwardType = 
        if RoleAttr#p_role_attr.active_points >= MinActivePoints ->
                ?TRADING_AWARD_TYPE_NOT_BIND;
           true ->
                ?TRADING_AWARD_TYPE_BIND
        end,
    ...
    if Type =:= 1 ->
       ...
       andalso TypeId =/= 0 ->
            case mod_bag:check_inbag_by_typeid(RoleId,TypeId) of
                false ->
                    ?DEBUG("~ts,Type=~w,TypeId=~w",["玩家背包没有此物品",Type,TypeId]),
                    erlang:throw({error,?_LANG_TRADING_RETURN_NOT_GOODS});
                _ ->
                    next
            end;
       true ->
            ?DEBUG("~ts,Type=~w,TypeId=~w",["参数出错",Type,TypeId]),
            erlang:throw({error,?_LANG_TRADING_RETURN_PARAM_ERROR})
    end,
    FactionId = RoleMapInfo#p_map_role.faction_id,
    case check_trading_bill_npc(FactionId,MapId,NpcId) of
...
do_trading_return3({Unique, Module, Method, DataRecord, RoleId, PId, Line},
    ...
    case db:transaction(
        ...
        {atomic,{ok,RoleTrading2,BillUpdateList,BillDeleteList,InUpdateList,InDeleteList}} ->
            catch hook_trading:hook({trading_return, RoleMapInfo#p_map_role.role_id}),
                    ...
                    Reason2 = 
                        case Reason of
                            {throw,{bag_error,BR}} ->
                                case BR of
                                    num_not_enough ->
                                        ?_LANG_TRADING_RETURN_BAG_ERROR;
                                    _ ->
                                        ?_LANG_TRADING_RETURN_ERROR
                                end;
                            _ ->
                                ?_LANG_TRADING_RETURN_ERROR
                        end,
                    ?ERROR_MSG("~ts,Reason=~w",["交还商票出错商票出错",Reason]),
...
do_trading_return4({Unique, Module, Method, DataRecord, RoleId, PId, Line},
    ...
    if RoleTrading#r_role_trading.last_bill > 0 ->
            ...
            catch common_achievement:hook(#r_achievement_hook{role_id = RoleId,event_ids = [306012]});
       true ->
            next
    end,
    FactionId = RoleMapInfo#p_map_role.faction_id,
...
do_trading_return5({_Unique, _Module, _Method, _DataRecord, RoleId, _PId, Line},
    ...
    TradingLog = #r_role_trading_log{
      ...
      role_level = RoleMapInfo#p_map_role.level,
      faction_id = RoleMapInfo#p_map_role.faction_id,
      family_id = RoleMapInfo#p_map_role.family_id,
      family_name = RoleMapInfo#p_map_role.family_name,
      bill = RoleTrading#r_role_trading.bill,
      max_bill = RoleTrading#r_role_trading.max_bill,
      trading_times = RoleTrading#r_role_trading.trading_times,
    ...
    if FamilyId =/= 0  ->
            if FamilyBill > 0 ->
                    catch global:send(FamilyProcessName,{add_money_when_role_trading, FamilyBill});
               true ->
                    next
            end;
       true ->
    ...
    %% 通知消息
    MessageFormat = 
        if RoleTrading#r_role_trading.award_type =:= ?TRADING_AWARD_TYPE_NOT_BIND ->
                ?_LANG_TRADING_RETURN_SUCC_NOT_BIND;
           true ->
                ?_LANG_TRADING_RETURN_SUCC_BIND
        end,
    LeftMessage = lists:flatten(io_lib:format(MessageFormat,
                                              [common_tool:to_list(RoleName),
                                               common_tool:to_list(IncomeBill),
...
do_t_trading_return(RoleId,DataRecord,RoleMapInfo,RoleTrading,IncomeItem) ->
    ...
    if IncomeBill > 0 ->
            if RoleTrading#r_role_trading.award_type =:= ?TRADING_AWARD_TYPE_NOT_BIND ->
                    %% 玩家获取银子，需要记录日志
                    common_consume_logger:gain_silver({RoleId, 0, IncomeBill, 
                                                       ?GAIN_TYPE_SILVER_TRADING,""}),
                    NewSilver = RoleAttr#p_role_attr.silver + IncomeBill,
                    RoleAttr2 = RoleAttr#p_role_attr{silver = NewSilver};
               true ->
                    common_consume_logger:gain_silver({RoleId, IncomeBill, 0, 
                                                       ?GAIN_TYPE_SILVER_TRADING, ""}),
                    NewSilver = RoleAttr#p_role_attr.silver_bind + IncomeBill,
                    RoleAttr2 = RoleAttr#p_role_attr{silver_bind = NewSilver}
            end,
...
do_t_trading_return(RoleId,DataRecord,_RoleMapInfo,RoleTrading,IncomeItem,IncomeBill,FamilyBill) ->
    Type = DataRecord#m_trading_return_tos.type,
    ...
    if Type =:= 2 ->
            ...
            ?DEBUG("RoleId=~w,BagIds=~w,InItemId=~w,InNumber=~w,InUpdateListT=~w,InDeleteListT=~w",[RoleId,BagIds,InItemId,InNumber,InUpdateListT,InDeleteListT]),
    ...
    case db:transaction(
        ...
        {aborted, Reason} ->
            Reason2 = 
                case Reason of
                    {throw,{bag_error,BR}} ->
                        ?DEBUG("~ts,bag_error=~w",["商贸商票兑换商贸宝典背包操作错误，兑换失败",BR]),
                        case BR of
                            not_enough_pos ->
...
do_trading_exchange4({Unique, Module, Method, _DataRecord, RoleId, PId, Line},
    ...
    ?DEBUG("~ts,SendSelf=~w",["兑换商贸宝典返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
...
do_trading_status3({Unique, Module, Method, DataRecord, RoleId, PId, Line},
            ...
            true ->
                case mod_trading_common:get_role_trading(RoleId) of 
                    ...
                    RoleTrading ->
                        ...
                        AwardType =  RoleTrading#r_role_trading.award_type,
                        TradingTimes2 = 
    ...
    ?DEBUG("~ts,SendSelf=~w",["获取玩家的商贸状态返回结果",SendSelf]),
    common_misc:unicast2(PId, Unique, Module, Method, SendSelf),
    case IsTradingMapId of 
        false ->
...
%% global:send(MapProcessName,{mod_trading,{update_role_trading,RoleId,RoleTrading}})
do_update_role_trading(RoleId,RoleTrading) ->
    ?DEBUG("~ts,RoleId=~w,RoleTrading=~w",["调试信息，同步信息处理",RoleId,RoleTrading]),
    mod_trading_common:put_role_trading(RoleId,RoleTrading),
    ok.
...
do_delete_role_trading(RoleId, RoleMapInfo, TradingStatus) ->
    ...
    case mod_trading_common:get_role_trading(RoleId) of
        undefined ->
            ?ERROR_MSG("~ts", ["玩家销毁商贸商票时，查找不到玩家商贸信息"]),
            ignore;
        RoleTrading ->
            RoleTradingBill = RoleTrading#r_role_trading.role_trading_bill,
            %% 商贸日志
...
notify_client_cancel_role_trading_status(RoleId) ->
    ...
    ?DEBUG("~ts,SendSelf=~w",["当玩家销毁商贸商票时，通知前端商贸状态变化",SendSelf]),
...
hook_first_enter_map(RoleId,RoleBase) ->
    ...
    if Trading =:= 1 ->
            %% 加载商贸相关数据到相应的地图进程中
            ?DEBUG("~ts",["玩家处于商贸状态，上线需要同步商贸信息到地图进程字典"]),
...
hook_first_enter_map2(RoleId,RoleBase) ->
    ...
    ?DEBUG("~ts,RoleTrading=~w",["玩家上次商贸没有完成，上线时需要初始化商贸信息",RoleTrading]),
...
hook_role_dead(RoleId,RoleMapInfo,SrcActorId,SrcActorType) ->
    case db:transaction(
           fun() -> 
        ...
        {aborted, Reason} ->
            case Reason of 
                not_trading_status ->
                    ?DEBUG("~ts",["玩家死亡时，不处于商贸活动状态，不需要处理"]);
                _ ->
...
hook_role_dead2(RoleId,RoleMapInfo,UpdateGoodsList,DeleteGoodsList) ->
    ?DEBUG("~ts,RoleId=~w",["玩家死亡时，处于商贸活动状态，需要删除商贸商票",RoleId]),
    Line = common_misc:get_role_line_by_id(RoleId),
    UnicastArg = {line, Line, RoleId},
    if UpdateGoodsList =/= [] ->
...
check_valid_distance(RoleId,NpcId) ->
    ...
    {MaxTx,MaxTy} = get_trading_role_npc_valid_distance(),
...
send_trading_broadcast_message(MapId,NowSeconds) ->
    ...
    case get_trading_broadcast_message_time_range(NowDate) of
        {ok,TimeRangeList,Interval} ->
            ?DEV("~ts,NowDate=~w,TimeRangeList=~w,Interval=~w",["当天需要发送的商贸广播消息时段和间隔数据",NowDate,TimeRangeList,Interval]),
            lists:foreach(
              fun({{SH,SM},{EH,EM}}) ->
        ...
        {error,Reason} ->
            ?DEBUG("~ts,MapId=~w,NowDate=~w,Reason=~w",["当前时间不需要执行商贸广播",MapId,NowDate,Reason]),
            next
    end.
            ...
            role ->
                {FactionId,RoleName} = 
                    case mod_map_actor:get_actor_mapinfo(SrcActorId,role) of
                        ...
                        #p_map_role{faction_id = FactionIdT,role_name = RoleNameT} ->
                            {FactionIdT,common_tool:to_list(RoleNameT)}
                    end,
                FactionName = 
                    if FactionId =:= 1 ->
