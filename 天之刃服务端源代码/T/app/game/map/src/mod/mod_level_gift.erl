...
send_role_level_gift(RoleID) ->
    %%判断时间礼包是否领取完
    case mod_time_gift:if_get_all_time_gift(RoleID) of
        true->
                ...
                false->
                    case common_transaction:transaction(
                        ...
                        {atomic,{ok,NewGoodsList}} ->
                            ...
                            db:dirty_write(?DB_ROLE_LEVEL_GIFT,
                                           #r_role_level_gift{role_id=RoleID,gifts=NewGifts}),
...
get_next_level_gift(RoleID)->
    case db:dirty_read(?DB_ROLE_LEVEL_GIFT,RoleID) of
    ...
    case State of 
        ...
        ?IS_OPENED->
            case common_config_dyn:find(level_gift,LastGiftLevel) of
                [{_TypeID1,NextGiftLevel1,_IfOpen1}] ->
                    case common_config_dyn:find(level_gift,NextGiftLevel1) of
                        [{TypeID2,_NextGiftLevel2,IfOpen2}]->
                            case  common_config_dyn:find(gift,TypeID2) of
                                [#r_gift{gift_list=GiftBaseList}] ->
                                    ...
                                    %% true:还有礼包为领取
                                    %% false：当前礼包是否是最后一个
                                    {ok,[#p_level_gift_info{id=NextGiftLevel1,goods_list=GoodsList,next_level=IfOpen2}],<<>>};
                                _ ->
