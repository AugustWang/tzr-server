...
%%%
%%% @end
%%% Created :  7 Dec 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
-module(mod_ybc_family).

%% API
-export([
         handle/2,return_family_ybc_silver/4
        ]).
...
do_ybc_agree_collect_check(RoleID) ->
    ...
    case  Status =:= ?ROLE_STATE_TRAINING orelse Status =:= ?ROLE_STATE_DEAD
        orelse StallSelf =:= true orelse (Ybc =/= undefined andalso Ybc =/= 0)  orelse Trading =:= 1
    ...
    of
        true ->
            if Trading =:= 1 ->
               ...
               true ->
                    case  Status =:= ?ROLE_STATE_TRAINING orelse Status =:= ?ROLE_STATE_DEAD of
                        true ->
...
do_family_ybc_agree_collect(Unique, Module, Method, RoleID, PID, MapID, TX, TY) ->
    case catch do_ybc_agree_collect_check(RoleID) of
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["处理同意门派拉镖拉人", Error]),
...
do_get_list(YbcID, Unique, Module, Method, PID) ->
    case mod_map_ybc:get_ybc_full_info(YbcID) of
        ...
        #r_ybc{role_list=RoleList} ->
            ...
            Record = #m_family_ybc_list_toc{members=Members},
                ...
                {aborted, Error} ->
                    case erlang:is_binary(Error) of
                        true ->
                            Reason = Error;
                        false ->
                            ?ERROR_MSG("~ts:~w", ["同意加入门派拉镖发生系统错误", Error]),
...
do_family_ybc_agree_publish_check(RoleID) ->
    case mod_map_role:get_role_base(RoleID) of
        {ok, #p_role_base{faction_id=FactionID, status=Status}} ->
            ...
            MapID = mgeem_map:get_mapid(),
            %% 检查玩家是否在史可法附近
            {YMapID, {YTX, YTY}} = common_ybc:get_family_ybc_publish_pos(FactionID),
        ...
        {aborted, Error} ->
            case erlang:is_binary(Error) of
                true ->
                    Reason = Error;
                false ->
                    ?ERROR_MSG("~ts:~w", ["发布镖车时扣除玩家银子出错", Error]),
...
do_family_ybc_commit(RoleID, YbcID, FamilyPID) ->
    case catch do_family_ybc_commit_check(RoleID, YbcID) of
        ok ->            
            %% 遍历获取哪些门派成员在NPC附近，直接加钱即可，状态更新暂时有门派去处理（事务过程尽量不要放在地图中进行）
            case mod_map_ybc:get_ybc_full_info(YbcID) of
                undefined ->
                ...
                #r_ybc{role_list=RoleList, create_time=CreatorTime} ->
                    ...
                    %% 成就 add by caochuncheng 2011-03-16
                    common_hook_achievement:hook({family_module,{family_ybc,[RoleID]}}),
                    FamilyPID ! {ybc_commit_succ, RoleList, RoleList, TimeOut}
            end;
        {error, Error} ->            
            FamilyPID ! {ybc_commit_error, Error};
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["检查门派镖车能否交镖时发生系统错误", Error]),
            FamilyPID ! {ybc_commit_error, ?_LANG_FAMILY_YBC_SYSTEM_ERROR_WHEN_COMMIT}
    end.   

do_family_ybc_commit2(RoleList, TimeOut, FamilyPID) ->    
    lists:foreach(
      fun({RID, RName, _RLevel, _SB, Silver}) ->
              case TimeOut of
                  true ->
                      ignore;
                  false ->
                      ...
                      case mod_map_role:get_role_attr(RID) of
                          {error, role_not_found} -> %%帮众没在本地图
                              common_misc:send_to_rolemap(RID, {mod_map_role, {return_family_ybc_silver, RID, RName, FamilyPID, Silver}});
                          {ok, _RoleAttr} ->
                              return_family_ybc_silver(RID,RName,FamilyPID,Silver)
                      end
              end
      end, RoleList).
...
return_family_ybc_silver(RoleID,RoleName,FamilyPID,Silver) ->
    case common_transaction:transaction(fun() -> t_return_family_ybc_silver(RoleID,Silver) end) of
        {atomic, _} ->
            {ok, RoleAttr} = mod_map_role:get_role_attr(RoleID),
            ChangeList = [
...
t_return_family_ybc_silver(RoleID,Silver) ->
    {ok, #p_role_attr{silver=OldSilver} = RoleAttr} = mod_map_role:get_role_attr(RoleID),
    mod_map_role:set_role_attr(RoleID, RoleAttr#p_role_attr{silver=OldSilver + get_ybc_silver_multiple() * Silver}),
    common_consume_logger:gain_silver({RoleID, 0, Silver, ?GAIN_TYPE_SILVER_FAMILY_YBC, ""}).
