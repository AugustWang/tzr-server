...
do_equip_quality3({Unique,Module,Method,DataRecord, RoleId, Pid, Line},EquipGoods,AddItemRecord) ->
    ...
    RefiningFee =#r_refining_fee{type = equip_quality_fee,
                                 equip_level = EquipGoods#p_goods.level,
                                 material_level = AddItemLevel,
                                 material_number = AddItemNum,
...
do_equip_signature2({_Unique,_Module,_Method,DataRecord, RoleId, _Pid, _Line}) ->
    ...
    {ok,EquipGoods}.
do_equip_signature3({Unique,Module,Method,DataRecord, RoleId, Pid, Line},EquipGoods) ->
    RefiningFee =#r_refining_fee{type = equip_signature_fee,
                                 equip_level = EquipGoods#p_goods.level,
                                 refining_index = EquipGoods#p_goods.refining_index,
                                 punch_num = EquipGoods#p_goods.punch_num,
                                 stone_num = EquipGoods#p_goods.stone_num,
...
do_t_equip_change_signature2(RoleId,DataRecord,Fee) ->
    ...
    %% 签名时需要处理装备套装属性
    NewEquip2 = mod_equip_fiveele:do_random_equip_whole_attr(NewEquip),
    mod_bag:update_goods(RoleId,NewEquip2),
    {ok,NewEquip2}.
...
do_equip_upgrade_link2({_Unique,_Module,_Method,DataRecord, RoleId, _Pid, _Line}) ->
    ...
    IsFiveEle = DataRecord#m_equip_build_upgrade_link_tos.is_five_ele,
    if IsFiveEle ->
            ...
            if erlang:is_record(FiveEleAttr,p_equip_five_ele) 
               andalso FiveEleAttr#p_equip_five_ele.id =/= 0 ->
                    next;
               FiveEleAttr#p_equip_five_ele.level =:= 1 ->
                    ?DEBUG("~ts",["装备升级，选择保留五行属性，但是五行属性级别为1不需要保留"]),
                    erlang:throw({error,?_LANG_EQUIP_CHANGE_U_PARAM_ERROR});
               true ->
                    ?DEBUG("~ts",["装备升级，选择保留五行属性，但装备是没有五行属性，参数错误"]),
                    erlang:throw({error,?_LANG_EQUIP_CHANGE_U_PARAM_ERROR})
            end;
       true ->
            next
...
do_equip_upgrade_link4({Unique,Module,Method,DataRecord, RoleId, Pid, Line},EquipGoods,NewEquipInfo,NewEquipGoods) ->
    ...
    FiveEleAttr = get_equip_five_ele_attr_by_upgrade(IsFiveEle,EquipGoods),
    ...
    NewGoods3 = 
        if erlang:is_record(OldWholeAttr,p_equip_whole_attr)
           andalso OldWholeAttr#p_equip_whole_attr.id =/= 0 ->
                NewGoods2 = mod_equip_fiveele:do_random_equip_whole_attr(NewGoods),
                WholeAttr = NewGoods2#p_goods.whole_attr,
                OldIndex = OldWholeAttr#p_equip_whole_attr.index,
                WholeAttr2 = WholeAttr#p_equip_whole_attr{index = OldIndex},
                NewGoods2#p_goods{whole_attr = WholeAttr2};
           true ->
                NewGoods
        end,
    NewGoods4 = 
...
do_equip_upgrade({Unique,Module,Method,DataRecord, RoleId, Pid, Line}) ->
    case catch do_equip_upgrade2({Unique,Module,Method,DataRecord, RoleId, Pid, Line}) of
        ...
        {ok,EquipGoods,EquipInfo,NextEquipInfo,BaseRecord,QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord} ->
            do_equip_upgrade3({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                             EquipGoods,EquipInfo,NextEquipInfo,BaseRecord,
                              QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord)
    end.
...
do_equip_upgrade2_5({Unique,Module,Method,DataRecord, RoleId, Pid,Line},
                        EquipGoods,EquipInfo,NextEquipInfo,BaseRecord,
                        QualityRecord,ReinforceRecord) ->
    FiveEleTypeId = DataRecord#m_equip_build_upgrade_tos.five_ele_type_id,
    FiveEleAttr = EquipGoods#p_goods.five_ele_attr,
    EquipMaterial = NextEquipInfo#p_equip_base_info.material,
    FiveEleLevel = case FiveEleAttr of
                       undefined ->
                           0;
                       _ ->
                           FiveEleAttr#p_equip_five_ele.level
                   end,
    FiveEleRecord = 
        if FiveEleLevel =:= 0 ->
                undefined;
           true ->
                if erlang:is_record(FiveEleAttr, p_equip_five_ele)
                   andalso FiveEleAttr#p_equip_five_ele.id =/= 0 ->
                        next;
                   FiveEleAttr#p_equip_five_ele.level =:= 1 ->
                        ?DEBUG("~ts,FiveEleAttr=~w",["装备升级，选择保留五行属性，但是五行属性级别为1不需要保留",FiveEleAttr]),
                        erlang:throw({error,?_LANG_EQUIP_CHANGE_U_PARAM_ERROR});
                   true ->
                        ?DEBUG("~ts,FiveEleAttr=~w",["此装备升级时，不需要选择保留五行属性",FiveEleAttr]),
                        erlang:throw({error,?_LANG_EQUIP_CHANGE_U_PARAM_ERROR})
                end,
                [FRecord] = 
                    case common_config_dyn:find(equip_change,equip_upgrade_fiveele_material) of
                        [ FMList ] ->
                            [R || R <- FMList,
                                  R#r_equip_upgrade_fiveele_material.five_ele_level =:= FiveEleLevel
                            ];
                        _ ->
                            ?DEBUG("~ts,FiveEleLevel=~w",["装备升级保留五行材料不合法1",FiveEleLevel]),
                            erlang:throw({error,?_LANG_EQUIP_CHANGE_U_FIVEELE_ERROR})
                    end,
                FiveEleItemLevel = FRecord#r_equip_upgrade_fiveele_material.item_level,
                FiveEleItem = 
                    case mod_equip_build:get_equip_build_goods(EquipMaterial,?EQUIP_BUILD_ADD,FiveEleItemLevel) of
                        [] ->
                            ?DEBUG("~ts,EquipMaterial=~w,FiveEleItemLevel=~w",["装备升级保留五行材料不合法2",EquipMaterial,FiveEleItemLevel]),
                            erlang:throw({error, ?_LANG_EQUIP_CHANGE_U_FIVEELE_ERROR});
                        [FItem] ->
                            FItem
                    end,
                if FiveEleItem#r_equip_build_item.item_id =/= FiveEleTypeId ->
                        ?DEBUG("~ts,FiveEleTypeId=~w,FileEleItem=~w",["装备升级保留五行材料不合法3",FiveEleTypeId,FiveEleItem]),
                        erlang:throw({error, ?_LANG_EQUIP_CHANGE_U_FIVEELE_ERROR});
                   true ->
                        next
                end,
    ...
    do_equip_upgrade2_6({Unique,Module,Method,DataRecord,RoleId,Pid,Line},
                        EquipGoods,EquipInfo,NextEquipInfo,BaseRecord,
                        QualityRecord,ReinforceRecord,FiveEleRecord).
...
do_equip_upgrade3({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
    ...
    do_equip_upgrade4({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                      EquipGoods,EquipInfo,NewEquipInfo,BaseRecord,
                      QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord,NewGoods).
do_equip_upgrade4({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                  EquipGoods,EquipInfo,NewEquipInfo,BaseRecord,
                  QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord,NewEquipGoods) ->
    %% 装备品质
    QualityTypeId = DataRecord#m_equip_build_upgrade_tos.quality_type_id,
    ...
    %% 装备五行
    FiveEleTypeId = DataRecord#m_equip_build_upgrade_tos.five_ele_type_id,
    FiveEleFlag = if FiveEleTypeId =/= 0 -> true; true -> false end,
    FiveEleAttr = get_equip_five_ele_attr_by_upgrade(FiveEleFlag,EquipGoods),
    NewGoods = NewEquipGoods#p_goods{
                 quality = Quality,
                 reinforce_result = ReinforceResult,
                 reinforce_rate = ReinforceRate,
    ...
    NewGoods4 = 
        if erlang:is_record(OldWholeAttr,p_equip_whole_attr)
           andalso OldWholeAttr#p_equip_whole_attr.id =/= 0 ->
                NewGoods3 = mod_equip_fiveele:do_random_equip_whole_attr(NewGoods2),
                WholeAttr = NewGoods3#p_goods.whole_attr,
                OldIndex = OldWholeAttr#p_equip_whole_attr.index,
                WholeAttr2 = WholeAttr#p_equip_whole_attr{index = OldIndex},
                NewGoods3#p_goods{whole_attr = WholeAttr2};
           true ->
                NewGoods2
        end,
    do_equip_upgrade5({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                      EquipGoods,EquipInfo,NewEquipInfo,BaseRecord,
                      QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord,NewGoods4).

do_equip_upgrade5({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                  EquipGoods,EquipInfo,NewEquipInfo,BaseRecord,
                  QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord,NewEquipGoods) ->
    RefiningFee =#r_refining_fee{type = equip_upgrade_fee,
    ...
    case mod_refining:get_refining_fee(RefiningFee) of
        {ok,Fee} ->
             do_equip_upgrade6({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
                               EquipGoods,EquipInfo,NewEquipInfo,BaseRecord,
                               QualityRecord,ReinforceRecord,FiveEleRecord,BindRecord,NewEquipGoods,Fee);
        {error,Error} ->
...
do_equip_upgrade6({Unique,Module,Method,DataRecord, RoleId, Pid, Line},
    ...
    case catch do_transaction_equip_upgrade(RoleId,DataRecord,EquipGoods,
                                            ...
                                            QualityRecord,ReinforceRecord,
                                            FiveEleRecord,BindRecord,NewEquipGoods,Fee) of
        {error,Error} ->
            do_equip_upgrade_error({Unique,Module,Method,DataRecord, RoleId, Pid, Line},Error);
...
do_transaction_equip_upgrade(RoleId,DataRecord,EquipGoods,
                             EquipInfo,NewEquipInfo,BaseRecord,
                             QualityRecord,ReinforceRecord,
                             FiveEleRecord,BindRecord,NewEquipGoods,Fee) ->
    case common_transaction:transaction(fun() -> 
...
do_transaction_equip_upgrade2(RoleId,DataRecord,_EquipGoods,
    ...
    {DelFiveList,UpdateFiveList} =
        if FiveEleRecord =:= undefined ->
                {[],[]};
           true ->
                FiveNumber = FiveEleRecord#r_equip_upgrade_fiveele_material.number,
                FiveTypeId = DataRecord#m_equip_build_upgrade_tos.five_ele_type_id,
                case do_transaction_consume_goods(RoleId,BagIdList,FiveTypeId,FiveNumber) of
                    {error,FError} ->
                        ?DEBUG("~ts,Error=~w",["扣除保留五行材料出错",FError]),
                        erlang:throw({error,FError});
                    {ok,DFList,UFList} ->
                        {DFList,UFList}
                end
        end,
...
do_transaction_equip_upgrade3(RoleId,NewEquipInfo,NewEquipGoods,DelBaseList,UpdateBaseList,
    ...
    MatrailBind7 = do_check_matrail_bind(DelFiveList),
    MatrailBind8 = do_check_matrail_bind(UpdateFiveList),
    ...
    do_transaction_equip_upgrade4(RoleId,NewEquipInfo,NewEquipGoods,DelBaseList,UpdateBaseList,
                                  DelQualityList,UpdateQualityList,DelReinList,UpdateReinList,
                                  DelFiveList,UpdateFiveList,DelBindList,UpdateBindList,MatrailBind).
...
do_equip_decompose2({_Unique,_Module,_Method,DataRecord, RoleId, _Pid, _Line}) ->
    ...
    {ok,EquipGoods,EquipInfo}.
do_equip_decompose3({Unique,Module,Method,DataRecord, RoleId, Pid, Line},EquipGoods,EquipInfo) ->
    RefiningFee =#r_refining_fee{type = equip_decompose_fee,
                                 equip_level = EquipGoods#p_goods.level,
                                 refining_index = EquipGoods#p_goods.refining_index,
                                 punch_num = EquipGoods#p_goods.punch_num,
                                 stone_num = EquipGoods#p_goods.stone_num,
...
do_equip_chang_deduct_fee_notify(RoleId, Line) ->
    ...
    case mod_map_role:get_role_attr(RoleId) of
        ...
        {error ,R} ->
            ?ERROR_MSG("~ts,Reason=~w",["获取角色属性出错，装备改造失败之后无法通知前端银子变化情况",R])
    end.
...
%% Flag 是否选择保留五行属性
%% EquipGoods p_goods
%% 返回 p_equip_five_ele or undefined
get_equip_five_ele_attr_by_upgrade(Flag,EquipGoods) ->
    case Flag of 
        true ->
            EquipGoods#p_goods.five_ele_attr;
        false ->
            case EquipGoods#p_goods.five_ele_attr of
                undefined ->
                    undefined;
                EquipFiveEleAttr ->
                    EquipFiveEleAttr#p_equip_five_ele{level = 1}
            end
    end.
%% 装备升级时根据条件重新获取装备绑定属性
%% Flag 是否选择保留绑定属性
