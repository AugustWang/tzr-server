...
-define(KILLNOTICE, "我国勇士[~s]在 ~s 成功击杀入侵者").
...
role_killed(RoleID, RoleName, FactionID,  SActorID, SRoleName, SFactionID,
            MapID, InReadoArea) ->
    %% 国战期间不发送死亡信件
    IsInWaroffaction = mod_map_role:is_in_waroffaction(FactionID),
    CountryTreasureMapID = mod_country_treasure:get_country_treasure_fb_map_id(),
    case  IsInWaroffaction orelse CountryTreasureMapID =:= MapID of
        false ->
        ...
        ?OFFICE_ID_KING ->
            {?WORLD_CENTRAL_BROADCAST, lists:flatten(io_lib:format("<font color=\"#FFFFFF\">天啊！~s<font color=\"#FFFF00\">~s</font>居然被 ~s 的<font color=\"#FFFF00\">~s</font>打败了</font>", 
                                                                   [get_faction_king_name_by_id(FactionID), RoleName, 
                                                                    get_faction_name_by_id(SrcFactionID), SrcRoleName]))};
        ?OFFICE_ID_JINYIWEI ->
            {?WORLD_CENTRAL_BROADCAST, lists:flatten(io_lib:format("<font color=\"#FFFFFF\">天啊！~s锦衣卫<font color=\"#FFFF00\">~s</font>居然被 ~s 的<font color=\"#FFFF00\">~s</font>打败了</font>",
                                                                     [get_faction_name_by_id(FactionID),
                                                                      RoleName, get_faction_name_by_id(SrcFactionID), SrcRoleName]))};
        ?OFFICE_ID_MINISTER ->
            {?WORLD_CENTRAL_BROADCAST, lists:flatten(io_lib:format("<font color=\"#FFFFFF\">天啊！~s内阁大臣<font color=\"#FFFF00\">~s</font>居然被 ~s 的<font color=\"#FFFF00\">~s</font>打败了</font>",
                                                                     [get_faction_name_by_id(FactionID),
                                                                      RoleName, get_faction_name_by_id(SrcFactionID), SrcRoleName]))};
        _ ->
            {?WORLD_CENTRAL_BROADCAST, lists:flatten(io_lib:format("<font color=\"#FFFFFF\">天啊！~s大将军<font color=\"#FFFF00\">~s</font>居然被 ~s 的<font color=\"#FFFF00\">~s</font>打败了</font>",
                                                                     [get_faction_name_by_id(FactionID),
                                                                      RoleName, get_faction_name_by_id(SrcFactionID), SrcRoleName]))}
    end.
...
get_faction_name_by_id(FactionID) ->
    if
        FactionID =:= 1 ->
            "<font color=\"#00FF00\">云州</font>";
        FactionID =:= 2 ->
            "<font color=\"#F600FF\">沧州</font>";
        true ->
            "<font color=\"#00CCFF\">幽州</font>"
    end.

get_faction_king_name_by_id(FactionID) ->
    if
        FactionID =:= 1 ->
            "<font color=\"#00FF00\">云州王</font>";
        FactionID =:= 2 ->
            "<font color=\"#F600FF\">沧州王</font>";
        true ->
            "<font color=\"#00CCFF\">幽州王</font>"
    end.    
