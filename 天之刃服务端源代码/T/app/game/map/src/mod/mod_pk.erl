...
reduce_pk_point(RoleID, Reduce, ReduceType) ->
    ...
    of
        {atomic, {PKPoint, RoleBase}} ->
            mod_map_role:do_update_map_role_info(RoleID, [{#p_map_role.pk_point, PKPoint}], mgeem_map:get_state()),
...
do_pkpoint_change(RoleID, RoleBase, PKPoint, PKPoint2) ->
    ...
    mod_map_role:do_update_map_role_info(RoleID, [{#p_map_role.pk_point, PKPoint2}], MapState),

    %% 发送到杀人者地图，杀人者跟被杀的人不一定在同一个地图，例如BUFF
    case PKPoint =:= 0 andalso PKPoint2 =/= 0 of
        true ->
            common_misc:send_to_rolemap(RoleID, {mod_map_role, {reduce_pk_point_start, RoleID}});
        _ ->
            ignore
    end,
    ...
    case PKPoint < 18 andalso PKPoint2 >= 18 of
        true ->      
            ...
            common_letter:sys2p(RoleID, Letter, "来自监狱-监狱长的信件", 14);
        _ ->
            ignore
    end.
