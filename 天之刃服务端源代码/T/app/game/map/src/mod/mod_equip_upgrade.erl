...
do_equip_upgrade2(RoleId,DataRecord) ->
    ...
    RefiningFeeRule =#r_refining_fee{
      type = equip_upgrade_fee,
      equip_level = NextEquipGoods#p_goods.level,
      refining_index = NextEquipGoods#p_goods.refining_index,
      punch_num = NextEquipGoods#p_goods.punch_num,
      stone_num = NextEquipGoods#p_goods.stone_num,
      equip_color = NextEquipGoods#p_goods.current_colour,
      equip_quality = NextEquipGoods#p_goods.quality},
    UpgradeFee = 
        case mod_refining:get_refining_fee(RefiningFeeRule) of
            {ok,UpgradeFeeT} ->
                UpgradeFeeT;
            {error,UpgradeFeeError} ->
                erlang:throw({error,UpgradeFeeError,0})
        end,
    {ok,EquipGoods,UpgradeGoods,NextEquipGoods,UpgradeFee}.
...
get_next_equip_goods2(EquipGoods,NextTypeId,MatrailBind) ->
    ...
    NewGoods = #p_goods{
      ...
      five_ele_attr = EquipGoods#p_goods.five_ele_attr,
