...
change(RoleID, FactionID, _FamilyID, SRoleID, SFactionID, SFamilyID, Flag) ->
    ...
    case FactionID =:= SFactionID orelse Flag =:= true orelse MapId =:= 10500 of
        false ->
...
add_gongxun(RoleID, Add) ->
    ...
    of
        {atomic, {Value, NewValue, RoleAttr}} ->
            ...
            global:send(mgeew_ranking,{ranking_element_update,ranking_role_gongxun,RoleGongXunRank}),
            global:send(mgeew_ranking,{ranking_element_update,ranking_role_today_gongxun,{add,RoleGongXunRank#p_role_gongxun_rank{gongxun=Add}}}),       
            ...
            common_mod_goal:hook_gongxun(RoleID, NewValue, Add),
            ok;
        {aborted, Error} ->
...
get_mul(SRoleID, FactionID, SFactionID,Level,SLevel) ->
    ...
    if
        KillerShouBianState =:= 1 andalso FactionID =/= SFactionID ->
            ...
            %%common_hook_achievement:hook({mod_mission,{shoubian_zhanggong,SRoleID}}),
            %%TODO 任务重构备忘修改
            %%有战功才有广播
            case Level < 40 orelse (SLevel > Level andalso SLevel-Level > 5) of
                false ->
                    common_broadcast:bc_send_msg_world(
                        ?BC_MSG_TYPE_CENTER, 
                        ?BC_MSG_SUB_TYPE, 
                         io_lib:format(?_LANG_MISSION_SHOU_BIAN_MUL_GONGXUN, [KillerRoleNameStr]));
                true ->
                    ignore
            end,
            2;
        ...
        {atomic, {Value, NewValue, RoleAttr}} ->
            case Value =:= NewValue of
                ...
                _ ->
                    ...
                    common_misc:unicast({role, RoleID}, ?DEFAULT_UNIQUE, ?ROLE2, ?ROLE2_ATTR_CHANGE, RR),
                    {ok,  #p_role_base{faction_id=FactionID}=RoleBase} = mod_map_role:get_role_base(RoleID),
                    RoleGongXunRank =common_ranking:get_gongxun_rank_record(RoleBase, RoleAttr#p_role_attr{gongxun=NewValue}),
                    global:send(mgeew_ranking,{ranking_element_update,ranking_role_gongxun,RoleGongXunRank}),
                    case Type of
                        dead ->
                            global:send(mgeew_ranking,{ranking_element_update,ranking_role_today_gongxun,{reduce,RoleID,FactionID,Reduce}});
                        _ ->
                            ignore
                    end,
