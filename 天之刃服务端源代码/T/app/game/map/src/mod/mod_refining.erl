...
%% 天工炉天工开物功能处理
handle({Unique,?REFINING,?REFINING_BOX,DataIn,RoleID,PID,Line,_State})->
    mod_refining_box:do_handle_info({Unique,?REFINING,?REFINING_BOX,DataIn,RoleID,PID,Line});
...
do_reinforce_equip3({E,S}=CheckGoods) -> 
    ...
    of
        ...
        {atomic, {Succ,Equip,Stuff,Prompt}} ->
            ...
            catch common_mod_achievement:hook(#r_achievement_hook{role_id = RoleId,event_ids = AchEventIdList}),
            %% 道具消费日志
...
%% 处理当材料是绑定时的，装备不绑定时，装备绑定处理
t_reinforce_equip4(Result,Equip,Stuff,MaterialBind) ->
    if MaterialBind ->
            case mod_refining_bind:do_equip_bind_for_reinforce(Equip) of
                {error,ErrorCode} ->
                    ?INFO_MSG("~ts,ErrorCode=~w",["装备强化时，当材料是绑定的，装备是不绑定时，处理绑定出错，只是做绑定处理，没有附加属性",ErrorCode]),
...
change_main_property(?REFINING_BLOOD,Pro,BasePro,TotalRate, Rate) ->
    Pro#p_property_add{
        blood = round(BasePro#p_property_add.blood *Rate/100)
      + Pro#p_property_add.blood - round(BasePro#p_property_add.blood * TotalRate/100)
     };
change_main_property(?REFINING_PHYSIC_ATT,Pro,BasePro,TotalRate, Rate) ->
    Max = round(BasePro#p_property_add.max_physic_att *Rate/100)
    + Pro#p_property_add.max_physic_att - round(BasePro#p_property_add.max_physic_att * TotalRate/100),
...
change_main_property(?REFINING_MAGIC_ATT,Pro,BasePro,TotalRate, Rate) ->
    Max = round(BasePro#p_property_add.max_magic_att * Rate/100)
    + Pro#p_property_add.max_magic_att - round(BasePro#p_property_add.max_magic_att * TotalRate/100),
...
change_main_property(?REFINING_PHYSIC_DEF,Pro,BasePro,TotalRate, Rate) ->
    Pro#p_property_add{
        physic_def = round(BasePro#p_property_add.physic_def *Rate/100)
      + Pro#p_property_add.physic_def - round(BasePro#p_property_add.physic_def * TotalRate/100)
     };
change_main_property(?REFINING_MAGIC_DEF,Pro,BasePro,TotalRate, Rate) ->
    Pro#p_property_add{
        magic_def = round(BasePro#p_property_add.magic_def *Rate/100)
      + Pro#p_property_add.magic_def - round(BasePro#p_property_add.magic_def * TotalRate/100)
     }.
...
do_compose3(Unique, Module, Method, RoleID, ComposeType, GoodsList, NextLevelID, TotalNum, BindNum, Div, Line) ->
    ...
    of 
        ...
        {atomic, {NormalGoodsList, BindGoodsList}} ->
            %% 新手任务hook add by luojincheng
            catch hook_refining_compose:hook({RoleID}),
            ...
            DataRecord = 
                if NormalGoodsList =:= undefined andalso BindGoodsList =:= undefined ->
                   ...
                   true ->
                        %% 成就 add by caochuncheng 2011-03-07
                        catch hook_refining_compose:hook({RoleID,NextLevelID}),
        ...
        case Bind of
            true ->
                case mod_refining_bind:do_equip_bind_for_punch(Equip) of
                    {error,ErrorCode} ->
                        ?INFO_MSG("~ts,ErrorCode=~w",["装备打孔时，材料是绑定的，装备不绑定，绑定出错，一般绑定处理",ErrorCode]),
...
do_inlay3(CheckGoods) -> 
    ...
    case db:transaction(F) of
        ...
        {atomic, {Equip,Stone,Symbol}} ->
            ...
            do_refining_deduct_fee_notify(RoleId,{role,RoleId}),
            %% 成就系统添加hook
            catch common_mod_achievement:hook(#r_achievement_hook{role_id = RoleId,event_ids = [100007]}),
    ...
    NewEquip = 
        if MatrailBind1 orelse MatrailBind2 ->
                case mod_refining_bind:do_equip_bind_for_inlay(Equip) of
                    {error,ErrorCode} ->
                        ?INFO_MSG("~ts,ErrorCode=~w",["装备镶嵌时，材料是绑定的，装备不绑定，绑定出错，一般绑定处理",ErrorCode]),
                        Equip#p_goods{bind = true};
                    {ok,BindGoods} ->
                        BindGoods
                end;
...
add_property(?REFINING_BLOOD,Pro,BasePro,{_Main,Dodge,DeadAttack,NoDefence},Quality,OpType) ->
    case OpType of 
        add ->
            Pro#p_property_add{
              blood = round(BasePro#p_property_add.blood * (Quality)/100)+
        ...
        del ->
            Pro#p_property_add{
              blood = Pro#p_property_add.blood - 
                  round(BasePro#p_property_add.blood * (Quality)/100),
...
add_property(?REFINING_PHYSIC_ATT,Pro,BasePro,{_Main,Dodge,DeadAttack,NoDefence},Quality,OpType) ->
    Value = round(BasePro#p_property_add.max_physic_att * (Quality)/100),
    case OpType of 
        add ->
            Pro#p_property_add{
...
add_property(?REFINING_MAGIC_ATT,Pro,BasePro,{_Main,Dodge,DeadAttack,NoDefence},Quality,OpType) ->
    Value = round(BasePro#p_property_add.max_magic_att * (Quality)/100),
    case OpType of 
        add ->
            Pro#p_property_add{
...
add_property(?REFINING_PHYSIC_DEF,Pro,BasePro,{_Main,Dodge,DeadAttack,NoDefence},Quality,OpType) ->
    case OpType of 
        add ->
            Pro#p_property_add{
              physic_def = round(BasePro#p_property_add.physic_def * (Quality)/100)+
        ...
        del ->
             Pro#p_property_add{
               physic_def = Pro#p_property_add.physic_def - 
                   round(BasePro#p_property_add.physic_def * (Quality)/100),
               dodge = Pro#p_property_add.dodge - Dodge,
               dead_attack = Pro#p_property_add.dead_attack - DeadAttack,
               no_defence = Pro#p_property_add.no_defence - NoDefence
             }
...
add_property(?REFINING_MAGIC_DEF,Pro,BasePro,{_Main,Dodge,DeadAttack,NoDefence},Quality,OpType) ->
    case OpType of 
        add ->
            Pro#p_property_add{
              magic_def = round(BasePro#p_property_add.magic_def * (Quality)/100)+
        ...
        del ->
            Pro#p_property_add{
              magic_def = Pro#p_property_add.magic_def - 
                  round(BasePro#p_property_add.magic_def * (Quality)/100),
              dodge = Pro#p_property_add.dodge - Dodge,
              dead_attack = Pro#p_property_add.dead_attack - DeadAttack,
              no_defence = Pro#p_property_add.no_defence - NoDefence
             }
...
%% 天工炉操作费用处理
%% 参数，RoleId用户Id，Fee 银子费用，单位文，EquipConsume 消费日志记录 结构为r_equip_consume
do_refining_deduct_fee(RoleId,Fee,EquipConsume) ->
    ...
    case mod_map_role:get_role_attr(RoleId) of
        ...
        {error,Error} ->
            ?ERROR_MSG("~ts,Error=~w",["查询角色有的银子时出错",Error]),
...
do_refining_deduct_fee_notify(RoleId,UnicastArg) ->
    case mod_map_role:get_role_attr(RoleId) of
        ...
        {error ,R} ->
            ?ERROR_MSG("~ts,Reason=~w",["获取角色属性出错，天工炉操作成功之后无法通知前端银子变化情况",R])
    end.
%% 获取强化，开孔，合成，镶嵌，拆卸的费用
