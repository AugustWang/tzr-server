    ...
    if
        AddNum < 0 ->
            NewCounterData = CounterData#mission_counter{
                commit_times= if (CurrentCommitTimes + AddNum) < 0 -> 0; true -> CurrentCommitTimes + AddNum end,
                succ_times = if (CurrentSuccTimes + AddNum) < 0 -> 0; true -> CurrentSuccTimes + AddNum end};
        true ->
            NewCounterData = CounterData#mission_counter{
    ...
    case common_mission:is_in_transaction() of
        true->
            case get(?MISSION_DATA_DICT_KEY_COPY(RoleID)) of
                undefined->
                    KeyError = 1,
                    get(?MISSION_DATA_DICT_KEY(RoleID));
                Data->
                    KeyError = 2,
                    Data
            end;
           ...
           fun(Mission,AccUseItemPointTuple) -> 
                   case AccUseItemPointTuple =:= undefined 
                            andalso (Mission#p_mission_info.model =:= ?MISSION_MODEL_16 
                                                                orelse Mission#p_mission_info.model =:= ?MISSION_MODEL_17) of
                       true ->
