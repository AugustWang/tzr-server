...
t_do_donate(RoleID, DGold) ->
    ...
    case Gold+GoldBind < DGold of
        true ->
            Reason = lists:flatten(io_lib:format(?_LANG_JAIL_DONATE_NOT_ENOUGH_GOLD, [DGold-Gold-GoldBind])),
    ...
    {RestGold, RestGoldBind} = mod_role2:calc_rest_money(Gold, GoldBind, PKPoints-PKPoints2),
    RoleAttr2 = RoleAttr#p_role_attr{gold=RestGold, gold_bind=RestGoldBind},
    mod_map_role:set_role_attr(RoleID, RoleAttr2),

    %% 消费日志
    common_consume_logger:use_gold({RoleID, GoldBind-RestGoldBind, Gold-RestGold, ?CONSUME_TYPE_GOLD_JAIL_DONATE,
                                      ""}),

    {PKPoints, PKPoints2, RestGold, RestGoldBind}.
