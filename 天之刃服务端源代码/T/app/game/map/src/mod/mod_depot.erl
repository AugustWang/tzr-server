...
do_dredge_depot({Unique, Module, Method, DataIn, RoleID, _Pid, Line,_State})->
    ...
    Data = 
        try
            ...
            common_hook_achievement:hook({mod_depot,{RoleID,BagID}}),
...
do_dredge_depot2(BagID, RoleID) ->
    case db:transaction(
           fun()->
                   ...
                   mod_bag:create_bag(RoleID,{BagID,0,0,6,7,42}),
                   [BagBasicInfo]= db:read(?DB_ROLE_BAG_BASIC_P,RoleID),
                   #r_role_bag_basic{bag_basic_list=BagBasicList} = BagBasicInfo,
                   NewBagBasicInfo = BagBasicInfo#r_role_bag_basic{bag_basic_list=[{BagID,0,0,6,7,42}|BagBasicList]},
...
do_destroy({Unique, Module, Method, DataIn, RoleID, _Pid, Line,_State}) ->
    ...
    case db:transaction(fun() -> delete_goods(RoleID, GoodsID) end) of
        ...
        {aborted, Reason} ->
            ?DEBUG("destroy_goods transaction fail, reason = ~w~n", [Reason]),
            R = #m_depot_destroy_toc{succ = false,reason = ?_LANG_SYSTEM_ERROR};
        {atomic, {ok,Goods}} ->
...
get_bag_and_pos(RoleID,BagID,Position) ->
    case 
        is_integer(BagID) andalso
        BagID > 0 andalso 
        is_integer(Position) andalso 
        Position > 0 
    of
        true ->
            {ok,BagID,Position};
...
do_tidy({Unique, Module, Method, DataIn, RoleID, _Pid, Line,_State}) ->
    ...
    case db:transaction(fun() -> mod_bag:tidy_bag(RoleID,BagID) end) of
        ...
        {atomic, {_,GoodsList}} ->
            ?DEBUG("~w~n",[GoodsList]),
            %% 特殊任务事件
            ?TRY_CATCH(hook_mission_event:hook_special_event(RoleID,?MISSON_EVENT_TIDY_DEPOT),MissionEventErr),
