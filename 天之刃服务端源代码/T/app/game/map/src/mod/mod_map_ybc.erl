...
ybc_change_map2(RoleID, YbcID, YbcMapInfo, NewMapProcessName, NewMapID, NewPos) ->
    ...
    case YbcMapInfo#p_map_ybc.group_type of
        2 ->
            case (NewMapID =:= 13101 andalso mgeem_map:get_mapid() =:= 13100) orelse
                (NewMapID =:= 13105 andalso mgeem_map:get_mapid() =:= 13101) orelse 
                (NewMapID =:= 12101 andalso mgeem_map:get_mapid() =:= 12100) orelse
                (NewMapID =:= 12105 andalso mgeem_map:get_mapid() =:= 12101) orelse 
                (NewMapID =:= 11101 andalso mgeem_map:get_mapid() =:= 11100) orelse
                (NewMapID =:= 11105 andalso mgeem_map:get_mapid() =:= 11101) 
            of
                true ->
                        ...
                        false -> 
                            ...
                            lists:foreach(
                              fun({RID, RName}) ->
                                      ...
                                      RoleMsgOther = io_lib:format(?_LANG_FAMILY_YBC_FARAWAY_OTHER, [RName]),
...
do_ybc_dead(YbcID, YbcMapInfo, RoleID) ->
    %% 广播镖车的死亡信息
    ...
    case db:dirty_read(?DB_YBC, YbcID) of
        [#r_ybc{role_list=RoleList, level=Level, faction_id=FactionID, group_id=GroupID, group_type=GroupType, creator_id=CreatorID}] ->
            SilverAll = lists:foldl(
                          fun({_RID, _RName, _RLevel, _SB, Silver}, Acc) ->
                                  Acc + Silver
                          end, 0, RoleList),            
            ...
            case Level > 30 of
                true ->
                    ...
                    mod_map_drop:drop_silver(YbcMapInfo#p_map_ybc.pos, DropSilver);
                false ->
                    ignore
            end,            
            %% 处理PK情况
...
do_timeout_ybc(YbcID) ->
    case db:dirty_read(?DB_YBC, YbcID) of
        [#r_ybc{role_list=RoleList, creator_id=CreatorID} = YbcInfo] ->
            ...
            if
                YbcGroup =:= 1 ->
                    Content=common_letter:create_temp(?PERSONAL_YBC_TIME_OUT, [?NPC_PERSONAL_BIAO_CHE]),
                    ?TRY_CATCH(common_letter:sys2p(CreatorID, Content, ?_LANG_YBC_TIMEOUT_LETTER_TITLE,3),Err),
                    mod_ybc_person:timeout(YbcID);
                true ->
                    Content = common_letter:create_temp(?FAMILY_YBC_TIME_OUT, []),
                    ?TRY_CATCH(common_letter:sys2p(CreatorID, Content, ?_LANG_YBC_TIMEOUT_LETTER_TITLE,3),Err),
...
do_enter(YbcInfo) ->
    ...
    case Status =/= ?YBC_STATUS_KILLED andalso EndTime > common_tool:now() of
        true ->
            mod_map_actor:enter(0, YbcID, YbcID, ybc, YbcMapInfo, 0, mgeem_map:get_state());
        false ->
            do_del_ybc(YbcID)
    end.
