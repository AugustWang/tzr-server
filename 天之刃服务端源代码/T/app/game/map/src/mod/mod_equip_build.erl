...
%% 获取装备五行改造材料
handle({Unique, ?EQUIP_BUILD, ?EQUIP_BUILD_FIVEELE_GOODS, DataRecord, RoleId, Pid, Line},State)
  when erlang:is_record(DataRecord,m_equip_build_fiveele_goods_tos) ->
    mod_equip_fiveele:do_equip_build_fiveele_goods({Unique, ?EQUIP_BUILD, ?EQUIP_BUILD_FIVEELE_GOODS, DataRecord, RoleId, Pid, Line},State);
%% 装备五行改造处理
handle({Unique, ?EQUIP_BUILD, ?EQUIP_BUILD_FIVEELE, DataRecord, RoleId, Pid, Line},State) 
  when erlang:is_record(DataRecord,m_equip_build_fiveele_tos) ->
    mod_equip_fiveele:do_equip_build_fiveele({Unique, ?EQUIP_BUILD, ?EQUIP_BUILD_FIVEELE, DataRecord, RoleId, Pid, Line},State);
...
do_transaction_build_equip({RoleId,DataRecord,EquipBaseInfo,AddGoodsList,AddItem,AddGoodsNum,Fee}) ->
    ...
    ?DEBUG("~ts,EquipPunchNum=~w",["打造装备的开孔数",EquipPunchNum]),
    %% 基础数据
...
do_transaction_build_equip3_1({RoleId,_DataRecord,EquipGoods,_AddItem,_AddGoodsNum,_Fee},EquipBind) ->
    ...
    %% 占用那一个背包和格子
    %% 装备打造时需要处理装备套装属性
    EquipGoods4 = mod_equip_fiveele:do_random_equip_whole_attr(EquipGoods3),
    {ok,[EquipGoods5]}= mod_bag:create_goods_by_p_goods(RoleId,EquipGoods4),
    {ok,EquipGoods5}.
%% 扣费
