...
%%%
%%% @end
%%% Created : 10 Oct 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
...
do_war_apply(AttackFactionID,DefenceFactionID) ->
    ...
    case check_map_is_pingjiang(mgeem_map:get_mapid()) of
        ...
        false ->
            ...
            Content = common_tool:to_list(AttackFactionName) ++ "将于20:00开始进攻" ++ common_tool:to_list(DefenceFactionName) ++ "，请广大国民做好战争准备，\\n国战胜利的所有国民获得2个小时的双倍经验时间。",
...
%%根据当前地图的ID检查当前地图有关国战的NPC是否存在，平江检查箭塔，王都检查张将军和国旗
waroffaction_server_npc_born() ->
...
%%      ?ERROR_MSG("555555555555555,~w",[GuarderInfo]),
    ...
    mod_server_npc:init_map_server_npc(MapName, MapID, [ServerNpcInfo], ?SERVER_NPC_CREATE_TYPE_MANUAL),
...
get_waroffaction_npc_typeid_list(MapID) ->
    case MapID of
        11100 ->
            [get_genernal_typeid_by_mapid(MapID),21202001];   %%云州王都张将军和国旗
        11102 ->
            [get_tower_typeid_by_mapid(MapID)];   %%云州平江瞭望塔
        12100 ->
            [get_genernal_typeid_by_mapid(MapID),22202001];   %%沧州王都
        12102 ->
            [get_tower_typeid_by_mapid(MapID)];   %%沧州平江
        13100 ->
            [get_genernal_typeid_by_mapid(MapID),23202001];   %%幽州王都
        13102 ->  
            [get_tower_typeid_by_mapid(MapID)];   %%幽州平江
        _ ->
              []
    end.
...
check_map_is_pingjiang(MapID) ->
    MapID =:= 11102 orelse MapID =:= 12102 orelse MapID =:= 13102.
%%判断是不是王都
%% check_map_is_jingcheng(MapID) ->
%%      MapID =:= 11100 orelse MapID =:= 12100 orelse MapID =:= 13100.
%%根据国家ID获取将军的type_id
get_general_npc_type_id(FactionID) ->
    case FactionID of
        1 -> get_genernal_typeid_by_mapid(11100);
        2 -> get_genernal_typeid_by_mapid(12100);
        3 -> get_genernal_typeid_by_mapid(13100)
    end.
%%根据国家ID获取国旗的type_id
...
tower_dead(_RoleName) ->
    ...
    set_war_stage(?WAROFFACTION_SECOND_STAGE),
    DefenceFactionName = get_faction_name_by_id(DefenceFactionID),
    AttackContent = DefenceFactionName ++ "平江哨塔已被摧毁，大家攻进" ++ DefenceFactionName ++ "王都，\\n击杀敌方禁卫将军，夺取下一阶段的胜利！",
...
general_dead(RoleName) ->
    ...
    DefenceFactionName = get_faction_name_by_id(DefenceFactionID),
    AttackFactionName = get_faction_name_by_id(AttackFactionID),
    AttackContent = DefenceFactionName ++ "禁卫将军已被我国勇士<font color=\"#FFFF00\">[" ++ common_tool:to_list(RoleName) ++ "]</font>击杀，胜利就在眼前，\\n请大家攻入王都王宫，摧毁" ++ DefenceFactionName ++ "国旗，夺取最终的胜利。",
    DefenceContent = "禁卫将军已被敌国玩家击杀，请所有国民坚守最后的防线，阻止" ++ AttackFactionName ++ "摧毁国 旗。",
    common_broadcast:bc_send_msg_faction(AttackFactionID,[?BC_MSG_TYPE_CENTER,?BC_MSG_TYPE_CHAT],?BC_MSG_TYPE_CHAT_COUNTRY,common_tool:to_list(AttackContent)),
    common_broadcast:bc_send_msg_faction(DefenceFactionID,[?BC_MSG_TYPE_CENTER,?BC_MSG_TYPE_CHAT],?BC_MSG_TYPE_CHAT_COUNTRY,DefenceContent),
...
faction_flag_dead(RoleName) ->
    ...
    DefenceContent = "虽然大家英勇奋战，但是无奈敌国实力太强，本次国战失败，\\n希望大家知耻而后勇，下次国战的时候一雪前耻!",
    common_broadcast:bc_send_msg_faction(AttackFactionID,[?BC_MSG_TYPE_CENTER,?BC_MSG_TYPE_CHAT],?BC_MSG_TYPE_CHAT_COUNTRY,common_tool:to_list(AttackContent)),
...
do_end_war_when_time_out() ->
    case erlang:get(?WAROFFACTION_WIN_FACTION_ID) of
        undefined ->
            ...
            write_waroffaction_record(AttackFactionID,AttackRecord),
            AttackFactionName = get_faction_name_by_id(AttackFactionID),
            DefenceRecord = "感谢大家的英勇奋战，" ++ AttackFactionName ++"国的入侵未能得逞，我国所有国民获得2个小时的双倍经验时间。",
        ...
        false ->
            case decute_faction_silver(AttackFactionID,100000) of
                {ok,NewSilver} ->
                    ...
                    RoleName = RoleMapInfo#p_map_role.role_name,
                    DefenceFactionName = get_faction_name_by_id(DefenceFactionID),
                    AttackRecord = OfficeName ++ "<font color=\"#FFFF00\">[" ++ common_tool:to_list(RoleName) ++ "]</font>花费10锭银子向" ++ DefenceFactionName ++ "发动国战",
                    write_waroffaction_record(AttackFactionID,AttackRecord),
                    AttackFactionName = get_faction_name_by_id(AttackFactionID),
                    DefenceRecord = AttackFactionName ++ "<font color=\"#FFFF00\">[" ++ common_tool:to_list(RoleName) ++ "]</font>花费10锭银子向我国发动国战！",
                    write_waroffaction_record(DefenceFactionID,DefenceRecord),
...
do_born_waroffaction_guarder_npc(Line, Unique, RoleID, FactionID, MapID, MapName, GuarderInfo, RoleName) ->
    ...
    case check_can_bug_guarder(GuarderInfo#r_waroffaction_guarder_info.key) of
        ok ->
            ...
            mod_server_npc:init_map_server_npc(MapName, MapID, ServerNpcList, ?SERVER_NPC_CREATE_TYPE_MANUAL),
...
write_record_when_buy_guarder(RoleID,FactionID,GuarderInfo,NpcName,RoleName) ->
    ...
    Record = OfficeName ++ "<font color=\"#FFFF00\">[" ++ common_tool:to_list(RoleName) ++ "]</font>花费" ++ Cost ++ "银子招募" ++ NpcName,
...
do_waroffaction_cycle_broadcast(AttackFactionID,DefenceFactionID) ->
    ...
    DefenceFactionName = get_faction_name_by_id(DefenceFactionID),
    AttackContent = "请参战国民前往" ++ DefenceFactionName ++ "国夺取平江石桥，并破坏" ++ DefenceFactionName ++ "哨塔。",
    DefenceContent = "请参战国民前往平江石桥进行防守，防止敌人破坏平江哨塔。",
...
get_faction_name_by_id(FactionID) ->
    case FactionID of
        1 ->"<font color=\"#00FF00\">云州</font>";
        2 ->"<font color=\"#F600FF\">沧州</font>";
        3 ->"<font color=\"#00CCFF\">幽州</font>"
    end.
        ...
        _ ->
            case mod_map_actor:get_actor_mapinfo(RoleID, role) of
                ...
                RoleMapInfo ->
                    ...
                    %% 一些特殊状态不能接受
                    case State =:= ?ROLE_STATE_DEAD orelse State =:= ?ROLE_STATE_TRAINING 
                        orelse State =:= ?ROLE_STATE_STALL of
