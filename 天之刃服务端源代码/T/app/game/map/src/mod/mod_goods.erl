            ...
            case get_equip_by_id(TargetID, GoodsID) of
                {error, _} ->
                    ...
                    case common_misc:is_role_online(TargetID) of
                        true ->
                            common_misc:send_to_rolemap(TargetID, {mod_goods, {get_goods_info, RoleID, TargetID, GoodsID, Type, Line, Unique}});
                        _ ->
...
do_destroy(Unique, Module, Method, DataRecord, RoleID, Line) ->
    ...
    case db:transaction(
        ...
        {atomic, {ok, GoodsID, NewSkin, GoodsInfo,use_equip}} ->
            hook_prop:hook(decreate, [GoodsInfo]),
            Data = #m_goods_destroy_toc{succ = true,id = GoodsID},
            %% 重算属性
            ...
            case NewSkin of
                undefined ->
                    next;
                _ ->
                    mod_map_role:do_update_map_role_info(RoleID, [{#p_map_role.skin, NewSkin}], mgeem_map:get_state())
            end
    end,
    common_misc:unicast(Line, RoleID, Unique, Module, Method, Data).
...
pick_dropthing2(DropThing,Unique,RoleID) ->
    ...
    {Result,Data} = 
        case db:transaction(
               fun() -> t_change_goods_owner({RoleID,GoodsID}) end)
        of
            {atomic, {NewInfo,OldInfo}} ->
                ...
                mod_map_drop:do_pick_dropthing_return(true, DropThing, mgeem_map:get_state()),
                hook_prop:hook(create, [NewInfo]),
                %% add by caochuncheng 2010-12-09 添加拾取物品通知队员消息
                catch do_pick_dropthing_notify(RoleID, NewInfo#p_goods{current_num=OldInfo#p_goods.current_num}),
                %% add by caochuncheng 2011-04-21 添加场景大战拾取物品世界通知
                catch do_pick_dropthing_world_notify(RoleID,NewInfo#p_goods{current_num=OldInfo#p_goods.current_num}),
            ...
            {aborted, {throw, {bag_error, not_enough_pos}}} ->
                mod_map_drop:do_pick_dropthing_return(false, DropThing, mgeem_map:get_state()),
...
pick_dropthing3(DropThing,Unique,RoleID) ->
    ...
    {Result,Data} = 
        case db:transaction(fun() -> t_creat_drop_goods(RoleID,DropThing) end) of
            {aborted, {throw, {bag_error, not_enough_pos}}} ->
                mod_map_drop:do_pick_dropthing_return(false, DropThing, mgeem_map:get_state()),
                {error, pick_dropthing_error(ID, ?_LANG_DROPTHING_BAG_FULL)};
            {aborted, Reason}->
            ...
            {atomic, Info} ->
                ...
                %% add by caochuncheng 2010-12-09 添加拾取物品通知队员消息
                catch do_pick_dropthing_notify(RoleID, Info#p_goods{current_num=Num}),
                %% add by caochuncheng 2011-04-21 添加场景大战拾取物品世界通知
                catch do_pick_dropthing_world_notify(RoleID,Info#p_goods{current_num=Num}),
...
pick_goods_log(GoodsList) ->
    lists:foreach(
      fun(Goods) ->
              ...
              common_item_logger:log(RoleID,Goods,?LOG_ITEM_TYPE_SHI_QU_HUO_DE)
      end,GoodsList).
...
t_creat_drop_goods(RoleID,DropThing)
  when is_record(DropThing,p_map_dropthing) ->
    ...
    case t_creat_drop_goods2(RoleID,Type,TypeID,Num,DropProperty,Bind) of
        {ok, Info} ->
            ?DEBUG("~ts,RoleId=~w,Goods=~w",["拾取到的物品信息昿",RoleID,Info]),
            Info;
        {error, Reason} ->
            db:abort(Reason)
    end.
...
t_creat_drop_goods2(RoleID,Type,TypeID,Num,DropProperty,Bind)
  when is_record(DropProperty,p_drop_property) ->
    ...
    case Type of
        ?TYPE_ITEM ->
            CreateInfo = #r_goods_create_info{bind=Bind, type=?TYPE_ITEM, type_id=TypeID, num=Num},
            {ok,[NewInfo]} = mod_bag:create_goods(RoleID,CreateInfo),
            {ok,NewInfo};
        ?TYPE_STONE ->
            ...
            {ok,NewInfo};
        ?TYPE_EQUIP  ->
            t_creat_drop_goods5(RoleID, Type, TypeID, Num, DropProperty);
        Error ->
            db:abort(Error)
    end.

t_creat_drop_goods5(RoleID, Type, TypeID, Num, DropProperty) ->
    %% case common_bag:get_empty_bag_pos(RoleID) of
    Info = #p_goods{
      type = Type,
      roleid = RoleID ,
      typeid = TypeID,
      ...
      current_num = Num},
    {ok,t_creat_drop_goods3(Type,Info,DropProperty)}.

t_creat_drop_goods3(?TYPE_EQUIP,Goods,DropProperty) ->
    case mod_equip:get_equip_baseinfo(Goods#p_goods.typeid) of
        error ->
            {error, ?_LANG_ITEM_NO_TYPE_EQUIP};
        {ok, BaseInfo} ->
            NewGoods = Goods#p_goods{sell_type = BaseInfo#p_equip_base_info.sell_type,
            ...
            t_creat_drop_goods4(#p_drop_property.bind,NewGoods,DropProperty)   
    end.

t_creat_drop_goods4(#p_drop_property.bind,Goods,DropProperty) ->
    t_creat_drop_goods4(#p_drop_property.use_bind,
                      Goods#p_goods{bind=DropProperty#p_drop_property.bind},
                      DropProperty);
t_creat_drop_goods4(#p_drop_property.use_bind,Goods,DropProperty) ->
    t_creat_drop_goods4(#p_drop_property.colour,
                      Goods#p_goods{use_bind=DropProperty#p_drop_property.use_bind},
                      DropProperty);
t_creat_drop_goods4(#p_drop_property.colour,Goods,DropProperty) ->
    t_creat_drop_goods4(#p_drop_property.quality,
                      Goods#p_goods{current_colour=DropProperty#p_drop_property.colour},
                      DropProperty);
t_creat_drop_goods4(#p_drop_property.quality,Goods,DropProperty) ->
    {Quality,SubQuality} = mod_refining_tool:get_equip_quality_by_color(DropProperty#p_drop_property.colour),
    t_creat_drop_goods4(#p_drop_property.hole_num,
                      Goods#p_goods{quality=Quality,sub_quality = SubQuality,quality_rate = 0},
                      DropProperty);
t_creat_drop_goods4(#p_drop_property.hole_num,Goods,DropProperty) ->
    NewGoods = mod_refining:equip_colour_quality_add(new,
                 Goods#p_goods{punch_num=DropProperty#p_drop_property.hole_num},1,1,1),
    #p_goods{roleid=RoleID}=NewGoods,
    ?DEV("~w,~w~n",[Goods,NewGoods]),
    %% 掉落的物品是装备的话，如果是绑定的，需要处理一下绑定操佊    
    NewGoods2 = 
        if NewGoods#p_goods.bind andalso NewGoods#p_goods.type =:= ?TYPE_EQUIP ->
                case mod_refining_bind:do_equip_bind_for_monster_flop(NewGoods#p_goods{bind = false}) of
                    {error,ErrorCode} ->
                        ?DEBUG("~ts,ErrorCode=~w,Goods=~w",["怪物掉落绑定装备，执行绑定操作失败，但不处理返回不绑定的装备",ErrorCode,NewGoods]),
                        NewGoods;
                    {ok,BindGoods} ->
                        ?DEV("~ts,Goods=~w,BindGoods=~w",["怪物掉落绑定装备，执行绑定操作成功结枿",NewGoods,BindGoods]),
                        BindGoods
                end;
           true ->
                NewGoods 
    ...
    NewGoods3 = 
        if NewGoods2#p_goods.bind 
           andalso NewGoods2#p_goods.type =:= ?TYPE_EQUIP ->
                if NewGoods2#p_goods.use_bind =:= 1 ->
                        NewGoods2#p_goods{bind = false};
                   true ->
                        NewGoods2#p_goods{use_bind = 0}
                end;
           true ->
                NewGoods2#p_goods{use_bind = 0}
        end,
    NewGoods4 = 
        case common_misc:do_calculate_equip_refining_index(NewGoods3) of
            {error,RIErrorCode} ->
  ...
  %%  ?DEBUG("~ts,",["计算装备精炼系数出错",NewNewNewGoods#p_goods.goodsid]),
    {ok,[NewGoods5]} = mod_bag:create_goods_by_p_goods(RoleID,NewGoods4),
    NewGoods5.
...
do_pick_dropthing_notify4(_RoleId,Goods,MapRoleInfo,RoleIdList) ->
    ...
    catch common_broadcast:bc_send_msg_role(RoleIdList,?BC_MSG_TYPE_SYSTEM,Message),
    ok.
        ...
        {error, _} ->
            case get_dirty_equip_by_id(RoleID, GoodsID) of
                {ok, GoodsInfo} ->
                    #m_goods_info_toc{info=GoodsInfo, type=Type, goods_id=GoodsID};
                _ ->
                    #m_goods_info_toc{succ=false, reason=?_LANG_GOODS_NOT_IN_BAG, type=Type, goods_id=GoodsID}
            end
    end.
