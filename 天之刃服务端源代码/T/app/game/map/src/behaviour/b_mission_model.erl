...
%%@doc 派发侦听器
dispatch_listener(RoleID, MissionID, ListenerTriggerData) when is_integer(MissionID) ->
    ...
    if
        ...
        true ->
            call_mission_model(RoleID, MissionID, listener_trigger, [ListenerTriggerData, PInfo])
    end;
dispatch_listener(RoleID, MissionIDList, ListenerTriggerData) when is_list(MissionIDList) ->
    try
        lists:foreach(fun(MissionID) ->
            case dispatch_listener(RoleID, MissionID, ListenerTriggerData) of
                {atomic, _} ->
                    ignore;
                Error ->
                    throw(Error)
