...
%%用于分布式启动
do_start_map_distribution(MAPID, MAPProcessName) ->
...
%%分线中的玩家下线时会call这个分支
handle_call({client_exit, _RoleID, _PID}, _, State) ->
    ?ERROR_MSG("傻X了，调用了废弃的代码逻辑",[]),
    %%catch do_line_client_exit(RoleID, PID),    
    {reply, ok, State};
...
create_map_if_not_exist(MAPID) ->
    ...
    case global:whereis_name(MAPProcessName) of
        undefined ->
            ...
            pool:pspawn(mgeem_router, do_start_map_distribution, [MAPID, MAPProcessName]),
            ok;
        _ ->
            ok
    end.

create_copy_map(MapID, MapName) ->
    case global:whereis_name(MapName) of
        undefined ->
             pool:pspawn(mgeem_router, do_start_map_distribution, [MapID, MapName]),
            ok;
        _ ->
            ok
    end.
...
get_all_map_node() ->
    case erlang:get(all_map_node) of
        undefined ->
            Nodes = pool:get_nodes(),
            erlang:put(all_map_node, Nodes),
            Nodes;
        N ->
            N
    end.
...
do_random_start(MapID, MapProcessName, Module, FromPID, Key) ->
    MapNodes = get_all_map_node(),
    Node = lists:nth(random:uniform(erlang:length(MapNodes)), MapNodes),
    rpc:async_call(Node, mgeem_router, do_create_map_distribution2, [MapID, MapProcessName, Module, FromPID, Key]).
    

do_create_map_distribution2(MapID, MapProcessName, Module, FromPID, Key) ->
    case supervisor:start_child(mgeem_map_sup, [{MapProcessName, MapID}]) of
        {ok, _MAPNewPid} ->
            FromPID ! {Module, {create_map_succ, Key}},
            ok;
        {error, {already_started, _}} -> 
            FromPID ! {Module, {create_map_succ, Key}},
            ok;
