...
join_family(RoleID, FamilyID, FamilyInfo) ->
    ?TRY_CATCH(  common_mod_goal:hook_family_change(RoleID, FamilyID) , Error1),
    ?TRY_CATCH(  common_mod_goal:family_level_up([RoleID], FamilyInfo#p_family_info.level) , Error2),
...
role_contribution_change(RoleID, NewContribution) ->
    common_mod_goal:role_family_contribution_change(RoleID, NewContribution).

level_up(Members, Level) ->
    RoleIDList = [RoleID || #p_family_member_info{role_id=RoleID} <- Members],
    common_mod_goal:family_level_up(RoleIDList, Level).


notify_family_contribute_change(RoleID,NewFamilyContrb)->
    %%通知map
    Info = {change_attr,family_contribute,RoleID,NewFamilyContrb},
