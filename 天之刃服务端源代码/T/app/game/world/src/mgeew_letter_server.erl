%%% -------------------------------------------------------------------
%%% @Author  : markycai <caisiqiang@mingchao.com>
%%% @doc   信件模块
%%% @end
%%% Created : 2011-3-23
%%% -------------------------------------------------------------------
...
%% 玩家检查获取物品-------------------------
do_check_accept_goods({_Unique, _Module, _Method, DataIn, RoleID, _Pid, _Line}) ->      
    #m_letter_accept_goods_tos{letter_id=ID,table = Table}=DataIn,
    %% 写入队列
    case wirte_in_request_queue(RoleID,ID,Table) of
        {ok,Queue}->
...
%% 玩家获取物品成功 -------------------
do_accept_goods({public,LetterID,RoleID})->
    %%数据库数据字段修改 外加清除进程字典
        ...
        [PublicLetter] when is_record(PublicLetter,r_public_letter) ->
            ...
            case lists:keyfind(LetterID, #r_letter_detail.id, LetterBox) of
                DetailLetter when is_record(DetailLetter,r_letter_detail) ->
                    ...
                    erase({?ACCEPT_GOODS_REQUEST_QUEUE,RoleID});
                false ->
                    ignore
            end;
        _->
...
do_accept_goods({personal,LetterID,RoleID})->
...
get_letter_content({{RecordType,Key},List},FactionID)->
    ?DEBUG("letter content ~w~n",[{RecordType,Key}]),
        case RecordType of
            ?DATABASE_LETTER->
...
%% 写入队列
wirte_in_request_queue(RoleID,LetterID,Table)->
    case get({?ACCEPT_GOODS_REQUEST_QUEUE,RoleID}) of
        Queues when is_record(Queues,r_accept_goods_request)->
            #r_accept_goods_request{time = Time} = Queues,
            case common_tool:now()-Time>?REQUEST_OVER_TIME of
                true->
                    NewQueues = 
                        #r_accept_goods_request{time = common_tool:now(),
                                                letter_id = LetterID,
                                                table = Table},
                    put({?ACCEPT_GOODS_REQUEST_QUEUE,RoleID},NewQueues),
                    {ok,NewQueues};
                false->
                    {error,too_often_request}
            end;
        undefined-> 
            NewQueues = 
                #r_accept_goods_request{time = common_tool:now(),
