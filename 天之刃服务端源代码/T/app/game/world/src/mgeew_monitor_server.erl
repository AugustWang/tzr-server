...
-define(MONITOR_LOG_DIR, "/data/logs/monitor/").


-define(CURRENT_VERSION_INFO,current_version_info).
-define(DUMP_VERSION_DATA, dump_version_data).
-define(VERSION_INFO_DIR,"/data/tzr/server/version_server.txt").
%% --------------------------------------------------------------------
%% Include files
%% --------------------------------------------------------------------
-include("mgeew.hrl").
...
do_dump_monitor_data()->
    %% 将ets数据dump到文件或DB中，
    case create_log_dir(?MONITOR_LOG_DIR) of
        ok->
            LogFileName = get_log_file_name(?MONITOR_LOG_DIR),
        ...
        {error,Reason}->
            ?ERROR_MSG("create_log_dir failed!Path=~w,Reason=~w",[?MONITOR_LOG_DIR,Reason])
    end,
    do_dump_to_mysql(),
...
get_log_file_name(LogDir)->
    ...
    lists:concat([LogDir, "/", FileName, ".log"]).
