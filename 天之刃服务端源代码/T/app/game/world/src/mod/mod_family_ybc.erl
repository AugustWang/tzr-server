...
%%注意，门派拉镖的接口Method和MsgTag都必须同时在此同步！
method_list()->
    [?FAMILY_YBC_PUBLISH,
     ?FAMILY_YBC_LIST,
     ?FAMILY_YBC_COMMIT,
...
do_ybc_invite(Unique, Module, Method, RoleID, PID) ->
    case catch do_ybc_invite_check(RoleID) of
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["邀请帮众拉镖发生系统错误", Error]),
...
do_ybc_giveup(Unique, Module, Method, RoleID, PID) ->
    case catch do_ybc_giveup_check(RoleID) of
        ok ->
            ...
            case CreatorID =:= RoleID of
                true ->
                    ...
                    log_family_ybc(lists:concat(["创建者", common_tool:to_list(common_misc:get_dirty_rolename(RoleID)),"解散了门派拉镖"])),
                    ok;
                false ->
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["放弃门派拉镖时发生系统错误", Error]),
    ...
    case catch do_ybc_sure_check(RoleID) of
        ok ->
            ...
            RoleList = lists:foldl(
                         fun(RID, Acc) ->
                                 [#p_role_attr{role_name=RoleName, level=Level}] = db:dirty_read(?DB_ROLE_ATTR, RID),
            ...
            [#p_role_attr{level=CreatorLevel}] = db:dirty_read(?DB_ROLE_ATTR, RoleID),
...
%% 门派镖车基础血量
-define(FAMILY_YBC_BASIC_HP, 50000).

%% 门派镖车基础等级血量
-define(FAMILY_YBC_LEVEL_HP, 1000).
...
do_ybc_publish(Unique, Module, Method, Record, RoleID, PID) ->
    ...
    case catch do_ybc_publish_check(RoleID, Level) of
        ...
        {error, Reason} ->
            do_ybc_publish_error(Unique, Module, Method, Reason, PID);
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["检查门派发布拉镖任务出错", Error]),
...
do_ybc_agree_publish(Unique, Module, Method, RoleID, PID) ->
    case catch do_ybc_agree_publish_check(RoleID) of
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["处理玩家同意门派拉镖出错", Error]),
...
do_ybc_agree_publish_check(RoleID) ->
    ...
    [#p_role_base{status=Status}] = db:dirty_read(?DB_ROLE_BASE, RoleID),
                ...
                {atomic, ok} ->
                    ...
                    LogResult = lists:foldl(
                                  fun({RID, RName, Level, _, _}, Acc) ->
                                          ...
                                          lists:concat([Acc, "帮众:", common_tool:to_list(RName),"获得奖励:经验值,",Exp,".门派贡献度,",FC])
                                  end, "", AllRoleList),

                    common_misc:send_to_rolemap(RoleID, {mod_map_ybc, {del_ybc, YbcID}}),
		    log_family_ybc(YbcID,lists:concat(["超时完成门派镖车任务,获得奖励的有---",LogResult])),
                    ok;
                {aborted, Error} ->
                    case erlang:is_binary(Error) of
                        ...
                        false ->
                            ...
                            ?ERROR_MSG("~ts:~w", ["处理完成门派镖车任务时发生系统错误", Error])
                    end,
                    do_ybc_commit_error(Unique, Module, Method, Reason, PID)
            end,
            clear_ybc_commit_request()
                    ...
                    LogResult = lists:foldl(
                                  fun({RID, RName, Level, _BSilver, SilverTmp},Acc) ->
                                          ...
                                          R2 = #m_family_ybc_commit_toc{return_self=false, exp=Exp, silver=Silver,
                                                                        ...
                                                                        active_point=ActivePoint},
                                          common_misc:unicast({role, RID}, ?DEFAULT_UNIQUE, Module, Method, R2),
                                          lists:concat([Acc, common_tool:to_list(RName), ",获得经验:", Exp, ",获得银两:", Silver, ",获得门派贡献:", FC])
                                  end,"",AllRoleList),
                    log_family_ybc(YbcID,lists:concat(["门派正常完成拉镖任务:获得奖励的有:",LogResult])),
...
%%门派交镖，使用异步方式完成，先发消息给地图，由地图确认哪些玩家可以获取奖励
                ...
                true ->
                    ...
                    case db:transaction(fun() -> t_do_ybc_commit_timeout(YbcID) end) of
                        {atomic, ok} ->
                            ...
                            log_family_ybc(YbcID,"超时完成门派镖车任务");
                        {aborted, Error} ->
                            ?ERROR_MSG("~ts:~w", ["处理完成门派镖车任务时发生系统错误", Error]),
    ...
    case catch do_ybc_kick_check(RoleID, TRoleID) of
        ok ->       
            ...
            case db:transaction(fun() -> t_do_ybc_kick(YbcID, TRoleID) end) of
                {atomic, NewYbcInfo} ->
                    [#p_role_base{role_name=TRoleName}] = db:dirty_read(?DB_ROLE_BASE, TRoleID),
                ...
                {aborted, Error} ->
                    case erlang:is_binary(Error) of
                        ...
                        false ->
                            ...
                            ?ERROR_MSG("~ts:~w", ["处理门派镖车踢人时发生系统错误", Error])
                    end,
...
do_ybc_collect(Unique, Module, Method, Record, RoleID, PID) ->
    ...
    case catch do_ybc_collect_check(RoleID) of
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["处理门派拉镖拉人动作时发生系统错误", Error]),
