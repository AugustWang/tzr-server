...
do_fmlskill_list_buff({Unique, Module, Method, _Record, RoleID, _PID, Line})->
    ...
    if
        FamilyLevel<1 ->
            ?SEND_ERR_TOC(m_fmlskill_list_buff_toc,<<"必须升级到1级门派才能领取门派技能福利。">>);
        true->
...
do_fmlskill_fetch_buff({Unique, Module, Method, Record, RoleID, _PID, Line})->
    ...
    if
        FamilyLevel<1 ->
            ?SEND_ERR_TOC(m_fmlskill_fetch_buff_toc,<<"必须升级到1级门派才能领取门派技能福利。">>);
        true->
            case mod_family:is_today_not_parttake(RoleID,fetch_buff) of
                ...
                _ ->
                    ?SEND_ERR_TOC(m_fmlskill_fetch_buff_toc,<<"每天只能领取一次门派技能状态！">>)
            end
    end.
...
do_fmlskill_fetch_buff_2({Unique, Module, Method, RoleID, Line},FmlBuffID,FamilyLevel)->
    ...
    case CurConb>=NeedFamilyConb of
        true->
            ...
            common_misc:send_to_rolemap(RoleID, {mod_map_family,{fetch_family_buff,self(),RoleID,FmlBuffID,BuffLevel}});
        _ ->
            ?SEND_ERR_TOC(m_fmlskill_fetch_buff_toc,<<"你的门派贡献度不足，请通过参加门派活动获得！">>)
    end.


%%@doc 执行领取门派BUFF的结果(从地图节点返回)
    ...
    case mod_family:is_owner_or_second_owner(RoleID, FamilyInfo) orelse mod_family:is_intermanager(RoleID, FamilyInfo) of
        true ->
            case common_config_dyn:find(family_skill,SkillID) of
                []->
                    ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"没有此门派技能可以研究">>);
                [SkillDetails]->
                    do_fmlskill_research_2(InterfaceInfo,FamilyInfo,SkillDetails)
            end;
        _ ->
            ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"只有掌门,长老或内务使才能研究门派技能">>)
    end.
...
do_fmlskill_research_2({Unique, Module, Method, Record, RoleID, _PID, Line}=InterfaceInfo,FamilyInfo,SkillDetails)->
    ...
    if
        FamilyLevel < 1 ->
            ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"必须升级到1级门派，才能研究门派技能">>);
        NextSkillLevel > MaxFamilySkillLevel->
            ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"已达到最高的门派技能等级，不需要继续研究">>);
        NextSkillLevel > LimitSkillLevel ->
            ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"必须升级门派，才能继续研究下一级门派技能">>);
        true->
            ...
            if
                DeductMoney>FamilyMoney ->
                    ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"门派资金不足，不能研究该技能">>);
                DeductActivePt>FamilyActivePt ->
                    ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"门派繁荣度不足，不能研究该技能">>);
                true->
  ...
  when is_record(FamilyInfo,p_family_info) andalso is_record(FamilySkillDetail,r_family_skill)->
    ...
    case db:transaction( fun()-> t_fmlskill_research_3(FamilyID,SkillID,NextSkillLevel,SkillCount) end ) of
        {atomic,ok}->
            ...
            FamilyMsg = common_misc:format_lang(<<"<font color=\"#FFFF00\">[~s]</font>成功研究门派技能【~s】，大家可以进入门派地图找“门派研究员”学习该技能。">>,[RoleName,SkillName]),
            broadcast_to_family_channel(FamilyID,FamilyMsg);
        {aborted,{error,max_skill_count}}->
            ?SEND_ERR_TOC(m_fmlskill_research_toc,<<"门派技能个数达到上限，必须升级门派才能研究新技能">>);
        {aborted,Error}->
            ?ERROR_MSG("研究门派技能出错，Error=~w",[Error]),
...
do_fmlskill_forget({Unique, Module, Method, _Record, RoleID, _PID, Line}=InterfaceInfo)->
    ...
    case mod_family:is_owner_or_second_owner(RoleID, FamilyInfo) orelse mod_family:is_intermanager(RoleID, FamilyInfo)  of
        ...
        _ ->
            ?SEND_ERR_TOC(m_fmlskill_forget_toc,<<"只有掌门或长老才能遗忘门派技能">>)
    end.
do_fmlskill_forget_2({Unique, Module, Method, Record, RoleID, _PID, Line},FamilyInfo)->
    ...
    if
        DeductMoney>FamilyMoney->
            ?SEND_ERR_TOC(m_fmlskill_forget_toc,<<"门派资金不足，不能遗忘该技能">>);
        true->
            case db:transaction( fun()-> t_fmlskill_forget_3(FamilyID,SkillID) end ) of
                ...
                {aborted,{error,not_exists_skill}}->
                    ?SEND_ERR_TOC(m_fmlskill_forget_toc,<<"门派尚未研究该技能，不需要遗忘">>);
                {aborted,Error}->
                    ?ERROR_MSG("遗忘门派技能出错，Error=~w",[Error]),
