...
%%降级信件
-define(WARNING_FAMILY_LETTER_TITLE,"门派降级提示").
...
family_maintain_cost() ->
    ...
    case mod_family:get_state() of
        undefined ->
            ignore;
        #family_state{family_info=#p_family_info{family_id=FamilyID, hour=H,minute=M}} ->
...
do_family_maintain_cost_fail(_Reason)->
    ...
    if Level > 1 ->
        ...
        Message = lists:concat(["由于你所在的门派所需的日常维护的资金或门派繁荣度不够，你的门派已经降至",Level-1,"级。需要重新升级门派"]),
        mod_family:update_state(NewState);
       ...
       true ->
        ...
        close_map(FamilyID),
        Message = lists:concat(["由于你所在的门派所需的日常维护的资金或门派繁荣度不够，你的门派已经降至",0,"级。需要重新开启门派地图"])
    end,
...
family_level_down_letter(State)->
    ...
    Tips = case Level>0 of
               true -> 
                   lists:flatten(io_lib:format("返还门派繁荣度~w点和门派资金~w文。",[BackAc,BackMoney]));
               false -> 
                   mod_family:get_text_with_npc("需要重新激活门派地图。\n      激活条件：50元宝、5个在线的门派帮众，找<a href=\"event:N|~s\"><font color=\"#FFFF00\"><u>王都—门派管理员</u></font></a>。",FactionID,[?NPC_ZONG_ZU_GUAN_LI_YUAN])
           end,
...
change_family_owner()->
    ...
    if OwnerInfo1=/=none andalso OwnerID=/=OwnerInfo1#p_family_member_info.role_id ->
           ...
           mod_family:update_state(NewState),
           %%发送广播
           Message =lists:flatten(io_lib:format("[~s] 成为了门派掌门",[NewOwnerName])),
           mod_family:broadcast_to_family_channel(Message),
           %%推送取消长老数据
           if SOwnerInfo=/=none ->
           ...
           %%修改称号 
           common_title:add_title(?TITLE_FAMILY,NewOwnerID,?FAMILY_TITLE_OWNER),
           common_title:add_title(?TITLE_FAMILY, OwnerID, ?DEFAULT_FAMILY_MEMBER_TITLE);
       true->
           ignore
    end.
