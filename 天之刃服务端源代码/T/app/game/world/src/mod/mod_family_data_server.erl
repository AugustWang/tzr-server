...
do_handle_info({delete,FamilyID})->
    ...
    try
        ...
        _:Reason->
            ?ERROR_MSG("删除门派出错,Reason=~w,stacktrace=~w",[Reason,erlang:get_stacktrace()])
    end;
...
do_create_family_info(FamilyInfo,CreateTime) when is_record(FamilyInfo,p_family_info)->
    ...
    try 
        ...
        _:Reason->
            ?ERROR_MSG("创建门派数据出错,Reason=~w,stacktrace=~w",[Reason,erlang:get_stacktrace()])
    end.
...
load_family_time_list()->
    try
        ...
        _:Reason->
            ?ERROR_MSG("从mysql加载门派的创建时间出错,Reason=~w,stacktrace=~w",[Reason,erlang:get_stacktrace()]),
            throw({error,Reason})
    end.
...
do_dump_family_data()->
    try 
        ...
        _:Reason->
            ?ERROR_MSG("更新门派数据出错,Reason=~w,stacktrace=~w",[Reason,erlang:get_stacktrace()])
    end.
