...
%%%
%%% @end
%%% Created :  9 Jul 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
-module(mod_family).
...
-export([do_add_ac/1,do_add_money/1,add_exp_for_role/2,do_add_contribution/2]).
...
%%@doc 将门派的更新，通知给地图
%% mod by caochuncheng 2011-10-22 地图进程只需要更新门派id,门派名称，不需要将整个数据发送到地图进程
notify_world_update({RoleID, clear_role_family_skill}) ->
    common_misc:send_to_rolemap(RoleID, {mod_map_family,{clear_role_family_skill, RoleID}});
notify_world_update({RoleID, cancel_family}) ->
    common_misc:send_to_rolemap(RoleID, {mod_map_family,{cancel_role_family_info, RoleID, cancel_family}});
notify_world_update({RoleID, family_contribute,NewFC}) ->
    common_misc:send_to_rolemap(RoleID, {mod_map_family,{update_role_family_info, RoleID, family_contribute,NewFC}});
notify_world_update({RoleId,join_family,FamilyId,FamilyName,FamilyLevel}) ->
    common_misc:send_to_rolemap(RoleId, {mod_map_family,{update_role_family_info, RoleId, join_family,FamilyId, FamilyName,FamilyLevel}}).
...
%%@doc 将Record广播给每个帮众（排除指定的RoleID之外）
...
join_family_for_role(RoleID,RoleName,FamilyName,RoleLevel)->
    case RoleLevel<50 of
        true->
            ...
            Content = common_letter:create_temp(?FAMILY_ROLE_JOIN_LETTER,[RoleName, FamilyName]),
...
handle_info(Info, State) ->
    ...
    {noreply, State}.
handle_info_in_catch({_Unique, ?PLANT, _Method, _Record, _RoleID, _PID, _Line}=Info) ->
    mod_family_plant:do_handle_info(Info);
...
%% 地图争夺战的奖励
do_handle_info({add_money_when_first_of_warofcity, Money}) ->
    do_add_money(Money);

%% 商贸活动的奖励
do_handle_info({add_money_when_role_trading, Money}) ->
    do_add_money(Money);

%% NPC 兑换功能
do_handle_info({add_money_when_npc_exchange, Money}) ->
    do_add_money(Money);

do_handle_info({add_active_when_by_npc_exchang,ActivePoints})->
    do_add_active_points(ActivePoints);


do_handle_info({add_ac_when_first_of_warofcity, AC}) ->
    do_add_ac(AC);

do_handle_info({add_money_when_sec_of_warofcity, Money}) ->
    do_add_money(Money);

do_handle_info({add_ac_when_sec_of_warofcity, AC}) ->
    do_add_ac(AC);

do_handle_info({add_money_when_third_of_warofcity, Money}) ->
    do_add_money(Money);

do_handle_info({add_ac_when_third_of_warofcity, AC}) ->
    do_add_ac(AC);

do_handle_info({add_ac_when_join_warofcity, AC}) ->
    do_add_ac(AC);
...
do_handle_info(call_member_bonfire) ->
    %%召集帮众来玩火                    
    R = #m_family_callmember_toc{succ=true, message="门派篝火已经点燃，饮酒拿经验，快去参加吧！",call_type=?CALL_TYPE_FAMILY_BONFIRE},
    ...
    common_broadcast:bc_send_msg_family(FamilyID,?BC_MSG_TYPE_CHAT,?BC_MSG_TYPE_CHAT_FAMILY,"门派篝火已经点燃，饮酒拿经验，快去参加吧！"),
    common_misc:broadcast_to_line(OutsideMapMembers, ?FAMILY, ?FAMILY_CALLMEMBER, R);

%% 宗族捐献
do_handle_info({family_donate,RoleID,AddContribute,AddMoney})->
    do_add_money(AddMoney),
...
do_update_contribution_trans(RoleID,NewFmlConb)->
    TransFun = fun()->
                       ...
                       db:write(?DB_ROLE_ATTR, RoleAttr#p_role_attr{family_contribute=NewFmlConb}, write) 
               end,
...
do_member_gather_request(Unique,Module,?FAMILY_GATHERREQUEST,RoleID,Pid,Line)->
    State = get_state(),
    ExtInfo = State#family_state.ext_info,
    [EleRoleStatus] = db:dirty_read(?DB_ROLE_BASE,RoleID),
    ...
    TotalCondition =  
        ((not (StallState =:= true)) 
         ...
         andalso EleRoleStatus#p_role_base.status =/= ?ROLE_STATE_TRAINING
...
%% CALL_TYPE_BOSS，需要检查门派副本是够开启,并且boss未被打死
...
do_common_boss_dead(RoleIDList, RoleNum) ->
    ...
    common_hook_achievement:hook({family_module,{common_boss_dead,RoleIDList}}),
...
do_add_exp_to_members(MemberList,Level)->
    ...
    lists:foreach(fun(RoleID)->
                          case mod_family:is_today_not_parttake(RoleID,common_boss) of
                              true->
                                  [#p_role_attr{level = MemberLevel}|_] = db:dirty_read(?DB_ROLE_ATTR,RoleID),
                                  GainExp = MemberLevel*MemberLevel*ExpBase,
                                  ?DEBUG("common_boss_for_exp ~w",[{RoleID,GainExp}]),
                                  RealGainExp = hook_activity_family:hook_activity_expr(RoleID,GainExp),
                                  add_exp_for_role(RoleID,RealGainExp);
                              _ ->
                                  ignore
                          end
                  end,MemberList).
...
%% 1.门派频道广播  2。给自己的提示  3.召唤门派成员参与战斗
...
get_online_and_outside_map_members(State)->
    ...
    lists:foldl(fun(RoleID,Acc)->
                        ...
                        case (Status =:= ?ROLE_STATE_DEAD) orelse (Status=:=?ROLE_STATE_TRAINING) of
                            true->
                                Acc;
                            _ ->
...
family_channelbrd_and_gather_members({RoleID,BossType})->
    ...
    %% todo:需要判定成员此刻的位置,如果已经在门派地图内,则不发送该通知
    lists:foreach(fun(T)->
...
do_add_active_points(Num) ->
    ...
    NewFamilyInfo = FamilyInfo#p_family_info{active_points=AP+Num},
    NewState = State#family_state{family_info=NewFamilyInfo},
    update_state(NewState),
    R = #m_family_active_points_toc{new_points=AP+Num},
    broadcast_to_all_members(?FAMILY, ?FAMILY_ACTIVE_POINTS, R).
...
do_call_uplevelboss3(Unique,Module,Method,RoleID,Line,FamilyInfo)->
    ...
    case RemainAp >= 0  of
        ...
        false->
            ...
            Reason = lists:concat(["门派繁荣度不足",mod_family_misc:get_uplevel_activepoints(Level+1),",不能召唤"]),
            do_call_uplevelboss_error(Unique,Module,Method,Reason,RoleID,Line)
...
do_gm_uplevel(RoleID)->
    ...
    case FamilyInfo#p_family_info.level >= 6 of
        false ->
            do_uplevel2(Unique, Module, Method, RoleID, 6001);
        true ->
...
do_uplevel(Unique, Module, Method, RoleID, Line) ->
    %%获取当前门派，判断是否已经满级了
    %%判断升级门派的条件是否达到：是否打败了升级boss，是否满足门派繁荣度和门派资金
    State = get_state(),
    FamilyInfo = State#family_state.family_info,
    case FamilyInfo#p_family_info.level >= 6 of
        false ->
...
do_uplevel2(Unique, Module, Method, RoleID, Line) ->
    ...
    R = #m_family_uplevel_toc{new_level=NextLevel, money=NewMoney, active_points=NewActivePoints},
    RB = #m_family_uplevel_toc{return_self=false, new_level=Level+1, money=NewMoney, active_points=NewActivePoints},
    common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
    broadcast_to_all_members_except(Module, Method, RB, RoleID),
    %%门派升级成功之后世界中央广播 by natsuki
    
    %%恭喜 云州门派 [门派名字] 升级至 xx 级
    FactionName = common_misc:get_faction_name(FactionID),
    WorldBrdContent = lists:concat(["恭喜",common_tool:to_list(FactionName),"的门派 ",common_tool:to_list(FamilyName)," 升级至",Level+1,"级"]),
    common_broadcast:bc_send_msg_world(?BC_MSG_TYPE_CENTER,?BC_MSG_SUB_TYPE,WorldBrdContent),
    
    %%通知获得补偿的维护费用
    CompensateMsg = lists:concat(["获得升级门派后奖励的门派资金",common_tool:silver_to_string(CompensateMoney)
                                  ,",门派繁荣度",CompensateActivePt,"。"]),
    common_broadcast:bc_send_msg_role(RoleID,?BC_MSG_TYPE_SYSTEM,CompensateMsg).
...
do_reduce_role_gold_for_enble_map_succ(RoleID, RoleAttr) ->
    ...
    update_state(NewState),
    Content = lists:flatten(io_lib:format("~s 门派成功创建门派地图",[FamilyInfo#p_family_info.family_name])),
	...
	true ->
            case get_enable_map_request() of
                undefined ->           
                    ...
                    common_role_money:reduce(RoleID, [{gold_any, FamilyEnableMapGold,?CONSUME_TYPE_GOLD_ENABLE_MAP,""}], 
...
do_cancel_title2(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    ...
    case lists:keyfind(TargetRoleID, 2, OldMembers) of
        ...
        OldMemberInfo ->
            ...
            common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
            common_title:add_title(?TITLE_FAMILY,TargetRoleID,?DEFAULT_FAMILY_MEMBER_TITLE),
...
do_role_online(RoleID) ->
    ...
    ?TRY_CATCH( common_mod_goal:family_level_up([RoleID],(NewState#family_state.family_info)#p_family_info.level),FamilyInfoLevelError),
...
do_set_second_owner2(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    ...
    case erlang:length(OldSecondOwners) >= get_max_number_of_second_owner(FamilyInfo#p_family_info.level) of
        false ->
            ...
            common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
            common_title:add_title(?TITLE_FAMILY,TargetRoleID,?FAMILY_TITLE_SECOND_OWNER),
...
do_set_owner3(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    ...
    common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
    common_title:add_title(?TITLE_FAMILY,TargetRoleID,?FAMILY_TITLE_OWNER),
    common_title:add_title(?TITLE_FAMILY, RoleID, NewTitleForOldOwner),
...
do_set_title(Unique, Module, Method, Record, RoleID, Line) ->
    ...
    case lists:member(common_tool:to_list(Title), ForbidTitleList) of
        false ->
            State = get_state(),
            FamilyInfo = State#family_state.family_info,
            %%验证是否有权限，验证是否可以被设置称号
...
do_set_title2(Unique, Module, Method, {TargetRoleID, Title}, RoleID, Line) ->    
    ...
    case lists:keyfind(TargetRoleID, 2, Members) of
        ...
        MemberInfo ->
            ...
            update_state(NewState),
	        common_title:add_title(?TITLE_FAMILY,TargetRoleID,Title),
            %%广播
...
do_dismiss(Unique, Module, Method, RoleID, Line) ->
    %%判断是否是掌门，将来可能有更多的判断，例如门派站期间不能解散门派
    State = get_state(),
    #p_family_info{family_id=FamilyID, owner_role_id=OwnerRoleID,
                   faction_id=FactionID} = State#family_state.family_info,
    case OwnerRoleID =:= RoleID of
        true ->
             case common_warofking:is_family_join(FactionID, FamilyID) andalso common_warofking:is_begin_war(FactionID) of
                 true ->
                        ...
                        {atomic, _} ->
                            ...
                            lists:foreach(
                              fun(#p_family_member_info{role_id=MID}) ->
                                      common_title:remove_by_typeid(?TITLE_FAMILY,MID),
                                      hook_family_change:hook({MID, 0, FamilyID})
                              end, Members),
                            RoleIDList = get_member_role_id_list(),
                            lists:foreach(
                              fun(RID) ->
                                      notify_world_update({RID, cancel_family})
                              end, RoleIDList),
                            broadcast_to_all_members_except(Module, Method, RB, RoleID),
...
do_leave(Unique, Module, Method, RoleID, Line) ->
    %%判断是否是对应门派的成员，是否是掌门或者长老，更新玩家信息，广播通知族内所有人
    State = get_state(),
    FamilyInfo = State#family_state.family_info,
        ...
        false ->
            case db:transaction(fun() -> t_do_leave(RoleID, FamilyID) end) of
                {atomic, ok} ->
                    ...
                    NewState = State#family_state{family_info=NewFamilyInfo, family_members=NewMembers},
                    update_state(NewState),
                    mod_family:notify_world_update({RoleID, cancel_family}),
                    hook_family_change:hook({RoleID, 0, FamilyID}),
                    %%hook_family:hook_family_changed({0,RoleID}),
                    R = #m_family_leave_toc{role_id=RoleID},
                    common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
                    common_title:remove_by_typeid(?TITLE_FAMILY,RoleID),
                    %%更新门派贡献度清零
...
t_do_leave(RoleID, FamilyID) ->
    ...
    case RoleBase#p_role_base.family_id =:= FamilyID of
        true ->
            ...
            db:write(?DB_ROLE_EXT, NewRoleExt, write),
...
t_do_dismiss(FamilyID, Members) ->
    ...
    lists:foreach(
      fun(M) ->
              ...
              [RoleExt] = db:read(?DB_ROLE_EXT, RoleID, write),
              NewRoleExt = RoleExt#p_role_ext{family_last_op_time=common_tool:now()},
              ok = db:write(?DB_ROLE_EXT, NewRoleExt, write),
...
do_agree_f(Unique, Module, Method, Record, RoleID, Line) ->
    ...
    case FamilyInfo#p_family_info.cur_members >= get_max_member(FamilyInfo#p_family_info.level) of
        false ->  
             case common_warofking:is_family_join(FactionID, FamilyID) andalso common_warofking:is_begin_war(FactionID) of
                 true ->
                     do_agree_f_error(Unique, Module, Method, ?_LANG_FAMILY_CANNT_CHANGE_MEMBER_WHEN_JOIN_WAROFKING, RoleID, Line);
                 false ->
            ...
            case is_special_family_date() of
                true->
                    case can_join_family_in_special_date(RoleID) of
                        ...
                        _ ->
                            do_agree_f_error(Unique,Module,Method,?_LANG_JOIN_FAMILY_MAX_TIMES,RoleID,Line)
                    end;
                _ ->
                    do_agree_f3(Unique, Module, Method, {State, RequestList, FamilyInfo, TargetRoleID}, RoleID, Line)
            end;
...
do_agree_f3(Unique, Module, Method, {State, RequestList, FamilyInfo, TargetRoleID}, RoleID, Line) ->
   ...
   case db:transaction(fun() -> t_do_agree_f(TargetRoleID, FamilyID, FamilyName) end) of
	    ...
	    hook_family:hook_family_conbtribute_change(TargetRoleID,0),
            mod_family:notify_world_update({TargetRoleID, join_family,FamilyInfo#p_family_info.family_id,
                                            FamilyInfo#p_family_info.family_name,FamilyInfo#p_family_info.level}),
            ...
            ?DEBUG("~ts:~w ~w", ["同意玩家进入门派，即将触发门派信息改变的hook", RoleID, FamilyID]),
	    %%这是以前写的,后端统计用
            hook_family_change:hook({TargetRoleID, FamilyID, 0}),
	    %%这是role_attr新加了family_id字段,需要通知更新
           ...
           common_hook_achievement:hook({family_module,{agree,TargetRoleID,FamilyID,NewRoleBase}}),
        ...
        {aborted, Error} ->
            case erlang:is_binary(Error) of
                ...
                false ->
                    Reason = ?_LANG_SYSTEM_ERROR,
                    ?ERROR_MSG("~ts:~w", ["同意玩家加入门派申请失败", Error])
            end,
            %%删除该申请记录
            NewRequestList = lists:keydelete(TargetRoleID, 2, RequestList),
            NewState = State#family_state{requests=NewRequestList},
...
do_fire2(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    %%检查是不是门派成员
    State = get_state(),
    #p_family_info{family_id=FamilyID, faction_id=FactionID, ybc_status=YbcStatus} = State#family_state.family_info,
    case YbcStatus of
        ?FAMILY_YBC_STATUS_DOING ->
...
do_fire3(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    ...
    case db:transaction(fun() -> t_do_fire(TargetRoleID) end) of
        {atomic, TargetRoleName} ->
            ...
            NewState = State#family_state{family_members=NewMembers, family_info=NewFamilyInfo},
            FamilyInfo = State#family_state.family_info,
            notify_world_update({TargetRoleID, cancel_family}),
            update_state(NewState),
            FamilyID = FamilyInfo#p_family_info.family_id,
            hook_family_change:hook({TargetRoleID, 0, FamilyID}),
            common_title:remove_by_typeid(?TITLE_FAMILY,TargetRoleID),
            %% hook_family:hook_family_changed({0,TargetRoleID}),
...
send_message_to_expel_member(TargetRoleID,RoleID,FamilyName)->
    ?DEBUG("expel familymember ~w ",[TargetRoleID]),
    A = [#p_role_base{role_name = RoleName }|_] = db:dirty_read(?DB_ROLE_BASE,RoleID),
    ?DEBUG("expel a ~w",[A]),
    Content = common_letter:create_temp(?FAMILY_FIRE_MEMBER_LETTER,[common_tool:to_list(RoleName),FamilyName]),
    T = ?COMMON_FAMILY_LETTER(TargetRoleID,Content,"掌门给你的信",14),
    ?DEBUG("finalexpel ~w",[T]).
...
t_do_fire(TargetRoleID) ->
    [RoleBase] = db:read(?DB_ROLE_BASE, TargetRoleID),
    NewRoleBase = RoleBase#p_role_base{family_id=0, family_name=[]},
    db:write(?DB_ROLE_BASE, NewRoleBase, write),
    %%更新角色最后一次离开门派的时间
    [RoleExt] = db:read(?DB_ROLE_EXT, TargetRoleID),
    NewRoleExt = RoleExt#p_role_ext{family_last_op_time=common_tool:now()},
    db:write(?DB_ROLE_EXT, NewRoleExt, write),
...
do_agree2(Unique, Module, Method, FamilyID, RoleID, Line) ->
    ...
    case common_warofking:is_family_join(FactionID, FamilyID) andalso common_warofking:is_begin_war(FactionID) of
        ...
        false ->
            %%检查是否真的有邀请，检查对方门派是否已经满员，检查角色等级是否满足、检查是否已有门派
            case db:transaction(fun() -> t_do_agree(RoleID, FamilyID, FamilyName) end) of
                {atomic, RoleBase} ->            
                    ...
                    mod_family:notify_world_update({RoleID, join_family,NewFamilyInfo#p_family_info.family_id,
                                                    NewFamilyInfo#p_family_info.family_name,NewFamilyInfo#p_family_info.level}),
                    Invites = State#family_state.invites,
                ...
                {aborted, ErrorInfo} ->
                    ...
                    ?ERROR_MSG("~ts:~w", ["同意加入门派失败", ErrorInfo]),
                    do_agree_error(Unique, Module, Method, Reason, RoleID, Line)
            end
    end.
%%成功则返回该玩家的基本属性
t_do_agree(RoleID, FamilyID, FamilyName) ->
    %%检查是否真的有邀请，检查目标门派是否已经满员，检查是否已有门派
    [RoleBase] = db:read(?DB_ROLE_BASE, RoleID, write),
    case RoleBase#p_role_base.family_id > 0 of
        true ->
...
%%玩家申请加入某个门派
%%条件：等级、同国家、没有加入其他门派、上一次门派操作超过24小时
do_request(Unique, Module, Method, RoleID, Line) ->
    case catch do_request_check(RoleID) of
        ...
        {'EXIT', Error} ->
            ?ERROR_MSG("~ts:~w", ["检查申请加入门派条件时出错", Error]),
            Reason = ?_LANG_SYSTEM_ERROR,
    ...
    case db:transaction(fun() -> t_do_request(RoleID, FamilyID) end) of
        ...
        {aborted, Error} ->
            case erlang:is_binary(Error) of
                ...
                false ->
                    Reason = ?_LANG_SYSTEM_ERROR,
                    ?ERROR_MSG("~ts:~w", ["写入玩家门派申请失败", Error])
            end,
            do_request_error(Unique, Module, Method, Reason, RoleID, Line)
    end.
...
do_request_check_role(RoleID) ->
    ...
    case RoleAttr#p_role_attr.level >= MinLevelJoinFamily of
        true ->
            ok;
        false ->
            throw(?_LANG_FAMILY_LEVE_NOT_ENOUGH)
    end,
    %%检查是否已有门派
    case RoleBase#p_role_base.family_id > 0 of
        true ->
    ...
    case db:dirty_match_object(?DB_ROLE_BASE, #p_role_base{_='_', role_name=TargetRoleName2}) of
        ...
        [TRole] ->
            ...
            case catch do_invite_check(RoleID, TargetRoleID) of
                ...
                {'EXIT', Error} ->
                    ?ERROR_MSG("~ts:~w", ["检查邀请加入门派条件时出错", Error]),
                    Reason = ?_LANG_SYSTEM_ERROR,
                    do_invite_error(Unique, Module, Method, Reason, RoleID, Line)
            end
    end.
do_invite2(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
    ...
    case db:transaction(fun() -> t_do_invite(FamilyID, FamilyName, TargetRoleID, RoleID, SrcRoleName) end) of
        ...
        {aborted, Error} ->
            ?ERROR_MSG("~ts:~w", ["写入门派邀请玩家记录失败", Error]),
            Reason = ?_LANG_SYSTEM_ERROR,
            do_invite_error(Unique, Module, Method, Reason, RoleID, Line)
    end.
...
do_invite_check(RoleID, TargetRoleID) ->      
    ...
    case TargetRoleAttr#p_role_attr.level >= MinLevelJoinFamily of
        true ->
            ok;
        false ->
            throw(?_LANG_FAMILY_LEVE_NOT_ENOUGH_WHEN_INVITE)
    end,
    %%判断对方上次离开门派的时间
    case is_special_family_date() of
        true->
...
get_max_member(Level) ->
    [#r_family_config{max_member=V}] = common_config_dyn:find(family,Level),
    V.


%%门派每个等级最多可以设置多少长老
get_max_number_of_second_owner(Level) ->
    [#r_family_config{second_owners=V}] = common_config_dyn:find(family,Level),
    V.
...
%%将gen_server state中的门派信息保存到数据库中
do_dump_data() ->
    State = get_state(),
...
%%门派地图降级（就是门派降级），不需要递归计算，每天最多只降一级
...
get_last_login_time(RoleID)->
    Result = db:dirty_read(?DB_ROLE_EXT,RoleID),
    if length(Result =:= 1) ->
	    [#p_role_ext{last_login_time=LastLoginTime}] = Result,
	    LastLoginTime;
       true->
	    0
    end.
...
%% init_farm_for_family_map(FamilyID)->
%%     FarmList = get_default_farm_list(),
%%     R = #r_family_plant{family_id=FamilyID,farm_list=FarmList,max_farm_id=?DEFAULT_FARM_SIZE},
%%     db:dirty_write(?DB_FAMILY_PLANT,R).
%% 
%% %%获取默认门派的种植田地
%% get_default_farm_list()->
%%     %%默认10个已开垦的土地
%%     lists:foldl(fun(N,Acc)-> 
%%                         E = #p_map_farm{farm_id=N,status=?FARM_STATUS_NOT_SOW},
%%                         [E|Acc]
%%                 end, [], lists:seq(1, ?DEFAULT_FARM_SIZE)).
...
clear_old_protector() ->
    ...
    common_title:remove_by_typeid(?TITLE_FAMILY, Left),
    common_title:remove_by_typeid(?TITLE_FAMILY, Right),
    update_state(NewState). 
...
get_max_gongxun() ->
	...
	F = fun(XRoleID,BRoleID) -> 
		  {ok, #p_role_attr{gongxun=GongXun}} = common_misc:get_dirty_role_attr(XRoleID), 
		  {ok, #p_role_attr{gongxun=GongXun1}} = common_misc:get_dirty_role_attr(BRoleID), 
		  GongXun>GongXun1 
		end,
    ...
    case erlang:length(GongList)>=2 of
        true ->
           ...
           common_title:add_title(?TITLE_FAMILY,Left,?FAMILY_TITLE_LEFT_PROTECTOR),
           common_title:add_title(?TITLE_FAMILY,Right,?FAMILY_TITLE_RIGHT_PROTECTOR), 
...
do_set_Interior_Manager2(Unique, Module, Method, TargetRoleID, RoleID, Line) ->
	...
	case OleInterID>0 of
		true ->
			...
			common_title:add_title(?TITLE_FAMILY,OleInterID,?DEFAULT_FAMILY_MEMBER_TITLE);		
		false ->
    ...
    common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
    common_title:add_title(?TITLE_FAMILY,TargetRoleID,?FAMILY_TITLE_INTERIOR_MANAGER),
...
do_notify_online(Unique, Module, Method, _Record, RoleID, Line)->
    ...
    OnlineMembers = lists:foldl(
                   fun(M,NewList) ->
                           case M#p_family_member_info.online of
                               true ->
                                   [#p_online_info{memberid=M#p_family_member_info.role_id,otherattr=0}|NewList];
                               false ->
                                   NewList
                           end
                   end,[], Members),
