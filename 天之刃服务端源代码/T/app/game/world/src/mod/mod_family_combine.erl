...
-define(COMBINE_FAMILY_LETTER_TITLE,"并入门派 <font color=\"#3be450\">~s</font>的通知").
...
check_do_combine_request2(TeammateHasConfirm,CombineFamily,TargetFamily,FamilyInfo,TeammateFamilyInfo) ->
    ...
    IsFamilyJoin = common_warofking:is_family_join(CombineFactionID, CombineFamilyID),
    IsBeginWar = common_warofking:is_begin_war(CombineFactionID),
    if
        IsFamilyJoin =:= true andalso IsBeginWar =:= true ->
        ...
        {atomic, _} ->
            ...
            lists:foreach(
              fun(#p_family_member_info{role_id=RoleID,role_name=RoleName,role_level=RoleLevel}) ->
                      mod_family:notify_world_update({RoleID, join_family,TargetFamily#p_family_info.family_id,
                                                      TargetFamily#p_family_info.family_name,TargetFamily#p_family_info.level}),
                      common_title:remove_by_typeid(?TITLE_FAMILY,RoleID),
                      hook_family_change:hook({RoleID, 0, CombineFamilyID}),
                      hook_family_change:hook({RoleID, TargetFamilyID, CombineFamilyID}),
                      %% hook_family:hook_family_changed({TargetFamilyID,RoleID}),
                      common_title:add_title(?TITLE_FAMILY,RoleID,?DEFAULT_FAMILY_MEMBER_TITLE),
