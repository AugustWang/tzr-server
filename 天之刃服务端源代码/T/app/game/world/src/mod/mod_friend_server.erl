...
%% 玩家经验瓶
-define(role_exp_bottle,role_exp_bottle).
-define(persistent_exp_bottle_interval,300000).%% 5分钟持久化经验瓶
-define(role_exp_bottle_type_friend,1). %% 好友祝福经验类型
%%%===================================================================
%%% API
%%%===================================================================
...
%%上线提醒
online_notice(RoleID,Level) ->
    catch gen_server:cast({global, ?SERVER}, {online_notice, RoleID, Level}).

%%离线通知
offline_notice(RoleID,Level) ->
    catch gen_server:cast({global, ?SERVER}, {offline_notice, RoleID, Level}).
...
init([]) ->
    ...
    %% 随机种子
    random:seed(now()),
    erlang:send_after(?persistent_exp_bottle_interval,self(),{persistent_exp_bottle}),
    {ok, #friend_state{}}.
...
handle_cast(Msg, State) ->
    ...
    ?TRY_CATCH(do_handle_cast(Msg),ERROR1),
    {noreply, State}.
...
do_handle_info({Unique, Module, ?FRIEND_BOTTLE, DataIn, RoleID, PID, _Line}) ->
    do_friend_bottle(Unique, Module, ?FRIEND_BOTTLE, DataIn, RoleID, PID);
...
do_handle_info({persistent_exp_bottle}) ->
    do_persistent_exp_bottle();
...
do_handle_cast({online_notice, RoleID, Level}) ->
    do_online_notice(RoleID, Level);
do_handle_cast({offline_notice, RoleID, Level}) ->
    do_offline_notice(RoleID, Level);
...
do_add_friendly(RoleID, FriendID, Add, Type) ->
    try
        ...
        case Result of
            {ok, Friendly2} ->
                catch hook_friendly_change:hook({RoleID, FriendID, Add, Type}),
...
do_offline_notice(RoleID, Level) ->
    ...
    do_offline_role_exp_bottle(RoleID, Level).
...
do_upgrade_notice(RoleID, OldLevel, NewLevel) ->
    ...
    %% 好友祝福
    [CanCongratulaLevelList] = common_config_dyn:find(friend,can_congratula_level),
    case lists:member(NewLevel,CanCongratulaLevelList) of
        true ->
        ...
        FriendList ->
            try
                InfoList = 
                    lists:map(
                      fun(FriendInfo) ->
                              #r_friend{friendid=FriendID, type=Type, friendly=Friendly, relative=Rela} = FriendInfo,
                              get_friend_info(FriendID, Type, Friendly, Rela)
                      end, FriendList),
...
do_modify(Unique, Module, Method, DataIn, RoleID, Line) ->
    ...
    of
        {atomic, _} ->
            ...
            common_hook_achievement:hook({mod_friend_server,{modify_info,RoleID}}),
            %%广播给好友
            case get_in_friend_list(RoleID) of
                {error, _} ->
                    ok;
...
do_congratula2(Unique, Module, Method, RoleID, ToFriendID, Congra, PID) ->
    ...
    [#p_role_base{role_name=FriendName}] = db:dirty_read(?DB_ROLE_BASE, ToFriendID),
    FriendInfo = get_dirty_friend_info(RoleID, ToFriendID),
    #r_friend_base_info{add_exp = AddExp}= get_friend_base_info_by_friendly(FriendInfo#r_friend.friendly),
    %% 增加好友度
    ...
    [MaxCongratulaNumber] = common_config_dyn:find(friend,max_congratula_number),
    [AddExpRateCongratula] = common_config_dyn:find(friend,add_exp_rate_congratula),
    CurCongratulaNumber = get_role_exp_bottle_times(RoleID),
    ToCurCongratulaNumber = get_role_exp_bottle_times(ToFriendID),
    %% 前20个发送祝福的好友能获得经验
    case CurCongratulaNumber =< MaxCongratulaNumber of
        true ->
            ExpGet = common_tool:ceil(math:pow(FriendRoleLevel, 2) * (1 + AddExp/100) * 4);
        _ ->
            ExpGet = common_tool:ceil(math:pow(FriendRoleLevel, 2) * (1 + AddExp/100) * (AddExpRateCongratula / 100))
    end,
    case ToCurCongratulaNumber =< MaxCongratulaNumber of
        true ->
            ToExpGet = common_tool:ceil(math:pow(FriendRoleLevel, 2) * (1 + AddExp/100) * 4);
        _ ->
            ToExpGet = common_tool:ceil(math:pow(FriendRoleLevel, 2) * (1 + AddExp/100) * (AddExpRateCongratula / 100))
    end,
    BottleExp = add_role_exp_bottle_exp(RoleID,ExpGet,CurCongratulaNumber + 1),
    ToBottleExp = add_role_exp_bottle_exp(ToFriendID,ToExpGet,ToCurCongratulaNumber + 1),
    %% 更新祝福列表
    put({?DICT_FRIEND_CONGRATULA, ToFriendID}, {FriendRoleLevel, [RoleID|CongratulaList]}),
    ReturnSelf = #m_friend_congratulation_toc{exp_add=ExpGet, from_friend=FriendName, hyd_add=AddCongratulaFriendly,
                                              bottle_exp = BottleExp},
    ToFriend = #m_friend_congratulation_toc{return_self=false, exp_add=ToExpGet, hyd_add=AddCongratulaFriendly,
                                            from_friend=RoleName, congratulation=Congra,bottle_exp = ToBottleExp},
...
do_friend_bottle(Unique, Module, Method, DataIn, RoleId, PId) ->
    case catch do_friend_bottle2(RoleId,DataIn) of
        {error,Reason,ReasonCode} ->
            do_friend_bottle_error(Unique, Module, Method, DataIn, RoleId, PId, Reason, ReasonCode);
        {ok,do_query,ExpBottleInfo} ->
            DataRecord = #m_friend_bottle_toc{succ= true,
                                              op_type = DataIn#m_friend_bottle_tos.op_type,
                                              bottle_exp = ExpBottleInfo#r_role_exp_bottle.cur_exp,
                                              award_exp = 0},
            common_misc:unicast2(PId, Unique, Module, Method, DataRecord);
        {ok,ExpBottleInfo} ->
            do_friend_bottle3(Unique, Module, Method, DataIn, RoleId, PId, ExpBottleInfo)
    end.
do_friend_bottle2(RoleId,DataIn) ->
    #m_friend_bottle_tos{op_type = OpType} = DataIn,
    case OpType =:= 1001 andalso OpType =:= 1002 of
        true ->
            erlang:throw({error,?_LANG_FRIEND_BOTTLE_OP_TYPE,0});
        _ ->
            next
    end,
    ExpBottleInfo = 
        case get_role_exp_bottle(RoleId) of
            {ok,ExpBottleInfoT} ->
                ExpBottleInfoT;
            _ ->
                erlang:throw({error,?_LANG_FRIEND_BOTTLE_EXP_NULL,0})
        end,
    case OpType =:= 1001 of
        true ->
            erlang:throw({ok,do_query,ExpBottleInfo});
        _ ->
            next
    end,
    case OpType =:= 1002 of
        true ->
            [MinAwardExpBottleLevel] = common_config_dyn:find(friend,min_award_exp_bottle_level),
            {ok, #p_role_attr{level=RoleLevel}} = common_misc:get_dirty_role_attr(RoleId),
            case RoleLevel >= MinAwardExpBottleLevel of
                true ->
                    next;
                _ ->
                    erlang:throw({error,?_LANG_FRIEND_BOTTLE_MIN_LEVEL,0})
            end,
            next;
        _ ->
            next
    end,
    case ExpBottleInfo#r_role_exp_bottle.cur_exp =< 0 of
        true ->
            erlang:throw({error,?_LANG_FRIEND_BOTTLE_EXP_NULL,0});
        _ ->
            next
    ...
    {ok,ExpBottleInfo}.
do_friend_bottle3(Unique, Module, Method, DataIn, RoleId, PId, ExpBottleInfo) ->
    ?TRY_CATCH(common_misc:add_exp_unicast(RoleId, ExpBottleInfo#r_role_exp_bottle.cur_exp),AddExpByBottleError),
    DataRecord = #m_friend_bottle_toc{succ= true,
                                      op_type = DataIn#m_friend_bottle_tos.op_type,
                                      bottle_exp = 0,
                                      award_exp = ExpBottleInfo#r_role_exp_bottle.cur_exp},
    set_role_exp_bottle(RoleId, ExpBottleInfo#r_role_exp_bottle{cur_exp = 0}),
    common_misc:unicast2(PId, Unique, Module, Method, DataRecord),
    ok.

do_friend_bottle_error(Unique, Module, Method, DataIn, _RoleId, PId, Reason, ReasonCode) ->
    DataRecord = #m_friend_bottle_toc{succ=false,
                                      op_type = DataIn#m_friend_bottle_tos.op_type,
                                      reason=Reason, 
...
t_do_modify(RoleID, RoleExt) ->
    [OldRoleExt] = db:read(?DB_ROLE_EXT, RoleID, write),
...
get_friend_info(FriendID, FriendType, Friendly, Rela) ->
    try
        [FriendBase] = db:dirty_read(?DB_ROLE_BASE, FriendID),
        [FriendAttr] = db:dirty_read(?DB_ROLE_ATTR, FriendID),
        [FriendExt] = db:dirty_read(?DB_ROLE_EXT, FriendID),
        #p_friend_info{
                        roleid=FriendID,
                ...
                _ ->
                    case if_select(30) of
                        true ->
                            ...
                            #p_role_attr{role_name=TRoleName, level=TLevel, category=Category} = TRoleAttr,
                            
                            RecommentList2 = [#p_recommend_member_info{role_id=TRoleID, 
                                                                       role_name=TRoleName, 
                                                                       level=TLevel,
                                                                       category = Category}|RecommentList],
...
friendly_add_notify(RoleID, FriendID, Friendly, Friendly2) ->
    ...
    case FriendLevel =:= FriendLevel2 of
        true ->
            ignore;
        _ ->
            {ok, {SelfLetter, FriendLetter}, {SelfMsg, FriendMsg}} = get_notify_letter_and_msg(RoleID, Friendly2, FriendID, Desc, AttackAdd),
...
%% @doc 获取信件及提示信息
get_notify_letter_and_msg(RoleID, Friendly, FriendID, Desc, AttackAdd) ->
    ...
    case Friendly of
        0 ->
            {ok,
             {common_letter:create_temp(?FRIEND_ZERO_LEVEL_CHANGE_LETTER,[RoleName, FriendName, Desc]),
              common_letter:create_temp(?FRIEND_ZERO_LEVEL_CHANGE_LETTER,[FriendName, RoleName, Desc])},
...
%% -define(role_exp_bottle,role_exp_bottle).
%% ========= 经验瓶 ===========
get_role_exp_bottle(RoleId) ->
    case erlang:get({?role_exp_bottle,RoleId}) of
        undefined ->
            {error,not_found};
        ExpBottleInfo ->
            {ok,ExpBottleInfo}
    end.
set_role_exp_bottle(RoleId,ExpBottleInfo) ->
    erlang:put({?role_exp_bottle,RoleId}, ExpBottleInfo).

%% 玩家下线
do_offline_role_exp_bottle(RoleId, _Level) ->
    case get_role_exp_bottle(RoleId) of
        {ok,ExpBottleInfo} ->
            db:dirty_write(?DB_ROLE_EXP_BOTTLE_P, ExpBottleInfo);
        _ ->
            ignore
    end.
%% 玩家上线
do_online_role_exp_bottle(RoleId, _Level) ->
    case get_role_exp_bottle(RoleId) of
        {ok,ExpBottleInfo} ->
            ignore;
        _ ->
            case db:dirty_read(?DB_ROLE_EXP_BOTTLE_P, {RoleId,?role_exp_bottle_type_friend}) of
                [] ->
                    ExpBottleInfo = #r_role_exp_bottle{key = {RoleId,?role_exp_bottle_type_friend}, 
                                                       role_id = RoleId,
                                                       total_exp = 0,
                                                       cur_exp = 0,
                                                       times = 0,
    ...
    case ExpBottleInfo#r_role_exp_bottle.cur_exp > 0 of
        true -> %% 通知经验瓶
            DataRecord = #m_friend_bottle_toc{op_type = 1001,succ = true,
                                              bottle_exp = ExpBottleInfo#r_role_exp_bottle.cur_exp,
                                              award_exp = 0},
            common_misc:unicast({role,RoleId}, ?DEFAULT_UNIQUE, ?FRIEND, ?FRIEND_BOTTLE, DataRecord);
        _ ->
            ignore
    end, 
    ok.
...
do_persistent_exp_bottle() ->
    erlang:send_after(?persistent_exp_bottle_interval,self(),{persistent_exp_bottle}),
    RoleIdList = db:dirty_all_keys(?DB_USER_ONLINE),
    case RoleIdList =:= [] of
        true ->
            ignore;
        _ ->
           do_persistent_exp_bottle2(RoleIdList)
    end.
do_persistent_exp_bottle2(RoleIdList) ->
    lists:foreach(
      fun(RoleId) ->
    ...
    ok.
%% 获得玩家好友经验瓶次数
get_role_exp_bottle_times(RoleId) ->
    {NowDate,_NowTime} = common_tool:seconds_to_datetime(common_tool:now()),
    TodaySeconds = common_tool:datetime_to_seconds({NowDate,{0,0,0}}),
    case get_role_exp_bottle(RoleId) of
        {ok,ExpBottleInfo} ->
            case ExpBottleInfo#r_role_exp_bottle.op_time > TodaySeconds of
                true ->
                    ExpBottleInfo#r_role_exp_bottle.times;
                _ ->
                    0
            end;
        _ ->
...
%% 添加玩家经验瓶经验
%% 返回可获取的经验瓶经验
add_role_exp_bottle_exp(RoleId,AddExp,NewTimes) ->
    case get_role_exp_bottle(RoleId) of
        {ok,ExpBottleInfoT} ->
            ExpBottleInfo = ExpBottleInfoT#r_role_exp_bottle{total_exp = ExpBottleInfoT#r_role_exp_bottle.total_exp + AddExp,
                                                             cur_exp = ExpBottleInfoT#r_role_exp_bottle.cur_exp + AddExp, 
                                                             times = NewTimes, 
    ...
    set_role_exp_bottle(RoleId,ExpBottleInfo),
    ExpBottleInfo#r_role_exp_bottle.cur_exp.