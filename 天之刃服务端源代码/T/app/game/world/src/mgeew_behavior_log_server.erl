...
-export([log_enter_flash_window/2,
         log_accept_first_task/2
         ]).
...
-define(USER_ENTER_FLASH_WINDOW_LIST, user_enter_flash_window_list).
-define(USER_ACCEPT_FIRST_TASK_LIST,user_accept_first_task_list).
...
%%@doc 角色进入flash角色页窗口统计
log_enter_flash_window(RoleID,LoginIP)->
    global:send(?MODULE,{enter_flash_window,RoleID,LoginIP}),
    ok.

%%@doc 陈圆圆任务统计
log_accept_first_task(RoleID,LoginIP)->
    global:send(?MODULE,{accept_first_task,RoleID,LoginIP}),
    ok.
...
do_handle_info({accept_first_task,RoleID,LoginIP})->
    case get(?USER_ACCEPT_FIRST_TASK_LIST) of 
        Lists when is_list(Lists) ->
            case lists:member(RoleID,Lists) of
                ...
                false->
                    ...
                    put(?USER_ACCEPT_FIRST_TASK_LIST,[RoleID|Lists]),
                    do_log_record(Rec)
            end;
        _->
            ?ERROR_MSG("USER_ACCEPT_FIRST_TASK_LIST",[]),
            ignore
    end;

%%@doc 角色进入flash角色页窗口统计
do_handle_info({enter_flash_window,RoleID,LoginIP})->
    case get(?USER_ENTER_FLASH_WINDOW_LIST) of
        Lists when is_list(Lists) ->
            case lists:member(RoleID,Lists) of
                true->
                    ignore;
                false->
                    ...
                    put(?USER_ENTER_FLASH_WINDOW_LIST,[RoleID|Lists]),
                    do_log_record(Rec)
            end;
        _->
            ?ERROR_MSG("USER_ENTER_FLASH_WINDOW_LIST is error",[]),
            ignore
    end;
...
do_dump_behavior_logs(Queues)->
    try
        ...
        mod_mysql:batch_insert(t_log_behavior,FieldNames,QueuesInsert,3000),
...
do_load_history_data()->
    ResultSet1 = mod_mysql:select("select  distinct `role_id` from  t_log_behavior where behavior_type = 1"),
    {ok,List1} = ResultSet1, 
    
    put(?USER_ENTER_FLASH_WINDOW_LIST,[ RoleID ||[RoleID]<-List1 ]),
    
    TaskResultSet = mod_mysql:select("select distinct `role_id` from t_log_behavior where behavior_type = 2"),
    {ok,TaskList} = TaskResultSet,
    
    put(?USER_ACCEPT_FIRST_TASK_LIST,[ RoleID ||[RoleID]<-TaskList ]).
