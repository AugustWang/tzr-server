...
%%
init(RankInfo) ->
    init2(RankInfo,?MODULE,?DB_EQUIP_REFINING_RANK,fun(E1,E2) -> cmp(E1,E2) end).
...
rank() ->
    rank2(?MODULE,?DB_EQUIP_REFINING_RANK,fun(E1,E2) -> cmp(E1,E2) end).
...
update({GoodsInfo,RoleID}) ->
    update2({GoodsInfo,RoleID},?MODULE,?DB_EQUIP_REFINING_RANK,fun(E1,E2) -> cmp(E1,E2) end).
...
get_role_rank(RoleID) ->
    ?DEBUG("~w",[RoleID]),
    case db:dirty_match_object(?DB_EQUIP_REFINING_RANK,#p_equip_rank{role_id = RoleID, _ = '_'}) of
        [] ->
            undefined;
        EquipRankList ->
