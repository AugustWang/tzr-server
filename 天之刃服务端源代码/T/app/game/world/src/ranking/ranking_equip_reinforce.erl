...
%%
init(RankInfo) ->
    ranking_equip_refining:init2(RankInfo,?MODULE,?DB_EQUIP_REINFORCE_RANK,fun(E1,E2) -> cmp(E1,E2) end).
 


rank() ->
    ranking_equip_refining:rank2(?MODULE,?DB_EQUIP_REINFORCE_RANK,fun(E1,E2) -> cmp(E1,E2) end).


update({GoodsInfo,RoleID}) ->
    ranking_equip_refining:update2({GoodsInfo,RoleID},?MODULE,?DB_EQUIP_REINFORCE_RANK,fun(E1,E2) -> cmp(E1,E2) end).
...
get_role_rank(RoleID) ->
    case db:dirty_match_object(?DB_EQUIP_REINFORCE_RANK,#p_equip_rank{role_id = RoleID,_='_'}) of
        [] ->
            undefined;
        EquipRankList ->
            lists:foldr( 
