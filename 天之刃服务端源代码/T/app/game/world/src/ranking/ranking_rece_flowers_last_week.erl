...
init(_RankInfo) ->
    case db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,#p_role_rece_flowers_last_week_rank{_ = '_'}) of
        [] ->
...
rank2() ->
    case db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_THIS_WEEK_RANK,#p_role_rece_flowers_this_week_rank{_ = '_'}) of
	[]->
	    put({?MODULE,ranking_info_list},[]),
		OldLastWeekRoleRankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,#p_role_rece_flowers_last_week_rank{ _ = '_'}),
        lists:foreach(fun(OldLastWeekRank) -> db:dirty_delete_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,OldLastWeekRank) end, OldLastWeekRoleRankList);
	ThisWeekRankList ->
        LastWeekRankList= this_week_to_last_week_rank(ThisWeekRankList),
        {ok,ReceList} = do_rank(LastWeekRankList),
		lists:foreach(fun(OldThisWeekRank) -> db:dirty_delete_object(?DB_ROLE_RECE_FLOWERS_THIS_WEEK_RANK,OldThisWeekRank) end, ThisWeekRankList),
        put({?MODULE,ranking_info_list},ReceList),
		OldLastWeekRoleRankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,#p_role_rece_flowers_last_week_rank{ _ = '_'}),
		lists:foreach(fun(OldLastWeekRank) -> db:dirty_delete_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,OldLastWeekRank) end, OldLastWeekRoleRankList),
        lists:foreach(fun(ReceFlowers)-> db:dirty_write(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,ReceFlowers) end,ReceList)
    end.
...
update(reset) ->
	RankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_LAST_WEEK_RANK,#p_role_rece_flowers_last_week_rank{ _ = '_'}),
