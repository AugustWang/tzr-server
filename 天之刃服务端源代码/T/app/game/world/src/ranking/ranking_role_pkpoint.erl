...
update(RolePkPointRank) when is_record(RolePkPointRank,p_role_pkpoint_rank) ->
    RoleID = RolePkPointRank#p_role_pkpoint_rank.role_id,
    FactionID = RolePkPointRank#p_role_pkpoint_rank.faction_id,
    ranking_minheap:update_heap(RolePkPointRank,RoleID,?DB_ROLE_PKPOINT_RANK,?MODULE,{?MODULE,FactionID});
update(Info)->
    ?ERROR_MSG("ranking role pkpoint unknow info:~w",[Info]).
...
get_role_rank(RoleID) ->
    case db:dirty_read(?DB_ROLE_PKPOINT_RANK,RoleID) of
        [] ->
            undefined;
        [RoleLevelRank] ->
            #p_role_pkpoint_rank{pk_points = PKPoint,ranking = Rank} = RoleLevelRank,
            #p_role_all_rank{ key_value=PKPoint, ranking = Rank}
    end.
...
read_rank_data(RankSize,FactionID) ->
    ...
    case db:dirty_match_object(?DB_ROLE_BASE,#p_role_base{faction_id = FactionID, _ = '_'}) of
        [] ->
            nil;
        List -> 
            lists:foreach(fun(RoleBase) -> 
                                  case RoleBase#p_role_base.pk_points>18 of
                                      true->
                                        RolePKPointRank = common_ranking:get_pkpoint_rank_record(RoleBase),
                                        update(RolePKPointRank);
                                      false->
                                          ignore
                                  end
                          end,List)
...
init2(RankSize,FactionID) ->
    case db:dirty_match_object(?DB_ROLE_PKPOINT_RANK,#p_role_pkpoint_rank{faction_id = FactionID, _ = '_'}) of
        [] ->
            read_rank_data(RankSize,FactionID);
        RoleRankList ->
            rank2(FactionID),
...
rank2(FactionID) ->
    List = db:dirty_match_object(?DB_ROLE_PKPOINT_RANK,#p_role_pkpoint_rank{faction_id = FactionID, _ = '_'}),
    ...
    List2 = lists:foldr(
              fun(RolePkPointRank,Acc) ->
                      ...
                      case NewPkPoint >= 18 of
                          ...
                          false ->
                              db:dirty_delete_object(?DB_ROLE_PKPOINT_RANK,RolePkPointRank),
                              Acc
                      end
              end,[],TmpList),
    ?DEBUG("~w",[List2]),
    ...
    {_,List4} = lists:foldl(
              fun(RolePkPointRank,{Rank,Acc}) ->
                      ...
                      db:dirty_write(?DB_ROLE_PKPOINT_RANK,NewRolePkPointRank),
