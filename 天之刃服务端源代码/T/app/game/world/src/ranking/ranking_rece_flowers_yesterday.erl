...
init(_RankInfo) ->
    case db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_YESTERDAY_RANK,
...
rank() ->
    ...
    case (H =:= 0 andalso M < 5)  of
        true ->
			OldReceYesterDayRankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_YESTERDAY_RANK,#p_role_rece_flowers_yesterday_rank{ _ = '_'}),
            lists:foreach(fun(OldReceYesterDayRank) -> db:dirty_delete_object(?DB_ROLE_RECE_FLOWERS_YESTERDAY_RANK,OldReceYesterDayRank) end, OldReceYesterDayRankList),
	    ...
	    {ok,NewList} = do_rank(NewListTmp),
			...
			OldReceToDayRankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_TODAY_RANK,#p_role_rece_flowers_today_rank{ _ = '_'}),
            lists:foreach(fun(OldReceToDayRank) -> db:dirty_delete_object(?DB_ROLE_RECE_FLOWERS_TODAY_RANK,OldReceToDayRank) end, OldReceToDayRankList),
            put({ranking_rece_flowers_today,ranking_info_list},[]),
            common_title:add_title(?TITLE_ROLE_RECE_FLOWERS_YESTERDAY,0,NewList);
        false ->
            ignore
    end.
    ...
    {_,RoleReceRankxList3} =
        lists:foldl(
          fun(ReceRank,{Rank,Acc}) ->
                  {Rank-1,[ReceRank#p_role_rece_flowers_yesterday_rank{ranking = Rank,title=case Rank of 1 -> "鲜花宝贝";_ -> "" end }|Acc]}
...
update(reset) ->
	RankList = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_YESTERDAY_RANK,#p_role_rece_flowers_yesterday_rank{ _ = '_'}),
...
today_to_this_week_rank(List)when is_list(List) ->
    lists:foreach(
      fun(ToDayRank) ->
              ...
              case db:dirty_read(?DB_ROLE_RECE_FLOWERS_THIS_WEEK_RANK,RoleID) of
	    		[]->
				NewCharm = Charm;
			[ThisWeekRoleRece]->
		...
		ThisWeekRank =
	      ...
	      db:dirty_write(?DB_ROLE_RECE_FLOWERS_THIS_WEEK_RANK,ThisWeekRank)
      ...
      fun(ToDayRank) ->
              ...
              YesterDayRank = 
                  #p_role_rece_flowers_yesterday_rank{ranking= Ranking,
                                                      ...
                                                      family_name = FamilyName,                    
                                                      faction_id = FactionID,                    
                                                      title = case Ranking of 1 -> "鲜花宝贝";_ -> "" end},
              db:dirty_write(?DB_ROLE_RECE_FLOWERS_YESTERDAY_RANK,YesterDayRank),
              YesterDayRank
      end,List);
