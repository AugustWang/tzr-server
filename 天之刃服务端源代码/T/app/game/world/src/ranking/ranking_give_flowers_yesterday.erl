...
init(_RankInfo) ->
    case db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_YESTERDAY_RANK,
    ...
    {_,RoleGiveRankxList3} =
        lists:foldl(
          fun(ReceRank,{Rank,Acc}) ->
                 {Rank-1,[ReceRank#p_role_give_flowers_yesterday_rank{ranking = Rank,title=case Rank of 1 -> "护花使者";_ -> "" end}|Acc]}
...
rank() ->
    ...
    case (H =:= 0 andalso M < 5)  of
        true ->
            OldGiveYesterDayRankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_YESTERDAY_RANK,#p_role_give_flowers_yesterday_rank{ _ = '_'}),
            lists:foreach(fun(OldGiveYesterDayRank) -> db:dirty_delete_object(?DB_ROLE_GIVE_FLOWERS_YESTERDAY_RANK,OldGiveYesterDayRank) end, OldGiveYesterDayRankList),
	    ...
	    %% end                                    
            ...
            OldGiveToDayRankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_TODAY_RANK,#p_role_give_flowers_today_rank{ _ = '_'}),
            lists:foreach(fun(OldGiveToDayRank) -> db:dirty_delete_object(?DB_ROLE_GIVE_FLOWERS_TODAY_RANK,OldGiveToDayRank) end, OldGiveToDayRankList),
            put({ranking_give_flowers_today,ranking_info_list},[]),
            common_title:add_title(?TITLE_ROLE_GIVE_FLOWERS_YESTERDAY,0,NewList);
        false ->
            ignore
    end.

update(reset) ->
	RankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_YESTERDAY_RANK,#p_role_give_flowers_yesterday_rank{ _ = '_'}),
...
today_to_this_week_rank(List)when is_list(List) ->                                      
    lists:foreach(                                                                          
      fun(ToDayRank) ->                                                                 
              ...
              case db:dirty_read(?DB_ROLE_GIVE_FLOWERS_THIS_WEEK_RANK,RoleID) of        
                        []->                                                            
                                NewScore = Score;                                       
                        [ThisWeekRoleGive]->   
              ...
              db:dirty_write(?DB_ROLE_GIVE_FLOWERS_THIS_WEEK_RANK,ThisWeekRank)         
      ...
      fun(ToDayRank) ->
              ...
              YesterDayRank = 
                  #p_role_give_flowers_yesterday_rank{ranking= Ranking,
                                                      ...
                                                      family_name = FamilyName,                    
                                                      faction_id = FactionID,                    
                                                      title = case Ranking of 1 -> "护花使者";_ -> "" end},
              db:dirty_write(?DB_ROLE_GIVE_FLOWERS_YESTERDAY_RANK,YesterDayRank),
              YesterDayRank
      end,List);
