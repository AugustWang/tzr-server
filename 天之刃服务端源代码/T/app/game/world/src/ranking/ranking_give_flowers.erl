...
init(RankInfo) ->
    ...
    case db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_RANK,#p_role_give_flowers_rank{_='_'}) of
        []->
            init_rank_data(RankSize);
        RoleRankList1 ->
            RoleRankList2 = rank2(RoleRankList1),
            common_title:add_title(?TITLE_ROLE_GIVE_FLOWERS,0,RoleRankList2),
            NewList = lists:foldr(
                        fun(RoleRank2,Acc2)->
...
rank()->
    List1 = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_RANK,#p_role_give_flowers_rank{_ = '_'}),
    List2 = rank2(List1),
    common_title:add_title(?TITLE_ROLE_GIVE_FLOWERS,0,List2).        

%% update({RoleBase,RoleAttr,Score})
%%   when RoleBase#p_role_base.sex =:= 1 andalso Score>0 ->
...
%%     ranking_minheap:update_heap(RoleRank,RoleID,?DB_ROLE_GIVE_FLOWERS_RANK,?MODULE,?MODULE);
%% 
%% %% 变性后删除数据
%% update({RoleBase,_RoleAttr,_Score})->
%%     ranking_minheap:delete(?MODULE,?MODULE,RoleBase#p_role_base.role_id),
%%     db:dirty_delete(?DB_ROLE_GIVE_FLOWERS_RANK,RoleBase#p_role_base.role_id);
%% update(_)->
%%     ignore.

update({update,RoleGiveFlowersRank})->
    ranking_minheap:update_heap(RoleGiveFlowersRank,RoleGiveFlowersRank#p_role_give_flowers_rank.role_id,?DB_ROLE_GIVE_FLOWERS_RANK,?MODULE,?MODULE);
update({clear,RoleID})->
    ranking_minheap:delete(?MODULE,?MODULE,RoleID),
    db:dirty_delete(?DB_ROLE_GIVE_FLOWERS_RANK,RoleID);
update(_)->
    ignore.
...
get_role_rank(RoleID) ->
    ...
    case lists:keyfind(RoleID,#p_role_give_flowers_rank.role_id,RoleRankList) of
        ...
        GiveRank ->
            ...
            #p_role_all_rank{rank_name= <<"送花谱">>, key_name= <<"送花得分">>, key_value=Score, ranking = Rank}
    end.
%%
%%====================== Local Functions =======================
%% 
        ...
        List->
            lists:foreach(
              fun(RoleBase) ->
                      ...
                      case RoleGiveInfo#r_give_flowers.score> 0 of
                          true->
                            {ok,RoleAttr} = common_misc:get_dirty_role_attr(RoleBase#p_role_base.role_id),
                            RoleGiveFlowersRank=common_ranking:get_give_flowers_rank_record(RoleBase, RoleAttr, RoleGiveInfo#r_give_flowers.score),
                            update({update,RoleGiveFlowersRank});
                          false->
                              ignore
                      end
