...
update({add,RoleGongXunRank})->
    RoleID = RoleGongXunRank#p_role_gongxun_rank.role_id,
    FactionID = RoleGongXunRank#p_role_gongxun_rank.faction_id,
    case ranking_minheap:key_find({?MODULE,FactionID},RoleID) of
        undefined ->
            ranking_minheap:update_heap(RoleGongXunRank,RoleID,?DB_ROLE_TODAY_GONGXUN_RANK,?MODULE,{?MODULE,FactionID});
        {Rank,_}->
             OldGongXun = Rank#p_role_gongxun_rank.gongxun,
             AddGongXun = RoleGongXunRank#p_role_gongxun_rank.gongxun,
             ranking_minheap:update_heap(RoleGongXunRank#p_role_gongxun_rank{gongxun=OldGongXun+AddGongXun},RoleID,?DB_ROLE_TODAY_GONGXUN_RANK,?MODULE,{?MODULE,FactionID})
    end;
update({reduce,RoleID,FactionID,Change})->
    case ranking_minheap:key_find({?MODULE,FactionID},RoleID) of
        undefined ->
            ignore;
        {Rank,_} ->
            OldGongXun = Rank#p_role_gongxun_rank.gongxun,
            ranking_minheap:update_heap(Rank#p_role_gongxun_rank{gongxun=OldGongXun-Change},RoleID,?DB_ROLE_TODAY_GONGXUN_RANK,?MODULE,{?MODULE,FactionID})
    end;
update(_)->
    ignore.

%% update({add,RoleAttr,Change}) ->
%%    % ?ERROR_MSG("rolettr = ~w",[RoleAttr]),
%%     #p_role_attr{
%%                  role_id = RoleID,
%%                  level = Level,
%%                  exp = Exp,
...
%%                     ranking_minheap:update_heap(RoleLevelRank,RoleID,?DB_ROLE_TODAY_GONGXUN_RANK,?MODULE,{?MODULE,FactionID});
%%                 {Rank,_} ->
%%                    % ?ERROR_MSG("~w",[Rank]),
%%                     OldGongXun = Rank#p_role_gongxun_rank.gongxun,
...
get_role_rank(RoleID) ->
    case db:dirty_read(?DB_ROLE_TODAY_GONGXUN_RANK,RoleID) of
        [] ->
            undefined;
        [RoleLevelRank] ->
            #p_role_gongxun_rank{gongxun = GongXun,ranking = Rank} = RoleLevelRank,
            #p_role_all_rank{rank_name= <<"今日战功榜">>, key_name= <<"战功">>, key_value=GongXun, ranking = Rank}
    end.
...
init2(RankSize,FactionID) ->
    RoleRankList = db:dirty_match_object(?DB_ROLE_TODAY_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}),
    rank2(FactionID),
    NewList = lists:foldr(
...
rank2(FactionID) ->
    List = db:dirty_match_object(?DB_ROLE_TODAY_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}),
    List2 = lists:foldr(
              fun(RoleGongXunRank,Acc) ->
                      ...
                      case GongXun > 0 of
                          true ->
                              [RoleGongXunRank|Acc];
                          false ->
                              db:dirty_delete_object(?DB_ROLE_TODAY_GONGXUN_RANK,RoleGongXunRank),
                              Acc 
                      end
              end,[],List),
    ...
    case (H =:= 23 andalso M > 55) orelse (H =:= 0 andalso M < 5)  of
        true ->
            ...
            lists:foreach(fun(Obj) -> db:dirty_delete_object(?DB_ROLE_TODAY_GONGXUN_RANK,Obj) end,List),
        ...
        false ->
            {_,List4} = lists:foldl(
                          fun(RoleGongXunRank,{Rank,Acc}) ->
                                  NewRoleGongXunRank = RoleGongXunRank#p_role_gongxun_rank{ranking = Rank},
                                  db:dirty_write(?DB_ROLE_TODAY_GONGXUN_RANK,NewRoleGongXunRank),
                                  {Rank-1,[NewRoleGongXunRank|Acc]}
                          end,{length(List3),[]},List3),
