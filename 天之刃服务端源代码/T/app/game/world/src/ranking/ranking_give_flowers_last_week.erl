...
init(_RankInfo) ->
    case db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,#p_role_give_flowers_last_week_rank{_ = '_'}) of
        [] ->
...
rank2() ->
    case db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_THIS_WEEK_RANK,#p_role_give_flowers_this_week_rank{_ = '_'}) of
        []->
            put({?MODULE,ranking_info_list},[]),
			OldLastWeekRoleRankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,#p_role_give_flowers_last_week_rank{ _ = '_'}),
        	lists:foreach(fun(OldLastWeekRank) -> db:dirty_delete_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,OldLastWeekRank) end, OldLastWeekRoleRankList);
        ThisWeekRankList ->
            LastWeekRankList=this_week_to_last_week_rank(ThisWeekRankList),
            {ok,GiveList} = do_rank(LastWeekRankList),
            lists:foreach(fun(OldThisWeekRank) -> db:dirty_delete_object(?DB_ROLE_GIVE_FLOWERS_THIS_WEEK_RANK,OldThisWeekRank) end, ThisWeekRankList),
            put({?MODULE,ranking_info_list},GiveList),   
            OldLastWeekRoleRankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,#p_role_give_flowers_last_week_rank{ _ = '_'}),
			lists:foreach(fun(OldLastWeekRank) -> db:dirty_delete_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,OldLastWeekRank) end, OldLastWeekRoleRankList),
            lists:foreach(fun(GiveFlowers)->db:dirty_write(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,GiveFlowers) end,GiveList)
    end.
...
update(reset) ->
	RankList = db:dirty_match_object(?DB_ROLE_GIVE_FLOWERS_LAST_WEEK_RANK,#p_role_give_flowers_last_week_rank{ _ = '_'}),
