...
init(RankInfo) ->
    ...
    case db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_RANK,#p_role_rece_flowers_rank{_='_'}) of
        []->
            init_rank_data(RankSize);
        RoleRankList1 ->
            RoleRankList2 = rank2(RoleRankList1),
            common_title:add_title(?TITLE_ROLE_RECE_FLOWERS,0,RoleRankList2),
            NewList = lists:foldr(
                        fun(RoleRank2,Acc2)->
...
rank()->
    List1 = db:dirty_match_object(?DB_ROLE_RECE_FLOWERS_RANK,#p_role_rece_flowers_rank{_ = '_'}),
    List2 = rank2(List1),
    common_title:add_title(?TITLE_ROLE_RECE_FLOWERS,0,List2).     

update({update,RoleReceFlowersRank}) when is_record(RoleReceFlowersRank,p_role_rece_flowers_rank)->
    RoleID = RoleReceFlowersRank#p_role_rece_flowers_rank.role_id,
    ranking_minheap:update_heap(RoleReceFlowersRank,RoleID,?DB_ROLE_RECE_FLOWERS_RANK,?MODULE,?MODULE);
update({clear,RoleID})->
    db:dirty_delete(?DB_ROLE_RECE_FLOWERS_RANK,RoleID);
update(_)->
    ignore.

%% update({RoleBase,RoleAttr})
%%   when RoleBase#p_role_base.sex =:= 2 andalso RoleAttr#p_role_attr.charm>0 ->
...
%%     ?DEBUG("RECEIVE rolerank : ~w~n",[RoleRank]),
%%     ranking_minheap:update_heap(RoleRank,RoleID,?DB_ROLE_RECE_FLOWERS_RANK,?MODULE,?MODULE);
%% 
%% %% 变性后删除数据
%% update({RoleBase,_RoleAttr})->
%%     ranking_minheap:delete(?MODULE,?MODULE,RoleBase#p_role_base.role_id),
%%     db:dirty_delete(?DB_ROLE_RECE_FLOWERS_RANK,RoleBase#p_role_base.role_id);
%% update(_)->
%%     ignore.
        ...
        List->
            lists:foreach(
              fun(RoleBase) ->
                   ...
                   case RoleAttr#p_role_attr.charm > 0 of
                       true->
                           RoleReceFlowersRank = common_ranking:get_rece_flowers_rank_record(RoleBase, RoleAttr),
                           update({update,RoleReceFlowersRank});
                       false->
                           ignore
                   end
              end,List)
