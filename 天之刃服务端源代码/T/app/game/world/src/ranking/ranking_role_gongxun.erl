...
rank() ->
    ...
    common_title:add_title(?TITLE_ROLE_GONGXUN_RANK,0,List4).
...
%%             ranking_minheap:update_heap(RoleLevelRank,RoleID,?DB_ROLE_GONGXUN_RANK,?MODULE,{?MODULE,FactionID})
%%     end.  
update(RoleGongXunRank) when is_record(RoleGongXunRank,p_role_gongxun_rank)->
    RoleID = RoleGongXunRank#p_role_gongxun_rank.role_id,
    FactionID = RoleGongXunRank#p_role_gongxun_rank.faction_id,
    ranking_minheap:update_heap(RoleGongXunRank,RoleID,?DB_ROLE_GONGXUN_RANK,?MODULE,{?MODULE,FactionID});
update(Info)->
    ?ERROR_MSG("ranking_role_gongxun unknow info :~w",[Info]).
...
get_role_rank(RoleID) ->
    case db:dirty_read(?DB_ROLE_GONGXUN_RANK,RoleID) of
        [] ->
            undefined;
        [RoleLevelRank] ->
            #p_role_gongxun_rank{gongxun = GongXun,ranking = Rank} = RoleLevelRank,
            #p_role_all_rank{key_value=GongXun, ranking = Rank}
    end.
...
init2(RankSize,FactionID) ->
    case db:dirty_match_object(?DB_ROLE_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}) of
        [] ->
            read_rank_data(RankSize,FactionID);
        RoleRankList ->
            rank2(FactionID),
...
rank2(FactionID) ->
    List = db:dirty_match_object(?DB_ROLE_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}),
                      ...
                      case common_misc:get_dirty_role_base(RoleID) of
                          {ok,RoleBase} ->
                              ...
                              case NewGongXun > 0 of
                                  ...
                                  false ->
                                      db:dirty_delete_object(?DB_ROLE_GONGXUN_RANK,RoleGongXunRank),
                                      Acc       
                              end;
                          _ ->
                              db:dirty_delete_object(?DB_ROLE_GONGXUN_RANK,RoleGongXunRank),
    ...
    {_,List4} = lists:foldl(
              fun(RoleGongXunRank,{Rank,Acc}) ->
                      ...
                      db:dirty_write(?DB_ROLE_GONGXUN_RANK,NewRoleGongXunRank),
