...
init(RankInfo) ->
    ...
    case db:dirty_match_object(?DB_ROLE_WORLD_PKPOINT_RANK,#p_role_pkpoint_rank{_ = '_'}) of
        [] ->
...
rank() ->
    List = db:dirty_match_object(?DB_ROLE_WORLD_PKPOINT_RANK,#p_role_pkpoint_rank{_ = '_'}),
    ...
    List2 = lists:foldr(
              fun(RoleWorldPkPointRank,Acc) ->
                      ...
                      case NewPkPoint >= 18 of
                          ...
                          false ->
                              db:dirty_delete_object(?DB_ROLE_WORLD_PKPOINT_RANK,RoleWorldPkPointRank),
                              Acc
                      end
              end,[],TmpList),
    ?DEBUG("~w",[List2]),
    ...
    {_,List4} = lists:foldl(
              fun(RoleWorldPkPointRank,{Rank,Acc}) ->
                      ...
                      db:dirty_write(?DB_ROLE_WORLD_PKPOINT_RANK,NewRoleWorldPkPointRank),
    ...
    common_title:add_title(?TITLE_WORLD_PKPOINT_RANK,0,List4).
...
update(RolePkPointRank) when is_record(RolePkPointRank,p_role_pkpoint_rank) ->
    ...
    ranking_minheap:update_heap(RolePkPointRank,RoleID,?DB_ROLE_WORLD_PKPOINT_RANK,?MODULE,?MODULE);
update(Info)->
    ?ERROR_MSG("ranking role world pkpoint unknow info:~w",[Info]).
...
get_role_rank(RoleID) ->
    case db:dirty_read(?DB_ROLE_WORLD_PKPOINT_RANK,RoleID) of
        [] ->
            undefined;
        [RoleLevelRank] ->
