...
get_role_rank(RoleID) ->
    case db:dirty_read(?DB_ROLE_YESTERDAY_GONGXUN_RANK,RoleID) of
        [] ->
            undefined;
        [RoleLevelRank] ->
            #p_role_gongxun_rank{gongxun = GongXun,ranking = Rank} = RoleLevelRank,
            #p_role_all_rank{rank_name= <<"昨日战功榜">>, key_name= <<"战功">>, key_value=GongXun, ranking = Rank}
    end.
   

update(RoleGongXunRankList,FactionID) ->
    OldRoleRankList = db:dirty_match_object(?DB_ROLE_YESTERDAY_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}),
    lists:foreach(fun(OldRank) -> db:dirty_delete_object(?DB_ROLE_YESTERDAY_GONGXUN_RANK,OldRank) end, OldRoleRankList),
    lists:foreach(fun(Rank) -> db:dirty_write(?DB_ROLE_YESTERDAY_GONGXUN_RANK,Rank) end, RoleGongXunRankList),
...
init2(FactionID) ->
    RoleRankList = db:dirty_match_object(?DB_ROLE_YESTERDAY_GONGXUN_RANK,#p_role_gongxun_rank{faction_id = FactionID, _ = '_'}),
