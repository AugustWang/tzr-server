...
-define(KING_LETTER_TITLE,"国王玉玺").
%%张居正NPC ID
-define(NPC_ZHANG_JU_ZHENG(FactionID),erlang:element(2,lists:keyfind(FactionID,1,[{1,11100109},{2,12100109},{3,13100109}]))).
...
do_check_take_equip(RoleID,TakeOfficeID) ->
    ...
    case OfficeID =:= TakeOfficeID of
        true ->
            ok;
        false ->
            erlang:throw({error, io_lib:format(?_LANG_OFFICE_NOT_RIGHT_TO_TAKE_OFFICE_EQUIP,[common_tool:to_list(?OFFICE_NAME(TakeOfficeID))])})
    end,
    %%王座争霸战期间
    ...
    if
        ...
        true ->
            case db:transaction(fun() -> t_do_set_king(RoleID, OldKingRoleID, ?OFFICE_ID_KING, KingName) end) of
                {atomic, {OldOfficeID, NewRoleAttr, NewOldKingRoleAttr}} ->                
                    ...
                    %%添加称号
                    common_title:add_title(?TITLE_KING, RoleID, FactionID),
                    common_misc:unicast({role, RoleID}, ?DEFAULT_UNIQUE, ?ROLE2, ?ROLE2_ATTR_RELOAD, 
                                        #m_role2_attr_reload_toc{role_attr=NewRoleAttr}),
                    case OldKingRoleID > 0 of
                        true ->
                            lose_office_retrieve_equip(OldKingRoleID,common_office:get_king_name(FactionID)),
                            common_misc:unicast({role, OldKingRoleID}, ?DEFAULT_UNIQUE, ?ROLE2, ?ROLE2_ATTR_RELOAD, 
                                                #m_role2_attr_reload_toc{role_attr=NewOldKingRoleAttr});
                        false ->
                            ignore
                    end,
                    case OldOfficeID =:= ?OFFICE_ID_KING of
                        ...
                        false ->
                            case OldOfficeID > 0 of
                                true ->
                                    common_title:remove_by_typeid(get_titleid_by_officeid(OldOfficeID), RoleID),
...
lose_office_retrieve_equip(RoleID,OfficeID,OfficeName) ->
    Msg = io_lib:format(?_LANG_OFFICE_RETRIEVE_OFFICE_EQUIP, [common_tool:to_list(OfficeName),
        ...
        {ok, TRoleID, OfficeName, FactionInfo} ->
            case db:transaction(fun() -> t_do_disappoint(TRoleID) end) of
                {atomic, ok} ->
                    ...
                    db:dirty_write(?DB_FACTION, NewFactionInfo),
                    %%解除指定玩家的指定称号
                    common_title:remove_by_typeid(get_titleid_by_officeid(OfficeID), TRoleID),
                    %%通知双方操作结果
                    R = #m_office_disappoint_toc{office_id=OfficeID},
                    common_misc:unicast(Line, RoleID, Unique, Module, Method, R),
        ...
        {ok, RoleName, Head, FactionID, OldOffice, FactionInfo} ->
            ...
            case db:transaction(fun() -> t_do_agree_appoint(RoleID, OfficeID, OfficeName) end) of
                {atomic, _} ->
                    ...
                    db:dirty_write(?DB_FACTION, NewFactionInfo),
                    %%设置称号
                    common_title:add_title(get_titleid_by_officeid(OfficeID), RoleID, FactionID),
                    %% 通知结果
        ...
        true ->
            ...
            case Silver >= MaxSilver of
                ...
                _ ->
                    ...
                    common_role_money:reduce(RoleID,[{silver, Money3, ?CONSUME_TYPE_SILVER_DONATE_FACTION_SILVER, ""}],
        ...
        {atomic, _} ->
            ...
            case Money >= 10000 of
                true ->
                    ...
                    case OfficeName of
                        [] ->
                            Content = io_lib:format(?OFFICE_DONATE_BC_NORMAL, [RoleName, common_misc:format_silver(Money), SexName, FactionName]);
                        _ ->
                            Content = io_lib:format(?OFFICE_DONATE_BC_OFFICE, [OfficeName, RoleName, common_misc:format_silver(Money), SexName, FactionName])
                    end,
                    %%广播通知
