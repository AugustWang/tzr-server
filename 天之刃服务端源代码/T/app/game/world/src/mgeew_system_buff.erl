...
%%%
%%% @end
%%% Created :  1 Nov 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
...
init_activity_monster_exp(IsNext) ->
    case common_config_dyn:find(mccq_activity, activity_monster_exp) of
        [] ->
            ignore;
        [[ActKey|_]] ->
            ...
            case common_activity:check_activity_time(IsOpen, StartTime, EndTime) of
                ...
                {false, RemainTime} ->
                    case RemainTime > 0 of
                        true ->
                            erlang:send_after(RemainTime*1000, self(), {add_world_exp_buff, Multiple, AwardExpLastHour*3600});
                        _ ->
                            init_activity_monster_exp2(T, IsNext)
                    end
            end
