...
router({?CHAT_IN_CHANNEL, _ModuleID, RoleID, DataRecord, GatewayPID, Unique}, State) ->
    ...
    case mod_gm:cmd(RoleID, TmpMsg) of
        {gm, Msg} ->
            Auth = gm;
        {not_gm, Msg} ->
    ...
    case Auth of
        ...
        {false, Reason} ->
            ...
            ?DEBUG("FailDataRecord=~p",[FailDataRecord]),
        ...
        true ->
            ...
            %%        ReduceList = [{silver_any, 200, ?CONSUME_TYPE_CHAT_WORLD, ""}],
            %%        common_role_money:reduce(RoleID, ReduceList, true, false, true);
            %%    true ->
            %%        ignore
            %%end,
            ?DEV("~ts:~w", ["通知频道进程广播聊天", gen_server:cast({global, ChannelSign}, {chat, RoleID, RoleChatInfo, Msg})]),
            SuccDataRecord = 
                #m_chat_in_channel_toc{succ=true, 
                                       msg=Msg,
                                       channel_sign=ChannelSign,
            ...
            GatewayPID ! {message, Unique, ?CHAT, ?CHAT_IN_CHANNEL, SuccDataRecord},
            ?DEV("~w", [mgeec_logger:log_channel(ChannelSign, RoleChatInfo, Msg)]),
            NewState = State#chat_role_state{last_chat_time=erlang:now()}            
    end,
    NewState;

router({?CHAT_IN_PAIRS, _ModuleID, RoleID, DataRecord, GatewayPID, Unique}, State) ->
    RoleChatInfo = State#chat_role_state.chat_role_info,    
    ...
    case Auth of
        ...
        true ->            
            ...
            common_hook_achievement:hook({chat_module,{?CHANNEL_TYPE_PAIRS,RoleID}}),
            NewState = State#chat_role_state{last_chat_time=erlang:now()}
...
do_level_change_channel(OldLevel, NewLevel, FactionID, State) ->
    ...
    if
        NewChannelInfo =/= false -> 
            ...
            ?DEV("do_level_change_channel, oldchannelinfo: ~w, newchannelinfo: ~w", [OldChannelInfo, NewChannelInfo]),
            #chat_role_state{role_id=RoleID, role_base=RoleBase, role_attr=RoleAttr, role_ext=RoleExt} = State,
            case OldChannelInfo of
                false ->
                    d_new_join_channel(RoleID, NewChannelType, NewChannelSign, RoleBase, RoleAttr, RoleExt),
...
d_new_join_channel(RoleID, ChannelType, ChannelSign, RoleBase, RoleAttr, RoleExt) ->
    try
        ChannelData = 
            #r_chat_role_channel_info{channel_sign=ChannelSign, 
                                      role_id=RoleID, 
