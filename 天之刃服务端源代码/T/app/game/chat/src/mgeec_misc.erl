...
cast_role_router(Key, RouterData) ->
...
%% --------------------------------------------------------------------
%% Function: d_get_chat_init_data/1
%% Description: 获取聊天服务器的角色信息
%% Parameter: int() RoleId 角色id |
%%            string()  RoleName  角色名
%% Return: tuple() {RoleBase #p_role_base, ChatRoleInfo, ChannelList} |
%%         bool()  false
%% --------------------------------------------------------------------
d_get_chat_init_data(Key) ->

    try
        RoleBase = d_get_role_base(Key),
        RoleAttr = d_get_role_attr(Key),
        RoleExt = d_get_role_ext(Key),
        ...
        ChannelList = get_channel_list(RoleBase, RoleAttr),
        ?DEV("d_get_chat_init_data, channellist: ~w", [ChannelList]),
              ...
              fun(Item) ->
                      ...
                      case ChannelType of
                          ?CHANNEL_TYPE_LEVEL ->
                              ...
                              %%玩家上线时更新下自己的信息
                              {_, NewChannelInfo} =
                                  dn_update_channel_role_info(RoleID, 
                                                              ChannelSign, 
                                                              ?CHANNEL_TYPE_LEVEL, 
                                                              true, 
                                                              RoleBase, 
        ...
        ?DEV("d_get_chat_init_data, newchannellist: ~w", [NewChannelList]),

        {RoleBase#p_role_base{role_name=NewRoleName}, 
         RoleAttr#p_role_attr{role_name=NewRoleName}, 
         RoleExt,
         ChatRoleInfo, 
         NewChannelList}
        ...
        _:Reason ->
            ?ERROR_MSG("~ts:~w ~w", ["脏读玩家数据失败了", Reason, erlang:get_stacktrace()]),
            false
    end.
...
%% --------------------------------------------------------------------
%% Function: get_channel_list/2
%% Description: 生成玩家的频道列表
%% Parameter: record() RoleAttr #p_role_attr
%% Parameter: record() RoleBase #p_role_base
%% Return: list() [#p_channel_info]
%% --------------------------------------------------------------------

get_channel_list(RoleBase, RoleAttr) ->    

    do_get_channel_list(RoleBase, RoleAttr, []).

do_get_channel_list(RoleBase, RoleAttr, ChannelList) ->
    {_, WorldChannelInfo} = 
        common_misc:chat_get_world_channel_info(),

    do_get_channel_list_2(RoleBase, RoleAttr, [WorldChannelInfo|ChannelList]).

do_get_channel_list_2(RoleBase, RoleAttr, ChannelList) ->
    ...
    do_get_channel_list_3(RoleBase, RoleAttr, [FactionChannelInfo|ChannelList]).

do_get_channel_list_3(RoleBase, RoleAttr, ChannelList) ->
    ...
    do_get_channel_list_4(RoleBase, RoleAttr, ChannelList2).

do_get_channel_list_4(RoleBase, RoleAttr, ChannelList) ->
    
    TeamID = RoleBase#p_role_base.team_id,
    ...
    do_get_channel_list_5(RoleBase, RoleAttr, ChannelList2).
    
do_get_channel_list_5(RoleBase, RoleAttr, ChannelList) ->
    ?DEV("do_get_channel_list_5, channellist: ~w", [ChannelList]),

    Level = RoleAttr#p_role_attr.level,
    FactionID = RoleBase#p_role_base.faction_id,
    ChannelInfo = get_level_channel(Level, FactionID),
    ?DEV("do_get_channel_list_5, channelinfo: ~w", [ChannelInfo]),
...
d_get_role_attr(RoleID) when erlang:is_integer(RoleID) andalso RoleID > 0 ->
    {ok, RoleAttr} = common_misc:get_dirty_role_attr(RoleID),
    RoleAttr;

d_get_role_attr(RoleName) when erlang:is_list(RoleName) andalso RoleName =/= "" ->
    RoleID = common_misc:get_roleid(RoleName),
    {ok, RoleAttr} = common_misc:get_dirty_role_attr(RoleID),
    RoleAttr;
...
%%dn-->不带try的脏操作
dn_update_channel_role_info(RoleID, ChannelSign, ChannelType, IsOnline, RoleBase, RoleAttr, RoleExt) ->
