...
-include("behavior/behavior_lang.hrl").
...
init([AdminSocket]) ->
    ...
    {ok,{AgentID,GameID}} = application:get_env(agentid_and_gameid),
        
    put(agentid_and_gameid,{AgentID,GameID}),
    SendIntervalTime = ?DEFAULT_SEND_INTERVAL_TIME,
    erlang:send_after(SendIntervalTime, self(), {send_message,SendIntervalTime}),
...
%%% Internal functions
%%%===================================================================
do_handle_info({event, run},_State) ->
    %%spawn_link(mgeel_s2s_client_receiver, start, [State#state.socket]);
    ignore;
