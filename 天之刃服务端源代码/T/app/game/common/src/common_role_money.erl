...
%%%
%%% @end
%%% Created : 21 Dec 2010 by QingliangCn <>
%%%-------------------------------------------------------------------
...
%% @doc 扣除操作
%% reducelist: [{reduce_type, Num, ConsumeType, ConsumeDetail}...]
%% reduce_type: glod_any, gold_bind, gold, silver_any, silver_bind, silver. any：先扣绑定再扣不不绑定
%% return: {?REDUCE_ROLE_MONEY_SUCC, RoleID, RoleAttr, SuccReturn} or {?REDUCE_ROLE_MONEY_FAILED, RoleID, Reason, FailedReturn}
reduce(RoleID, ReduceList, SuccReturn, FailedReturn)->
    reduce(RoleID, ReduceList, SuccReturn, FailedReturn,false).
reduce(RoleID, ReduceList, SuccReturn, FailedReturn,ShouldNotify) when is_boolean(ShouldNotify) ->
    common_misc:send_to_rolemap(RoleID, {mod_map_role, 
                                         {reduce_money, 
                                          {RoleID, erlang:self(), ReduceList, SuccReturn, FailedReturn,ShouldNotify}}}).
...
add(RoleID, AddList, SuccReturn, FailedReturn,ShouldNotify) when is_boolean(ShouldNotify) ->
    common_misc:send_to_rolemap(RoleID, {mod_map_role, 
                                         {add_money, 
                                          {RoleID, erlang:self(), AddList, SuccReturn, FailedReturn,ShouldNotify}}}).
...
change(RoleID, EventInfo, ChangeList, SuccReturn, FailedReturn) ->
    ...
    case common_misc:is_role_online(RoleID) of
        true ->
            common_misc:send_to_rolemap(RoleID, {mod_map_role,
                                                 {change_money,
                                                  {RoleID, erlang:self(), EventID, ChangeList, SuccReturn, FailedReturn}}});
        false ->
