...
do_clear_table_mnesia(DiskTab) when is_atom(DiskTab)->
    global:send(?db_persistent, {clear_table, DiskTab}).
...
%% @doc 将队列消息发送给db_persistent 进行持久化
    %%global:send(?db_persistent, {cache_queue, lists:reverse(get_q())}),
do_mnesia_queues( QueueList )->
	global:send(?db_persistent, {cache_subscriber_queue, QueueList}),
	ok.
