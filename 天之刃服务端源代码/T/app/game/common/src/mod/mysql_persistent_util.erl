...
%% Exported Functions
%%
-export([generate_sql_create_table/0]).
...
-define( SQL_FILE_PATH, "/data/mtzr/hrl/sql/persistent_tables.sql" ).
...
generate_sql_create_table()->
	generate_sql_create_table(no_drop).
	
generate_sql_create_table(Option)->
    ...
    lists:foreach(fun(TabName)-> 
                      ...
                      do_gensql_create_table(TabName,TabDef,Option)
                  end, TabNameList),
    io:format("generate_sql_create_table: ~p",[?SQL_FILE_PATH]),
    ok.
...
do_gensql_create_table(TabName,TabDef,GenOption) when is_atom(TabName),is_list(TabDef)->
	...
	Bytes = Sql,
	file:write_file( ?SQL_FILE_PATH, list_to_binary(Bytes),[append] ).
