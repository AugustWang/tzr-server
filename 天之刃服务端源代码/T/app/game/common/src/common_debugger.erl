...
spawn_trace(PID) ->    
    ...
    File = lists:concat(["/data/logs/tracer.", erlang:pid_to_list(PID), Year, Month, Day, H, I, ".log"]),
...
start_concurrency_profile() ->
    ...
    File = io_lib:format("/data/logs/ming2_concurrency_~w~w~w_~w~w~w", [Year, Month, Day, Hour, Min, Sec]),
