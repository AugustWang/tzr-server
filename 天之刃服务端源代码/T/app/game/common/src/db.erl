...
-export([
         match_load/3, read_load/3, 
         read_unload/3, match_unload/3,
         load_whole_table/2,
...
-define(db_persistent_process_down, db_persistent_process_down).
...
dirty_read(Tab, Key) ->
    case mnesia:dirty_read(Tab, Key) of
        [] ->
            case is_read_delay_load_tables(Tab) of
                true ->
                    SourceTab = common_tool:list_to_atom(lists:concat([Tab, "_p"])),
                    case mnesia:dirty_read(SourceTab, Key) of
                        [] ->
                            [];
                        [R] ->
    ...
    case mnesia:read(Tab, Key) of
        [] ->
            case is_read_delay_load_tables(Tab) of
                true ->
                    SourceTab = common_tool:list_to_atom(lists:concat([Tab, "_p"])),
                    case mnesia:read(SourceTab, Key) of
                        [] ->
                            [];
                        [R] ->
                            mnesia:write(Tab, R, write),
                            [R]
                    end;
                false ->
                    []
...
is_read_delay_load_tables(Tab)->
    lists:member(Tab, db_loader:delay_load_tables()) orelse lists:member(Tab, db_loader:delay_load_tables_with_offline()).


read(Tab, Key, LockKind) ->
    case mnesia:read(Tab, Key, LockKind) of
        [] ->
            case is_read_delay_load_tables(Tab) of
                true ->
                    SourceTab = common_tool:list_to_atom(lists:concat([Tab, "_p"])),
                    case mnesia:read(SourceTab, Key, LockKind) of
                        [] ->
                            [];
                        [R] ->
                            mnesia:write(Tab, R, write),
...
do_mysql_load(SourceTable, TargetTable)->
	%% 判断是否加载mysql的数据
	case mysql_persistent_config:is_mysql_table(SourceTable) of
		true->
			mysql_persistent_handler:load_whole_table(SourceTable,TargetTable);
		_->
			call_db_persistent({load_whole_table, SourceTable, TargetTable})
	end.

load_whole_table(SourceTable, TargetTable) ->
	%% 全表加载时，必须先去掉订阅
	ensure_db_persistent_started(),
	UnSubscribResult = db_subscriber:call_unsubscribe(TargetTable),
	
	case ?IS_USING_MYSQL_PERSISTENT of
		true->
			do_mysql_load(SourceTable, TargetTable);
		_->
			call_db_persistent({load_whole_table, SourceTable, TargetTable})
	...
	case UnSubscribResult of
		ok->
			db_subscriber:call_subscribe(TargetTable);
		_ -> ignore
	end.
	

%% insert(SourceTable, TargetTable, Key, set) ->
%% 有些表需要以匹配的方式载入数据
match_load(SourceTable, TargetTable, Pattern) ->
    ensure_db_persistent_started(),
    call_db_persistent({match_load, SourceTable, TargetTable, Pattern}).


%%直接读key方式载入数据
read_load(SourceTable, TargetTable, Key) ->
    ensure_db_persistent_started(),
    call_db_persistent({load, SourceTable, TargetTable, Key}).

%%从内存中卸载数据，会自动持久化该条记录
%% @SourceTable 持久化表
%% @TargetTable 内存表
read_unload(SourceTable, TargetTable, Key) ->
    ensure_db_persistent_started(),
    call_db_persistent({unload, SourceTable, TargetTable, Key}).


match_unload(SourceTable, _TargetTable, Pattern) ->
    ensure_db_persistent_started(),
    call_db_persistent({match_unload, SourceTable, Pattern}).


%%call db_persistent
call_db_persistent(Request) ->
    gen_server:call({global, ?db_persistent}, Request, infinity).
    

%%确认db_persistent进程正常服务中
ensure_db_persistent_started() ->
    case global:whereis_name(?db_persistent) of
        undefined ->
            exit(?db_persistent_process_down);
        _ ->
            ok
    end.
