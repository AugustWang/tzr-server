...
do_post_modulelist(AgentID, GameID, PostList)->
	lists:foreach(
	  fun({{ModuleTuple, MethodTuple}, BinList}) ->
			  mgeer_http:post(AgentID, GameID, ModuleTuple, MethodTuple, BinList),
...
do_unpost_module( AgentID, GameID, {_ModuleTuple, MethodTuple, DataBinary} )->
    ...
    try
        case MethodTuple of
            ?B_CONSUME_GOLD->
                Record = mgeer_packet:decode(consume_gold,DataBinary),
                mod_consume_receiver:write_gold_log(AgentID, GameID, Record);
            
            ?B_PAY_LOG ->
                Record = mgeer_packet:decode(pay_log,DataBinary),
