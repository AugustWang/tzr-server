...
handle_call({event, login_again}, _From, State) ->
    ...
    mgeer_packet:send(Socket, ?B_SERVER, ?B_SERVER_MSG, R),
...
handle_info({'EXIT', _Pid, _Reason}, State) ->
    spawn_link(mgeer_tcp_client_receiver, start, [State#state.socket, self()]),
    {noreply, State};
...
handle_info({info, {Module, Method, DataBin}}, State) ->
    mgeer_packet:send(State#state.socket, Module, Method, DataBin),
    {noreply, State};
...
process_name(AgentName, GameID) ->
    lists:append(["mgeer_game_", AgentName, "_", GameID]).
