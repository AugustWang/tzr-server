%%%----------------------------------------------------------------------
%%% File    : mgeer_networking.erl
%%% Author  : Qingliang
%%% Created : 2010-01-02
%%% Description: Ming game engine erlang
%%%----------------------------------------------------------------------

-module(mgeer_networking).
...
start() ->
    {ok, _} = supervisor:start_child(mgeer_sup, {mgeer_tcp_client_sup,
                                                 {mgeer_tcp_client_sup, start_link, []},
                                                 permanent, infinity, supervisor, 
                                                 [mgeer_tcp_client_sup]}).
...
start_listener(Port, AcceptorNum, OnConnect) ->
    {ok,_} = supervisor:start_child(
               mgeer_sup,
               {mgeer_tcp_listener_sup,
                {mgeer_tcp_listener_sup, start_link,
                 [Port, ?TCP_OPTS ,
                  {?MODULE, tcp_listener_started, [localhost]},
                  {?MODULE, tcp_listener_stopped, [localhost]},
                  OnConnect, AcceptorNum]},
                transient, infinity, supervisor, [mgeer_tcp_listener_sup]}),
    ok.

stop_tcp_listener(Host, Port) ->
    {ok, IPAddress} = inet:getaddr(Host, inet),
    Name = common_misc:tcp_name(mgeer_tcp_listener_sup, IPAddress, Port),
    ok = supervisor:terminate_child(mgeer_sup, Name),
    ok = supervisor:delete_child(mgeer_sup, Name),
    ok.
