...
$pay_money   = SS($_REQUEST['PayMoney']);     //充值金额，人民币
...
if(empty($order_id) || empty($ac_name) || empty($pay_gold) 
	|| $pay_gold < 0 || empty($pay_time) || empty($_REQUEST['ticket'])) {
	...
	$token = md5($API_SECURITY_TICKET_PAY . $order_id . $ac_name . $pay_money . $pay_gold . $pay_time);
	//echo $token;
	if($token != $_REQUEST['ticket']) {
...
if ($result == NULL) {
	log_die($logid, 'failed');
} else {
	if ($result['result'] == 'ok') {
		log_die($logid, 'true', 'true');
	}elseif ($result['result'] == 'used') {
		log_die($logid, 'PayNum exist');
	} else {
		log_die($logid, 'failed');
	}
}
