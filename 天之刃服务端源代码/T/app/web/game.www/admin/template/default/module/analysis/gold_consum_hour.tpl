...
<table class="SumDataGrid">
	<tr align="center">
		<th colspan="<{$diffDay+1}>"><{$startDate}> - <{$endDate}>  每天元宝消耗变化柱状图 </th>
	</tr>
	<tr align="center" valign="bottom" height="150">
		<th valign="middle">元宝消耗量</th>
		<{foreach from=$arrByDay.rows item=row}>
		<td><{$row.gold}>
			<hr class="<{if $row.gold>$arrByDay.max_gold*0.75}>hr_red<{else}>hr_green<{/if}>" style=" height:<{if $arrByDay.max_gold>0}><{$row.gold*120/$arrByDay.max_gold|round}><{else}>0<{/if}>px;" />
		</td>
		<{/foreach}>
	</tr>
	<tr align="center">
...
<table class="SumDataGrid">
	<tr align="center">
		<th colspan="25"><{$startDate}> - <{$endDate}>  24小时元宝消耗变化柱状图 </th>
	</tr>
	<tr align="center" valign="bottom" height="150">
		<th valign="middle">元宝消耗量</th>
		<{foreach from=$arrByAllHour.rows item=row}>
		<td><{$row.gold}>
			<hr class="<{if $row.gold>$arrByAllHour.max_gold*0.75}>hr_red<{else}>hr_green<{/if}>" style=" height:<{if $arrByAllHour.max_gold>0}><{$row.gold*120/$arrByAllHour.max_gold|round}><{else}>0<{/if}>px;" />
		</td>
		<{/foreach}>
	</tr>
	<tr align="center">
...
<table class="SumDataGrid">
	<tr align="center">
		<th colspan="26"><{$startDate}> - <{$endDate}>  元宝消耗量24小时统计趋势图  </th>
	</tr>
	...
	<tr align="center" valign="bottom" height="150" <{if $rowDay.onlineDate%2 ==0}>class="odd" <{/if}>>
		...
		<th valign="middle">元宝消耗量</th>
		<{foreach from=$rowDay.subRows key=hour item=gold}>
		<td><{$gold}>
			<hr class="<{if $gold>$arrByHour.max_gold*0.75}>hr_red<{else}>hr_green<{/if}>" style=" height:<{if $arrByHour.max_gold>0}><{$gold*120/$arrByHour.max_gold|round}><{else}>0<{/if}>px;" />
		</td>
		<{/foreach}>
	</tr>
