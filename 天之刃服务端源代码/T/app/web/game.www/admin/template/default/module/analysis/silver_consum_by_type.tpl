...
<b>实际数量</b>:实际开宝箱次数、实际购物道具总数量等 <br>
<b>操作次数</b>:使用绑定和银子的操作次数的总和<br>
<table class="SumDataGrid">
	<tr align="center" valign="bottom" height="150">
		<th valign="middle">银子总消耗量(文)</th>
		<{foreach from=$arrByDay item=row}>
		<td><{$row.silver}>
			<hr class="<{if $row.silver>$maxSilver*0.75}>hr_red<{else}>hr_green<{/if}>" style=" height:<{if $maxSilver>0}><{$row.silver*120/$maxSilver|round}><{else}>0<{/if}>px;" />
		</td>
		<{/foreach}>
	</tr>
