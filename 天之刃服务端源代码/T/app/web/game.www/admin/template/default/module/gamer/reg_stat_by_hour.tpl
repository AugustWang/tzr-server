...
<!-- SECTION  START -------------------------->
	...
	<tr class='table_list_head'>
		<td ></td><td>年</td><td>月</td><td>日</td><td>时</td><td>注册人数</td>
	</tr>

<form id="form1" name="form1" method="post" action="">
<{section name=loop loop=$keywordlist}>

	<{if $keywordlist[loop].hour==null || $keywordlist[loop].day==null || $keywordlist[loop].month==null || $keywordlist[loop].year==null}>
	<tr class='trRollup'>
	<{else}>
	...
	<{/if}>
		...
		<{if $keywordlist[loop].hour==null && $keywordlist[loop].day==null && $keywordlist[loop].month==null && $keywordlist[loop].year!=null}>年总计
		<{else}>
		<{$keywordlist[loop].month}>
		<{/if}>
		</td><td>
		<{if $keywordlist[loop].hour==null && $keywordlist[loop].day==null && $keywordlist[loop].month!=null && $keywordlist[loop].year!=null}>月总计
		<{else}>
		<{$keywordlist[loop].day}>
		<{/if}>
		</td><td>
		<{if $keywordlist[loop].hour==null && $keywordlist[loop].day!=null && $keywordlist[loop].month!=null && $keywordlist[loop].year!=null}>日总计
		<{else}>
