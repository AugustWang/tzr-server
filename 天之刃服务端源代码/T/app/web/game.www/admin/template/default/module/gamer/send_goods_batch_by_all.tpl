...
<a href="<{$URL_SELF}>?action=byAll" style="color: red;"><b>全部发送</b></a>
<a href="<{$URL_SELF}>?action=history" style="color: blue; text-decoration: underline;"><b>已发出信件</b></a>
<br /><br />
<font>注：只会发送14天内登录过的玩家</font>
		<form id="frm" action="?action=byAll" method="POST">
			<{if $err}>
...
　　document.all.time_state.options[s].selected=true;
		...
		<!-- end 支持as3 的富文本编辑器	-->
				...
				<tr>
					<td colspan="2" style="padding-left:100px;"><span style="color:red;" id="receiverCnt"></span>
						<input type="submit" onclick="return confirm('注意！你将给全部玩家发送道具。确定要发送？');" value="发 送"  />
					</td>
				</tr>
			</table>
		</form>	
