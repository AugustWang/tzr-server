...
<{/if}>
	...
	<br>
	<div>从<{$search_keyword1}>到<{$search_keyword2}>全部玩家首次元宝消费等级分布情况：
	<{ if $rsLevel }>
		...
		<table class="SumDataGrid" cellspacing="0" style="margin:5px;">
			<tr>
				...
				<{ foreach from=$rsLevel item=row }>
					<td align="center" height="120" valign="bottom"><{ $row.rate }>%<hr class="<{if $row.rate/$maxLevelRate > 0.75 }>hr_red<{else}>hr_green<{/if}>"  style="height:<{ $row.rate|round }>px;" /></td>
				<{ /foreach}>
			</tr>
			<tr>
				<th width="100">人数</th>
				<{ foreach from=$rsLevel item=row }>
					<td align="center" height="120" valign="bottom"><{ $row.cnt }><hr class="<{if $row.cnt/$maxLevelCnt > 0.75 }>hr_red<{else}>hr_green<{/if}>"  style="height:<{ $row.rate|round }>px;" /></td>
				<{ /foreach}>
			</tr>
			<tr>
				<th width="100">等级</th>
				<{ foreach from=$rsLevel item=row }>
					<th align="center"><{ $row.level }></th>
				<{ /foreach }>
			</tr>
		</table>
	<{ else }>
