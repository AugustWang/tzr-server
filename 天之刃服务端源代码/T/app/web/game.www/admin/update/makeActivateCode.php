 ...
 * 		并同时保存到同目录下的文件  MING_AGENTER . MY_SERVER_ID . active_number
 * 
 * 注意：
...
if ($paramType && $paramTimes && $paramCount  ) {
	...
	die('参数错误，运行示例：php ./makeActivateCode.php type=1 times=1 count=10');
}


//产生激活码的加密KEY，每个代理商的每个服，都不同。
$KEY='iVcIo9eH,cvjks3#' . AGENT_NAME . 'C,3Dx,fd3' . SERVER_ID;
...
$filename = AGENT_NAME . '_' . SERVER_ID . '_activate_code_' . $publishType . '_' . $publishTimes . '_' . strftime("%Y%m%d") . '.txt';
$text = '';
$arr = array();
