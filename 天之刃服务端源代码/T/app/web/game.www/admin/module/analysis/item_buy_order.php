...
$sql = "SELECT `buy_count` FROM ".T_STAT_ITEM_BUY." WHERE `mtime`>={$dateStartStamp} AND `mtime`<={$dateEndStamp}";
$rs  = GFetchRowSet($sql);

$tmp_rs = array();
foreach($rs as $key=>$value){
	$tmp_rs[$key] = extractData($value['buy_count']);
}
...
for($i=1;$i<=$num;$i++){
	...
	$order_data[$max_key] = $max_buy;
	unset($buy_data[$max_key]);
}


$sql2 = "SELECT `buy_count` FROM ".T_STAT_ITEM_BUY." WHERE `mtime`>={$last_days_start} AND `mtime` < {$dateStartStamp}";
$rs2 = GFetchRowSet($sql2);

$tmp_rs2 = array();
foreach($rs2 as $key=>$value){
	$tmp_rs2[$key] = extractData($value['buy_count']);
}
...
for($i=1;$i<=$num;$i++){
	$max_buy = 0;
	$max_key = 0;
	foreach($buy_data2 as $key=>$value){
		if($value >$max_buy){
			$max_buy = $value;
			$max_key = $key;
		}
	}
	$order_data2[$max_key] = $max_buy;
	unset($buy_data2[$max_key]);
}
...
$smarty->display("module/analysis/item_buy_order.tpl");
?>
