...
$sqlLevel = " SELECT `level`, COUNT(`id`) AS `cnt` FROM t_log_use_gold WHERE is_first=1 AND `mtime` BETWEEN {$dateStartStamp} AND {$dateEndStamp} GROUP BY `level` ORDER BY `level` ";
$rsLevel = GFetchRowSet($sqlLevel);
$levelTotalCnt = 0 ;
$maxLevelCnt = 0;
$maxLevelRate = 0;
foreach ($rsLevel as &$row) {
	$levelTotalCnt += $row['cnt'];
	if ($row['cnt'] > $maxLevelCnt) {
		$maxLevelCnt = $row['cnt'] ;
	}
}
foreach ($rsLevel as &$row) {
	$row['rate'] = $levelTotalCnt > 0 ? round($row['cnt']*100/$levelTotalCnt,1) : 0 ;
	if ($row['rate'] > $maxLevelRate) {
		$maxLevelRate = $row['rate'];
	}
}
...
$smarty->assign("rsLevel",$rsLevel);
$smarty->assign("maxLevelCnt",$maxLevelCnt);
$smarty->assign("maxLevelRate",$maxLevelRate);
...
$smarty->display("module/analysis/gold_first_use_stat.tpl");
exit;
//////////////////////////////////////////////////////////////
...
function getGoldUseStatData($startTime, $endTime, $checkArr = array())
{
	...
	if ($userid > 0)	//统计指定的玩家，自动忽略其它任何条件
	{
		...
		$sql = "SELECT l.`mtype`, count( l.`id` ) AS c, sum( l.`amount` ) AS ss ,( sum(`gold_bind`)+sum(`gold_unbind`) ) as gold, sum(l.`gold_bind`) AS gold_bind ,sum(l.`gold_unbind`) AS gold_unbind "
		 ...
		 . " WHERE l.is_first=1 AND `mtime`>={$startTime} AND `mtime`<={$endTime} "
	...
	else	//以下 不针对特定某个人进行统计
	{
		...
		{	//统计全部人的
			...
			$sql = "SELECT `mtype`, count( `id` ) AS c, sum( `amount` ) AS ss ,( sum(`gold_bind`)+sum(`gold_unbind`) ) as gold, sum(`gold_bind`) AS gold_bind ,sum(`gold_unbind`) AS gold_unbind "
				. " FROM `t_log_use_gold` "
				. " WHERE is_first=1 AND `mtime`>={$startTime} AND `mtime`<={$endTime} "
...
function getBuyLogStats ($level_type, $userid, $order, $timeStampStart = null, $timeStampEnd = null)
{
	...
	if (! $userid)
	{
		if($level_type == -1 || ($level_type == null && $level_type != 'all'))
		{
			$sql = "SELECT user_id,itemid,COUNT(itemid) as op_count, SUM(amount) as amount, " .
					...
					" SUM(`gold_bind`) as gold_bind , SUM(`gold_unbind`) as gold_unbind " . " FROM `t_log_use_gold` WHERE is_first=1 AND mtype={$typeBuyItem} ";
			if ($timeStampStart)
		...
		else
		{
			$sql = "SELECT tl.user_id,tl.itemid,COUNT(tl.itemid) as op_count, SUM(tl.amount) as amount, " .
					"( sum(`gold_bind`)+sum(`gold_unbind`) ) as gold," .
					" SUM(tl.gold_bind) as gold_bind,us.role_id " . " FROM `t_log_use_gold` tl,`db_role_base_p` us WHERE tl.is_first=1 AND tl.mtype={$typeBuyItem} ";
			if ($timeStampStart)
	...
	else
	{
		$sql = "SELECT `itemid`,COUNT(`itemid`) as op_count, SUM(`amount`) as amount, " .
				"( sum(`gold_bind`)+sum(`gold_unbind`) ) as gold," .
				" SUM(`gold_bind`) as gold_bind , SUM(`gold_unbind`) as gold_unbind " . " FROM `t_log_use_gold` WHERE  is_first=1 AND `mtype`={$typeBuyItem} AND `user_id`={$userid} " ;
		if ($timeStampStart)
			$sql .= " AND mtime >= {$timeStampStart} ";
		if ($timeStampEnd)
			$sql .= " AND mtime <= {$timeStampEnd} ";
