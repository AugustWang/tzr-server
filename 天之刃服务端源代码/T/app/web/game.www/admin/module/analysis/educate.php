...
{
	$where =" AND in_role_ids  like  "."'%{{$base['role_id']}}%' ";
}
else
{
	$where = "";
...
$sqlCnt = " SELECT count(*) as cnt FROM t_log_educate WHERE TRUE {$where} ";
$rsCnt = GFetchRowOne($sqlCnt);
$cnt = intval($rsCnt['cnt']);
$offset = ( $pageno-1 ) * $itemPerPage;

$sqlEducate = " SELECT * FROM t_log_educate WHERE TRUE {$where} order by start_time desc limit {$offset} , {$itemPerPage} ";
$rsEducate = GFetchRowSet($sqlEducate);
$pagelist	= getPages($pageno, $cnt, $itemPerPage);

foreach ($rsEducate as &$row) {
...
$smarty->assign($data);
$smarty->display("module/analysis/educate.tpl");
exit();