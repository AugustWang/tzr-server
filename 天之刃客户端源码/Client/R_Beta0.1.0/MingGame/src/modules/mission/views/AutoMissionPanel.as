package modules.mission.views {
	...
	public class AutoMissionPanel extends BasePanel {
		...
		public function AutoMissionPanel() {
			super();
			this.width=546;
			this.height=393;
...
//				this._tipsView.y=(itemNum + 1) * 26;
		...
		private function initView():void {
			...
			backUI.addChild(_AutoMissionList);
			...
			this._startBtn.x=backUI.x + backUI.width - this._startBtn.width - 3;
			this._startBtn.y=backUI.y + backUI.height;
			...
			_tipsText=ComponentUtil.createTextField("", backUI.x, _startBtn.y, tf, 200, 30, this);

			_tipsText.text='委托任务不会消耗精力值';
			_tipsText.mouseEnabled=false;
			_tipsText.filters=[Style.BLACK_FILTER];

			_costTotalText=ComponentUtil.createTextField("", _tipsText.x + _tipsText.textWidth + 10, this._startBtn.y, tf,
				100, 30, this);
