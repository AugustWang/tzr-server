package modules.conlogin.views {
	...
	public class ConloginRewardList extends UIComponent {
		...
		public function ConloginRewardList(rewardID:int, next_day:int, need_day:int) {
			...
			var titleBg:UIComponent=ComponentUtil.createUIComponent(1, 1, 328, 27);
			Style.setBorder1Skin(titleBg);
			addChild(titleBg);

			var titleXiushi:Sprite=Style.getViewBg("titleBg");
			titleXiushi.x=(328 - titleXiushi.width) / 2;
			titleXiushi.mouseChildren=titleXiushi.mouseEnabled=false;
			titleBg.addChild(titleXiushi);

			var tf:TextFormat=Style.textFormat;
			tf.align="center";
			tf.bold=true;
			titleText=ComponentUtil.buildTextField("", tf, 328, 25, titleBg);
			titleText.mouseEnabled=true;
			titleText.selectable=true;
			titleText.y=2;

			var itemBg:Sprite=Style.getSpriteBitmap(GameConfig.T1_VIEWUI,"borderItemBg");
			itemBg.x=28;
			itemBg.y=43;
			addChild(itemBg);

			goodsItem=new GoodsImage();
			goodsItem.x=goodsItem.y=13;
		...
		private function initBuyView():Sprite {
			...
			if (condition == '') {
				...
				var tf:TextFormat = Style.textFormat;
				tf.color = 0xFF0000;
				conditionText = ComponentUtil.createTextField(condition, 0, 25, tf, 236, 22, sp);
		...
		private function initGetView():Sprite {
			...
			if (condition == '') {
				...
				tf2.color = 0xFF0000;
				conditionText = ComponentUtil.createTextField(condition, 0, 25, tf2, 236, 22, sp);
				conditionText.mouseEnabled = true;
				conditionText.selectable = true;
			}
			return sp;
