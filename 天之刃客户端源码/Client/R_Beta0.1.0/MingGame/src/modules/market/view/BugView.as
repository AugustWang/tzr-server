package modules.market.view
{
	...
	public class BugView extends BasePanel
	{
		
		private var goodsItem:DisplayGoods;
		 ...
		 * 
		 */		
		public function setGoodsVo(ownerID:int,money:String,vo:BaseItemVO):void
		{
			if(!vo)
				return;
		...
		{
			goodsItem = new DisplayGoods();
			goodsItem.x = 20;
			goodsItem.y = 13;
			goodsItem.addEventListener(MouseEvent.ROLL_OUT, onMouseRollOutHandler);
			goodsItem.addEventListener(MouseEvent.ROLL_OVER, onMouseRollOverHandler);
			addChild(goodsItem);
		...
		private function onMouseRollOverHandler(e:MouseEvent):void{
			if (this.goodVO != null) {
				ToolTipManager.getInstance().show(this.goodVO, 0, 0, 0, "targetToolTip");
			}
		}
		...
		private function onOKhandler(evt:MouseEvent):void
		{
			if(num > 0)
			{
				MarketModule.getInstance().requestBuy(ownerId,goodsId,int(numInputTxt.text),price,goodVO);
				closeViewWindow();
			}else{
