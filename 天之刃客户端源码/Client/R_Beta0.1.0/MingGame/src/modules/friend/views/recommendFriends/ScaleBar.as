package modules.friend.views.recommendFriends
{
	...
	public class ScaleBar extends DragUIComponent
	{
		private static var _instance:ScaleBar;
		public static function getInstance():ScaleBar{
			if(!_instance){
				_instance = new ScaleBar();
			}
			return _instance;
		}
		public function ScaleBar()
		{
			...
			/*
			var recommendTxt:TextField = ComponentUtil.createTextField("推荐好友",35,10,null,70,25,this);
		...
		private function onClickForSmallHandler(evt:MouseEvent):void{
			WindowManager.getInstance().removeWindow(this);
			RecommendFriendView.getInstance().visible = true;
			RecommendFriendView.getInstance().x = this.x;
			RecommendFriendView.getInstance().y = this.y;
		}
	}
}