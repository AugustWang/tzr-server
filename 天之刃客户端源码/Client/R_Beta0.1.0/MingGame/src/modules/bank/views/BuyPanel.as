...
//		private var _cancelBtn:Button;
		...
		private function initView():void
		{
			...
			var numtxt:TextField = ComponentUtil.createTextField("买      入：",16,22,tf,60,20,this);
			var ybpricetxt:TextField = ComponentUtil.createTextField("买入单价：",16,50,tf,60,20,this);
			var paytxt:TextField = ComponentUtil.createTextField("共 需 要：",16,78,tf,60,20,this);
...
//			_moneyTxtIpt = createinputtext(88,76,144,NaN,this,false);
			...
			//43  111   77
			_sureBtn = ComponentUtil.createButton("买入元宝",91,129,93,25,this);
		...
		private function onShowTips(e:MouseEvent):void
		{
			...
			if(price>0&&num>0)
			{
				str = "花费："+ MoneyTransformUtil.silverToOtherString(price * num) + "\n扣手续费："+
		...
		public function setUnitPrice(value:int):void
		{
//			_unitPriceTF.text = "元宝单价：" + MoneyTransformUtil.silverToOtherString(value);
//			perPrice = value;
			
			if(value>0)
			{
				price = value;
				_priceTxtIpt.text = String(int(price/100));
		...
		private function onNumChange(e:Event):void
		{
			if(_numTxtIpt.text == ""||_numTxtIpt.text == "0")
			{
				...
				if(BankModule.getInstance().selfBuyArr.length>=5 && num>sumNum)
				{
					num = sumNum;
					_numTxtIpt.text = num+"";
				}
		...
		private function onPriceChange(e:Event):void
		{
			if(_priceTxtIpt.text == ""||_priceTxtIpt.text == "0")
			{
				...
				price = int(_priceTxtIpt.text) * 100;  // 单位是两。
			}
			if(num>0)
				setMoneyTxt();
		}
		...
		{
			var costMoney:Number = price * num*1.0 + price * num * BankConstant.FEERATE;
			var m_silvers:Number = Number( GlobalObjectManager.getInstance().user.attr.silver);
			var nOenough:String = "";
			
			if(costMoney>m_silvers)
			{
				_sureBtn.enabled = false;
				nOenough = "<font color='#f53f3c'>(银子不足)</font>";
			}else{
				_sureBtn.enabled = true;
			}
			
			_moneyTxt.htmlText = MoneyTransformUtil.silverToOtherString(costMoney) + nOenough; 
//			_moneyTxtIpt.text = MoneyTransformUtil.silverToOtherString(costMoney); 
		}
...
//			BankModel.getInstance().submitFn(num,price);
			...
			if(sumNum==0)
				sumNum = num;
			BankModule.getInstance().buyHandler(price,num,sumNum);
		}
