package modules.roleStateG.views.details
{
	...
	public class RoleOtherInfoView extends Sprite
	{
		...
		public function RoleOtherInfoView()
		{
			super();
			setupUI();
		}
		...
		public function setupUI():void
		{
			...
			var jiazuTF:TextField=ComponentUtil.createTextField("家族：", 12, 42, ttf, 45, 25, topBg);
			...
			var mengpaiTF:TextField=ComponentUtil.createTextField("门派：", 12, 14, ttf, 45, 25, topBg);
			mengpaiInput=createTextInput(mengpaiTF.x + 42, mengpaiTF.y - 2, 138, 24, topBg);
		...
		public function update(vo:m_role2_getroleattr_toc):void
		{
			...
			guojiInput.text=GameConstant.getNation(roleVO.faction_id);
			jiazuInput.text=roleVO.family_name;
			guanzhiInput.text=roleVO.office_name;
			//mengpaiInput.text=roleVO.base.family_name;
			
			shengwangPointInput.text=String(roleVO.cur_prestige); //当前声望
			jingliPointInput.text=String(roleVO.charm); //精力
			zhanggongPointInput.text=String(roleVO.gongxun); //战功
			pkPointInput.text=String(roleVO.pk_point); //PK
			meiliPointInput.text=String(roleVO.charm); //魅力
			
			var rankTitle:String="";
			if (rankTitle == "")
			{
				var lv:int=roleVO.level;
				if (lv < 50)
				{
					rankTitle="初入江湖";
				}
