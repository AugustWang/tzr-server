package modules.finery.views.item {
	...
	public class ExaltEquipItemRender extends UIComponent implements IDataRenderer {
		...
		public function ExaltEquipItemRender() {
			super();
			...
			bg.graphics.drawRoundRect(0,0,118,46,6,6);
			bg.graphics.endFill();
			this.addChild(bg);
			...
			numTxt=ComponentUtil.createTextField("", box.x + box.width + 2, nameTxt.y + nameTxt.textHeight + 2, null, 100,
				45, this);
		...
		private function onMouseOverHandler(event:MouseEvent):void {
			if (!_select) {
				bg.filters=[new GlowFilter(0xFBCF22, 0.5, 12, 12, 2, 1, true)];
			}
		}
		...
		public function select(value:Boolean):void {
			...
			_select=value;
			if (_select) {
				bg.filters=[new GlowFilter(0xFBCF22, 0.9, 6, 6, 2, 1, true)]; //#5CEAE7 FBCF22 #00EBF8
			} else {
				bg.filters=[];
			}
		}
		...
		override public function set data(value:Object):void {
			...
			switch (FineryModule.getInstance().getCurrentIndex()) {
				...
				case StovePanel.TABNAME_ARRAY.EQUIP_EXALT:
					...
					if (exaltEquip) {
						numTxt.htmlText=StoveMaterialFilter.percent(exaltEquip, data as BaseItemVO) + '%';
					} else {
						numTxt.htmlText="";
					}
					break;
